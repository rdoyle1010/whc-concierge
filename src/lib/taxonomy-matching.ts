...
function getLabel(score: number): { label: string; colour: string; bgColour: string } {
  ...
  if (score >= 45) return { label: 'Partial Match', colour: '#57544c', bgColour: '#F3F4F6' }
  return { label: 'Low Match', colour: '#57544c', bgColour: '#F3F4F6' }
}
...
export function calculateStructuredMatch(
  ...
  if (blocker.blocked) {
    ...
    return {
      ...
      label: 'Blocked', colour: '#57544c', bgColour: '#F3F4F6',
