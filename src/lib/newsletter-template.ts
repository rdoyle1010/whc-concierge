...
<body style="margin:0;background:#f3f6f8;font-family:Inter,-apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif;color:#1a1a1a">
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f6f8;padding:28px 12px">
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:${maxWidth}px;background:#ffffff;border:1px solid #e5e5e5;border-radius:20px;overflow:hidden">
        <tr><td style="background:#111111;padding:26px 30px;color:#fff">
        ...
        <tr><td style="padding:${layout === 'simple' ? '30px' : '36px 34px'}">
          ...
          ${featuredHtml}
          ${ctaUrl && ctaLabel ? `<div style="margin:30px 0 8px"><a href="${escapeHtml(ctaUrl)}" style="display:inline-block;background:#111111;color:#fff;text-decoration:none;padding:13px 20px;border-radius:8px;font-size:13px;font-weight:650">${ctaLabel}</a></div>` : ''}
        </td></tr>
