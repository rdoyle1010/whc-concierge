...
export function calculateMatchScore(candidate:any, job:any): {
  ...
  const empty={score:10,label:'Requirement Missing',colour:'#5a6a76',bgColour:'#F3F4F6',breakdown:emptyBreakdown,matchingSkills:[] as string[],hardStop:true,matchExplanation:'',distanceMiles:null,locationBasis:'unknown' as const}
  ...
  const colour=rounded>=90?'#16A34A':rounded>=75?'#1D4ED8':rounded>=60?'#D97706':'#5a6a76'
