...
export function featuredCard({ image, title, subtitle, href }: FeaturedCard): string {
  ...
  return `<a href="${escapeHtml(safeHttpUrl(href) || SITE)}" style="display:block;padding:14px;border:1px solid #e5e5e5;border-radius:12px;text-decoration:none;margin-bottom:10px;background:#fff">`
    ...
    + (safeImage
      ...
      : `<div style="width:52px;height:52px;border-radius:50%;background:#222321;color:#fff;text-align:center;line-height:52px;font-weight:600">${initial}</div>`)
    + `</td><td>`
    + `<span style="display:block;font-weight:600;color:#222321;font-size:14px">${escapeHtml(title)}</span>`
    + `<span style="display:block;font-size:12px;color:#57544c;margin-top:2px">${escapeHtml(subtitle)}</span>`
    + `</td></tr></table></a>`
}
...
export function featuredBlock(cards: FeaturedCard[]): string {
  ...
  return `<div style="margin:28px 0">`
    + `<p style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#57544c;font-weight:600;margin:0 0 12px">Featured this week</p>`
...
export function sponsorBlock(sponsor: Sponsor | null | undefined): string {
  ...
  return `<div style="margin:30px 0;border:1px solid #e5e5e5;border-radius:14px;overflow:hidden">`
    + `<div style="background:#ede8df;padding:8px 16px;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;color:#57544c;font-weight:700">Sponsored &middot; ${escapeHtml(name)}</div>`
    ...
    + (headline ? `<p style="margin:0 0 8px;font-size:17px;line-height:1.35;font-weight:650;color:#222321">${escapeHtml(headline)}</p>` : '')
    + (text ? `<p style="margin:0;font-size:14px;line-height:1.7;color:#465761">${escapeHtml(text)}</p>` : '')
    + (url
      ? `<div style="margin-top:16px"><a href="${escapeHtml(url)}" rel="nofollow sponsored" style="display:inline-block;background:#222321;color:#fff;text-decoration:none;padding:11px 18px;border-radius:8px;font-size:13px;font-weight:650">Find out more</a></div>`
      : '')
    + `</div></div>`
}
