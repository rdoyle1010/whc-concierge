...
- progressionSignals should explain readiness evidence, e.g. "Manages team and payroll responsibilities relevant to Spa Manager progression". Do not state they qualify for a role automatically.
- Return JSON only.
...
Return exactly this shape:
{
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({ model: CV_MODEL, reasoning: { effort: 'low' }, input: prompt, max_output_tokens: 1200 }),
    })
    if (!response.ok) {
      console.error('CV AI analysis error', response.status)
      return null
    }
    const parsed = parseJson(extractResponseText(await response.json()))
    if (!parsed) return null
    return {
      ...deterministic,
      businessSkills: uniq([...(deterministic.businessSkills || []), ...uniq(parsed.businessSkills)], 20),
      careerEvidence: uniq(parsed.careerEvidence, 8),
      progressionSignals: uniq(parsed.progressionSignals, 6),
      evidence: uniq([...(deterministic.evidence || []), 'AI reviewed transferable leadership and commercial evidence'], 20),
      aiEnhanced: true,
    }
  } catch (error) {
    console.error('CV AI analysis fallback', error instanceof Error ? error.message : error)
    return null
  } finally {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: profile } = await admin.from('candidate_profiles').select('user_id, cv_url').eq('id', profileId).single()
    ...
    if (!['pdf', 'docx'].includes(extension)) return NextResponse.json({ error: 'For CV analysis, please use a PDF or modern Word .docx file.' }, { status: 400 })
    ...
    const text = await extractText(Buffer.from(await file.arrayBuffer()), extension)
    if (text.trim().length < 80) return NextResponse.json({ error: 'Very little readable text was found. If this is a scanned CV, upload a text-based PDF or Word .docx file.' }, { status: 422 })

    const deterministic = analyseCvText(text)
    const suggestions = await aiEnhanceCv(text, deterministic) || deterministic

    // Record the explicit user-triggered processing event. Raw CV text is not stored in this ledger.
    await admin.from('consent_events').insert({
      user_id: user.id,
      consent_type: 'ai_cv_analysis',
      action: 'accepted',
      policy_version: '2026-08',
      wording: 'User requested WHC AI CV analysis. CV text is processed for suggestions and is not returned, persisted in the analysis response, or added to the profile without approval.',
      source: 'talent_profile_cv_analysis',
    }).then(() => {}).catch(() => {})
