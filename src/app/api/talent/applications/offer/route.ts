      ...
      if (employer?.user_id) {
        ...
        if (employerEmail) {
          ...
          await sendOfferResponseEmail(
            ...
            `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#10283b"><h2 style="font-weight:600">Offer ${verb}</h2><p style="line-height:1.6"><strong>${(candidate.full_name || 'The candidate').replace(/</g,'&lt;')}</strong> has ${verb} your offer for <strong>${(job.job_title || 'the role').replace(/</g,'&lt;')}</strong>.${note ? `</p><div style="background:#f7f7f7;border:1px solid #e8e0d0;border-radius:12px;padding:16px 20px;margin:20px 0;line-height:1.6;white-space:pre-wrap">${note.replace(/</g,'&lt;')}</div><p style="line-height:1.6">` : ' '}Sign in to take the next step.</p><p style="margin:28px 0"><a href="https://talent.wellnesshousecollective.co.uk/employer/applications" style="background:#0b2f4d;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none">Open applications</a></p><p style="color:#777;font-size:13px">Wellness House Collective</p></div>`,
          )
        }
      }
    }
