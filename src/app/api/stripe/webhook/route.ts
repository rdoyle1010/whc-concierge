...
export async function POST(req: NextRequest) {
  ...
  switch (verified.type) {
    case 'checkout.session.completed': {
      // The work itself lives in lib/stripe-checkout-fulfilment, shared with
      // the confirm route the browser hits on its way back from Stripe. Two
      // copies of "deliver what somebody paid for" would drift, and the one
      // that drifts is the one nobody watches.
      const session = verified.data.object as Stripe.Checkout.Session
      const outcome = await fulfilCheckoutSession(supabase, session, { requestUrl: req.url })
      if (!outcome.ok && outcome.retry) return await fulfilmentFailed(outcome.message)
      break
    }
