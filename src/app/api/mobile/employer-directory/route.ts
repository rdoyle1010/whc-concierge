...
async function upsertSwipe(admin: any, row: any) {
  return admin.from('swipes').upsert(row, { onConflict: 'swiper_id,swiper_type,target_id,target_type,context_job_id', ignoreDuplicates: false })
}

async function activeApplication(admin:any,candidateId:string,jobId:string){
  const {data}=await admin.from('applications').select('id,status').eq('candidate_id',candidateId)
    .or(`job_id.eq.${jobId},role_id.eq.${jobId}`).neq('status','draft')
    .order('submitted_at',{ascending:false,nullsFirst:false}).limit(1).maybeSingle()
  return data||null
}

async function ensureMutualMatch(admin: any, opts: { candidate: any; employer: any; job: any; score: number; candidateUserId: string; employerUserId: string; candidateEmail?: string | null; employerEmail?: string | null }) {
  const { candidate, employer, job, score, candidateUserId, employerUserId } = opts
  const { data: existing } = await admin.from('matches').select('id').eq('candidate_id', candidate.id).eq('employer_id', employer.id).eq('job_listing_id', job.id).maybeSingle()
  if (existing) return existing.id
  const now = new Date().toISOString()
  const { data: created, error } = await admin.from('matches').insert({ candidate_id: candidate.id, employer_id: employer.id, job_listing_id: job.id, match_score: score, candidate_swiped_at: now, employer_swiped_at: now, matched_at: now, status: 'active', messaging_unlocked: true }).select('id').single()
  ...
  await Promise.allSettled([
    createNotification(candidateUserId, 'new_match', "It's a match!", `${employerName} wants to progress your application for ${job.job_title}.`, '/talent/applications'),
    createNotification(employerUserId, 'new_match', "It's a match!", `${candidateName} is interested in ${job.job_title}. Review the submitted application to decide the next step.`, '/employer/applications'),
...
export async function GET(req: NextRequest) {
  try {
    ...
    const now = new Date().toISOString()
    const [{ data: rows, error }, { data: blocks }, { data: jobs }, { data: swipes }, { data: shortlist }, {data:applications}] = await Promise.all([
      admin.from('candidate_profiles').select(CANDIDATE_FIELDS).eq('approval_status', 'approved').or('profile_visible.eq.true,profile_visible.is.null').order('is_featured', { ascending: false }).order('created_at', { ascending: false }).limit(100),
      admin.from('profile_blocks').select('candidate_id').eq('blocked_employer_id', employer.id),
      admin.from('job_listings').select('*').eq('employer_id', employer.id).eq('is_live', true).or(`expires_at.is.null,expires_at.gt.${now}`).limit(25),
      ...
      admin.from('applications').select('id,candidate_id,role_id,job_id,status').neq('status','draft').is('archived_at',null),
    ])
    ...
    const interestedKey = new Set((swipes || []).filter((row: any) => row.action === 'right' && row.context_job_id).map((row: any) => `${row.target_id}:${row.context_job_id}`))
    const shortlisted = new Set((shortlist || []).map((row: any) => row.candidate_id))
    const applicationMap=new Map((applications||[]).map((a:any)=>[`${a.candidate_id}:${a.role_id||a.job_id}`,a]))
    ...
    const { data: candidateSwipes } = userIds.length ? await admin.from('swipes').select('swiper_id,target_id,action,context_job_id').in('swiper_id', userIds).eq('swiper_type', 'candidate').eq('target_type', 'job') : { data: [] as any[] }
    const candidateYes = new Set((candidateSwipes || []).filter((row: any) => row.action === 'right').map((row: any) => `${row.swiper_id}:${row.target_id}`))
    ...
    const candidates = visible.map((candidate: any) => {
      ...
      for (const job of liveJobs) {
        ...
        if (!best || result.score > best.matchScore) best = { matchScore: result.score, matchLabel: result.label, matchExplanation: result.matchExplanation || '', bestJob: job.job_title, bestJobId: job.id }
      }
  ...
  } catch (error: any) { return NextResponse.json({ error: error?.message || 'Talent directory unavailable' }, { status: 500 }) }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'unsave') { const { error } = await admin.from('shortlisted_candidates').delete().eq('employer_id', employer.id).eq('candidate_id', candidateId); if (error) return NextResponse.json({ error: 'Could not remove this saved professional.' }, { status: 500 }); return NextResponse.json({ success: true, saved: false }) }
    if (action === 'left') { const { error } = await upsertSwipe(admin, { swiper_id: user.id, swiper_type: 'employer', target_id: candidateId, target_type: 'candidate', action: 'left', context_job_id: null }); if (error) return NextResponse.json({ error: 'Your pass could not be saved.' }, { status: 500 }); return NextResponse.json({ success: true, matched: false }) }
    ...
    const [{ data: candidate }, { data: job }, { data: block }] = await Promise.all([
      admin.from('candidate_profiles').select('*').eq('id', candidateId).maybeSingle(), admin.from('job_listings').select('*').eq('id', jobId).eq('employer_id', employer.id).maybeSingle(), admin.from('profile_blocks').select('candidate_id').eq('candidate_id', candidateId).eq('blocked_employer_id', employer.id).maybeSingle(),
    ])
    ...
    const { error } = await upsertSwipe(admin, { swiper_id: user.id, swiper_type: 'employer', target_id: candidate.id, target_type: 'candidate', action: 'right', context_job_id: job.id })
