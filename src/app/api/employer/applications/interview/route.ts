  ...
  try {
    ...
    if (action === 'complete') {
      ...
      if (interview.status !== 'confirmed' || !interview.selected_slot) {
        return NextResponse.json({ error: 'The candidate must confirm an interview time before it can be completed.' }, { status: 409 })
      }
      const scheduled = new Date(interview.selected_slot)
      if (Number.isNaN(scheduled.getTime()) || scheduled.getTime() > Date.now()) {
        return NextResponse.json({ error: 'This interview cannot be completed before its confirmed time.' }, { status: 409 })
      }
      const { data: completed, error } = await admin.from('application_interviews')
    ...
    const { data: existingRounds } = await admin.from('application_interviews')
    ...
    if (roundNumber > 1) {
      ...
      if (!previous || previous.status !== 'completed') {
        return NextResponse.json({ error: `Complete interview ${roundNumber - 1} before scheduling interview ${roundNumber}.` }, { status: 409 })
      }
    }
    ...
    const notification = `${propertyName} has invited you to the ${stageLabel.toLowerCase()} for ${job.job_title}. Choose one of the proposed times in My Applications.`
