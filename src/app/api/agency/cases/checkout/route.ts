...
export async function POST(req: NextRequest) {
  ...
  const fee = Math.ceil(extra * 0.10)
  const total = extra + fee
  const stripe = getStripe()
  const session = await stripe.checkout.sessions.create({
    payment_method_types: ['card'],
    line_items: [{ price_data: { currency: 'gbp', product_data: { name: 'WHC Concierge - Agency Shift Adjustment', description: `Approved shift adjustment £${extra} + 10% WHC fee £${fee}. The professional receives the approved £${extra}.` }, unit_amount: Math.round(total * 100) }, quantity: 1 }],
    mode: 'payment',
    ...
    metadata: { type: 'agency_case_adjustment', case_id: row.id, booking_id: row.booking_id, employer_id: employer.id, extra: String(extra), fee: String(fee) },
  })
