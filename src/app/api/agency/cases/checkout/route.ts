...
export async function POST(req: NextRequest) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{ price_data: { currency: 'gbp', product_data: { name: 'Talent House Collective - Agency Shift Adjustment', description: `Approved shift adjustment £${extra} + ${pctLabel}% Talent House fee £${fee}. The professional receives the approved £${extra}.` }, unit_amount: Math.round(total * 100) }, quantity: 1 }],
    mode: 'payment',
