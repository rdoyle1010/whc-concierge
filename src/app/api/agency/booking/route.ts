  ...
  try {
    ...
    const enriched = bookings.map(b => {
      ...
      return {
        ...
        candidate_name: candidate?.full_name || 'Candidate', candidate_user_id: candidate?.user_id || null, cascade_total: Array.isArray(b.cascade_queue) ? b.cascade_queue.length : null, cascade_position: Array.isArray(b.cascade_queue) ? (b.cascade_index ?? 0) + 1 : null, employer_agency_plus: Boolean((employer as any)?.agency_plus_active), viewer_role: emp && b.employer_id === emp.id ? 'employer' : 'candidate',
      }
