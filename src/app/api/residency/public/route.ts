...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = admin.from('residency_profiles')
      .select('id,title,bio,description,full_name,primary_specialism,secondary_specialisms,services_offered,qualifications,brand_experience,product_houses,current_location,will_travel_to,travel_availability,preferred_duration,duration,day_rate,weekly_rate,monthly_rate,negotiable,available_from,availability_start,years_experience,is_featured,approval_status,candidate_profile_id')
    ...
    const { data: candidates } = candidateIds.length
      ...
      : { data: [] as any[] }
    const active = new Set((candidates || []).filter((candidate: any) => candidate.residency_member === true && ['active', 'trialing'].includes(candidate.residency_subscription_status || 'active')).map((candidate: any) => candidate.id))

    const visible = rows.filter((row: any) => active.has(row.candidate_profile_id)).map(toPublicResidencyProfile)
