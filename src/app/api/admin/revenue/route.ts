...
export async function GET() {
  ...
  const agencyGross = (agencyRes.data || []).reduce((sum: number, row: any) => sum + Number(row.amount_paid || 0), 0)
  const agencyPlatformRevenue = (agencyRes.data || []).reduce((sum: number, row: any) => sum + Number(row.platform_fee || 0), 0)
  ...
  const recordedRevenuePence = academyRevenue + agencyPlatformRevenue
  ...
  return NextResponse.json({
    ...
    note: 'Recorded revenue includes database-backed Academy payments and WHC Agency platform fees. Subscription/job revenue should not be treated as cash received until a central Stripe transaction ledger is added.',
  })
}
