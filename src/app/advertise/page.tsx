    ...
    <main id="main-content" className="pt-[76px]">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-12 md:py-16">
        <div className="max-w-3xl mb-8">
          <p className="text-[10px] uppercase tracking-[.18em] font-semibold text-[#1c1c1c]">Choose your placement</p>
          <h2 className="text-[32px] md:text-[42px] font-semibold tracking-[-.04em] text-[#1c1c1c] mt-2">Where do you want your brand to live?</h2>
          <p className="text-[13px] leading-6 text-[#555555] mt-3">All placements are rolling monthly subscriptions. They renew each month until cancelled.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-10">
          {(Object.entries(AD_PLACEMENTS) as [AdPlacementKey, (typeof AD_PLACEMENTS)[AdPlacementKey]][]).map(([key, config]) => {
            ...
            return <button key={key} type="button" onClick={() => setPlacement(key)} className={`text-left rounded-[22px] p-6 transition-all ${active ? 'bg-[#1c1c1c] text-white border border-[#1c1c1c] shadow-lg' : 'bg-white border border-[#dddddd] text-[#1c1c1c] hover:border-[#555555]'}`}>
              <div className="flex items-start justify-between gap-4"><div><p className={`text-[10px] uppercase tracking-[.15em] font-semibold ${active ? 'text-white/70' : 'text-[#1c1c1c]'}`}>{config.label}</p><p className={`text-[31px] font-semibold mt-2 ${active ? 'text-white' : 'text-[#1c1c1c]'}`}>£{formatPounds(priceFor(key))}<span className={`text-[11px] font-normal ${active ? 'text-white/55' : 'text-[#6b6b6b]'}`}> / month</span></p></div>{active && <Check size={18} className="text-white" />}</div>
              <p className={`text-[12px] leading-6 mt-4 ${active ? 'text-white/66' : 'text-[#555555]'}`}>{config.description}</p>
              <div className={`mt-5 pt-4 border-t space-y-2 ${active ? 'border-white/12' : 'border-[#dddddd]'}`}>
                <p className={`text-[11px] ${active ? 'text-white/78' : 'text-[#555555]'}`}><strong>Audience:</strong> {copy.audience}</p>
                <p className={`text-[11px] ${active ? 'text-white/78' : 'text-[#555555]'}`}><strong>Best for:</strong> {copy.bestFor}</p>
              </div>
            </button>
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[.9fr_1.1fr] gap-8 items-start">
          <div className="space-y-5">
            <div className="rounded-[22px] bg-white border border-[#dddddd] p-7">
              <p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#1c1c1c]">Your selected placement</p>
              <h2 className="text-[27px] font-semibold tracking-[-.03em] text-[#1c1c1c] mt-2">{selected.label}</h2>
              <div className="space-y-4 mt-6 text-[12px] leading-6 text-[#555555]">
                <div><p className="font-semibold text-[#1c1c1c]">Where it appears</p><p>{selectedAudience.where}</p></div>
                <div><p className="font-semibold text-[#1c1c1c]">Who sees it</p><p>{selectedAudience.audience}</p></div>
                <div><p className="font-semibold text-[#1c1c1c]">How long it runs</p><p>{AD_BILLING_COPY.short}</p></div>
                <div><p className="font-semibold text-[#1c1c1c]">When it starts</p><p>{AD_BILLING_COPY.start}</p></div>
              </div>
            </div>

            <div className="rounded-[22px] bg-[#f1f1f1] border border-[#dddddd] p-7 text-[12px] leading-6 text-[#555555]">
              <p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#1c1c1c] mb-3">Approval journey</p>
          ...
          <form onSubmit={submit} className="bg-white border border-[#dddddd] rounded-[22px] p-7 md:p-8 shadow-sm">
            <div className="flex items-start justify-between gap-4 pb-6 mb-6 border-b border-[#dddddd]"><div><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#1c1c1c]">Book this placement</p><h2 className="text-[27px] font-semibold tracking-[-.03em] text-[#1c1c1c] mt-2">{selected.label}</h2></div><p className="text-right text-[24px] font-semibold text-[#1c1c1c]">£{formatPounds(selectedPrice)}<span className="block text-[10px] font-normal text-[#6b6b6b]">per month · recurring</span></p></div>
            <div className="space-y-4">
              <label className="block text-[12px] text-[#555555]">Brand name<input required value={form.brandName} onChange={event => setForm({ ...form, brandName: event.target.value })} className="input-field mt-1" /></label>
              <label className="block text-[12px] text-[#555555]">Contact email<input required type="email" value={form.contactEmail} onChange={event => setForm({ ...form, contactEmail: event.target.value })} className="input-field mt-1" /></label>
              <label className="block text-[12px] text-[#555555]">Advert wording<input required maxLength={220} value={form.tagline} onChange={event => setForm({ ...form, tagline: event.target.value })} placeholder="A short line shown beside your logo" className="input-field mt-1" /></label>
              <label className="block text-[12px] text-[#555555]">Website link<input required type="url" value={form.websiteUrl} onChange={event => setForm({ ...form, websiteUrl: event.target.value })} placeholder="https://yourbrand.com" className="input-field mt-1" /></label>
              <label className="block text-[12px] text-[#555555]">Logo image link<input required type="url" value={form.logoUrl} onChange={event => setForm({ ...form, logoUrl: event.target.value })} placeholder="https://yourbrand.com/logo.png" className="input-field mt-1" /><span className="block mt-1 text-[10px] text-[#6b6b6b]">Direct public HTTPS link to a PNG, JPG, WebP or SVG logo.</span></label>
            </div>
