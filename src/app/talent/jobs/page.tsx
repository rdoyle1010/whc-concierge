    ...
    <DashboardShell role="talent" userName={profile?.full_name}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-7">
        <div>
          ...
          <div className="mb-0 mt-3 inline-flex rounded-xl border border-[#e3e7eb] bg-white p-1 text-[12px] font-semibold"><span className="rounded-lg bg-[#0b2f4d] px-4 py-2 text-white">Browse list</span><a href="/roles/match" className="rounded-lg px-4 py-2 text-[#5a6a76] hover:text-[#0b2f4d]">Match deck</a></div>
            ...
            {paginatedSorted.map(job => {
              ...
              return (
                <article key={job.id} className="card p-0 overflow-hidden">
                  <div className="relative h-36 bg-[#f5f6f8] overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      ...
                      <button type="button" onClick={() => toggleSave(job.id)} className={`p-2 border rounded-md transition-colors ${saved.has(job.id) ? 'bg-[#f5f6f8] border-accent/30 text-accent' : 'border-border text-muted hover:text-accent hover:border-accent/30'}`} title={saved.has(job.id) ? 'Unsave role' : 'Save role for later'}>
