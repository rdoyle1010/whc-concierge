            ...
            return (
              <div key={saved.id} className="bg-white border border-border rounded-xl p-5 hover:shadow-sm transition-all">
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${job.tier === 'Platinum' ? 'bg-ink text-white' : job.tier === 'Gold' ? 'bg-[#ede8df] text-[#222321]' : 'bg-surface text-muted'}`}>{job.tier || 'Standard'}</span>
