...
export default function TalentProfilePage() {
  ...
  async function handleCvUpload(e:React.ChangeEvent<HTMLInputElement>){
    ...
    u('cv_url',url);setCvSuggestions(null);setAiCvConsent(false);setMessage('CV uploaded. You can now choose whether to analyse it with WHC AI.')
  }
  ...
  return <DashboardShell role="talent" userName={profile.full_name}><div className="max-w-3xl">
    ...
    <div className="mb-8"><div className="flex items-center gap-3"><div className="flex-1 h-1.5 bg-[#e9e4dd] rounded-full overflow-hidden"><div className="h-full bg-[#1c1b1a]" style={{width:`${completionPct}%`}}/></div><span className="text-[12px]">{completionPct}%</span></div>{missingItems.length>0&&<p className="mt-2 text-[12px] text-[#1c1b1a]">To reach 100%, add: {missingItems.join(', ')}. <a href="/talent/onboarding" className="font-semibold underline">Use the Skills Wizard →</a></p>}{missingItems.length===0&&<p className="mt-2 text-[12px] text-emerald-700">Your profile is complete - it will rank at full strength in matching.</p>}</div>
    {message&&<div role="status" className="mb-6 rounded-lg bg-[#f3f0eb] px-4 py-3 text-[12px] text-[#57534e]">{message}</div>}
...
function Mini({label,value}:{label:string;value:string}){return <div className="rounded-lg bg-[#f3f0eb] p-3"><p className="text-[10px] text-muted">{label}</p><p className="font-semibold text-ink mt-1">{value}</p></div>}