...
export default function TalentProfilePage() {
  ...
  return <DashboardShell role="talent" userName={profile.full_name}><div className="max-w-3xl">
    ...
    <div className="mb-8"><div className="flex items-center gap-3"><div className="flex-1 h-1.5 bg-[#e3dcd1] rounded-full overflow-hidden"><div className="h-full bg-[#222321]" style={{width:`${completionPct}%`}}/></div><span className="text-[12px]">{completionPct}%</span></div>{missingItems.length>0&&<p className="mt-2 text-[12px] text-[#222321]">To reach 100%, add: {missingItems.join(', ')}. <a href="/talent/onboarding" className="font-semibold underline">Use the Skills Wizard →</a></p>}{missingItems.length===0&&<p className="mt-2 text-[12px] text-emerald-700">Your profile is complete - it will rank at full strength in matching.</p>}
    ...
    {message&&<div role="status" className="mb-6 rounded-lg bg-[#ede8df] px-4 py-3 text-[12px] text-[#57544c]">{message}</div>}
    <div className="space-y-6">
      ...
      <section className="dashboard-card space-y-4"><p className="eyebrow">Professional details</p><Field label="Current role level"><select className="input-field" value={profile.role_level||''} onChange={e=>u('role_level',e.target.value)}><option value="">Select</option>{ROLE_LEVELS.map(r=><option key={r}>{r}</option>)}</select></Field><Field label="Headline"><input className="input-field" value={profile.headline||''} onChange={e=>u('headline',e.target.value)} placeholder="e.g. Spa Manager | Luxury Hospitality | Commercial & People Leadership"/><AiWrite className="mt-2" field="talent_headline" value={profile.headline||''} onAccept={text=>u('headline',text)}/></Field><Field label="Where you work now"><input className="input-field" value={profile.current_employer||''} onChange={e=>u('current_employer',e.target.value)} placeholder="e.g. The Dorchester Spa, London"/></Field><label className="flex items-start gap-2.5 border border-[#dcd4c8] bg-[#ede8df] p-3 text-[12px] leading-6 text-[#57544c]"><input type="checkbox" className="mt-1" checked={!!profile.current_employer_visible} onChange={e=>u('current_employer_visible',e.target.checked)}/><span><span className="font-medium text-[#222321]">Let properties see where I work now.</span> Off by default. Employers find this useful, and you may not want your current employer knowing you are looking. You can change it whenever you like, and Private Career Mode hides it regardless.</span></label><Field label="About you"><textarea rows={6} className="input-field" value={profile.bio||''} onChange={e=>u('bio',e.target.value)} placeholder="The first thing a property reads. If you would rather not write it, press the button underneath and change what it gets wrong."/><AiWrite className="mt-2" field="talent_bio" value={profile.bio||''} onAccept={text=>u('bio',text)}/></Field><div className="grid sm:grid-cols-3 gap-4"><Field label="Experience years"><input type="number" className="input-field" value={profile.experience_years||''} onChange={e=>u('experience_years',e.target.value)}/></Field>{profile.agency_available?<><Field label="Agency day rate min (£)"><input type="number" className="input-field" value={profile.day_rate_min||''} onChange={e=>u('day_rate_min',e.target.value)}/></Field><Field label="Agency day rate max (£)"><input type="number" className="input-field" value={profile.day_rate_max||''} onChange={e=>u('day_rate_max',e.target.value)}/></Field></>:null}</div><Field label="Availability"><select className="input-field" value={profile.availability_status||''} onChange={e=>u('availability_status',e.target.value)}>{AVAILABILITY_STATUSES.map(a=><option key={a.value} value={a.value}>{a.label}</option>)}</select></Field></section>
...
function Mini({label,value}:{label:string;value:string}){return <div className="rounded-lg bg-[#ede8df] p-3"><p className="text-[10px] text-muted">{label}</p><p className="font-semibold text-ink mt-1">{value}</p></div>}