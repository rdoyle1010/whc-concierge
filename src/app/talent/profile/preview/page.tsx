        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          ...
          <div className="space-y-5">
            <section className="rounded-xl border border-border bg-white p-5">
              ...
              {badges.length ? <div className="space-y-2">{badges.map(badge => <div key={badge.course_slug} className="rounded-lg border border-accent/20 bg-[#FDF6EC] p-3"><p className="text-[12px] font-medium text-ink">{courseTitle(badge.course_slug)}</p><p className="mt-1 text-[10px] text-muted">Verified completion Â· {new Date(badge.completed_at).toLocaleDateString('en-GB')}</p><Link href={`/talent/academy/certificate/${badge.course_slug}`} className="mt-1 inline-block text-[10px] font-medium text-accent hover:underline">View certificate</Link></div>)}</div> : <p className="text-[11px] text-muted">Completed Academy courses will always appear here.</p>}
            </section>
            {(profile.day_rate_min || profile.day_rate_max) && <section className="rounded-xl border border-border bg-white p-5"><p className="eyebrow mb-2">Professional rate</p><p className="text-[18px] font-semibold text-accent">Â£{profile.day_rate_min}{profile.day_rate_max ? `â€“Â£${profile.day_rate_max}` : ''}<span className="text-[11px] font-normal text-muted"> /day</span></p></section>}
          </div>
        </div>
      </div>
    </DashboardShell>
