...
export default function EmployerSocialPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: prof } = await supabase.from('employer_profiles_private').select('*').eq('user_id', user.id).maybeSingle()
      setProfile(prof)
      if (prof) {
  ...
  return <DashboardShell role="employer" userName={profile?.company_name}>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 mb-9">
      {platforms.map(({ name, icon: Icon, status, text }) => <div key={name} className="dashboard-card">
        <div className="flex items-start justify-between gap-3"><div className="h-10 w-10 rounded-lg bg-[#ede8df] text-[#222321] flex items-center justify-center"><Icon size={18}/></div><span className="text-[10px] uppercase tracking-[.12em] text-[#57544c]">{status}</span></div>
        <h2 className="text-[20px] mt-4">{name}</h2><p className="text-[12px] leading-5 text-secondary mt-2">{text}</p>
      </div>)}
    </div>

    <div className="dashboard-card mb-8 bg-[#222321] text-white border-[#222321]">
    ...
    <section>
      ...
      {loading ? <div className="skeleton h-44 rounded-lg"/> : liveJobs.length === 0 ? <div className="dashboard-card py-12 text-center"><Megaphone size={24} className="mx-auto text-[#57544c] mb-3"/><h3 className="text-[20px]">No live jobs to promote</h3><p className="text-[12px] text-secondary mt-2">Once a paid job is live it will appear here with its social sharing controls.</p></div> : <div className="space-y-4">{liveJobs.map(job => {
