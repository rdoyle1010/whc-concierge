...
const FUNNEL_COLOURS = ['#222321', '#3a4239', '#474747', '#222321', '#3a4239', '#16a34a']
    ...
    <DashboardShell role="employer" userName={profile?.company_name}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="dashboard-card">
          ...
          <p className="text-[28px] font-semibold" style={{ color: '#57544c' }}>{stats.avgMatch > 0 ? `${stats.avgMatch}%` : '-'}</p>
            ...
            <div className="space-y-2.5">
              {topSkills.map((skill, i) => (
                <div key={skill} className="flex items-center gap-3">
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-semibold shrink-0" style={{ backgroundColor: '#ede8df', color: '#57544c' }}>{i + 1}</span>
                ...
                {sortedRows.map(row => (
                  <tr key={row.id} className="hover:bg-surface/50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${row.tier === 'Platinum' ? 'bg-ink text-white' : row.tier === 'Gold' ? 'bg-[#ede8df] text-accent' : row.tier === 'Silver' ? 'bg-neutral-100 text-neutral-600' : 'bg-surface text-muted'}`}>{row.tier}</span>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3">
                      {row.avgScore > 0 ? (
                        <span className="text-[13px] font-medium" style={{ color: row.avgScore >= 80 ? '#16A34A' : row.avgScore >= 60 ? '#57544c' : '#57544c' }}>{row.avgScore}%</span>
