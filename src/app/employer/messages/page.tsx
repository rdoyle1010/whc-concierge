            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="flex h-full flex-col items-center justify-center px-8 text-center">
                  ...
                  <p className="mt-2 text-[12px] leading-5 text-[#8a949b]">When interest is mutual, the conversation will appear here automatically.</p>
                </div>
                ...
                return (
                  <button type="button" key={convo.partnerId} onClick={() => setActiveConvo(convo.partnerId)} className={`group w-full border-b border-[#ececec] px-4 py-4 text-left transition ${selected ? 'bg-white' : 'hover:bg-white/70'}`}>
                    <div className="flex items-start gap-3">
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold ${selected ? 'border-[#555555] bg-[#f5f5f5] text-[#10283b]' : 'border-[#e0e0e0] bg-white text-[#59636c]'}`}>{initials}</div>
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1.5 truncate text-[11px] leading-4 text-[#8a949b]">{convo.lastMessage?.content || (convo.lastMessage?.attachment_name ? `Attachment: ${convo.lastMessage.attachment_name}` : 'Conversation ready')}</p>
                      </div>
                    </div>
                  </button>
                )
          ...
          <section className={`${activeConvo ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-[#fafafa]`}>
            {!activeConvo ? (
              <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                ...
                <p className="mt-2 max-w-sm text-[12px] leading-5 text-[#8a949b]">Matched talent and protected Residency discussions will stay organised here.</p>
              </div>
            ) : (
              <>
                <header className="flex min-h-[76px] items-center gap-3 border-b border-[#e5e5e5] bg-white px-4 md:px-6">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[10px] text-[#8a949b]">{residencyPrivate ? 'Protected Residency conversation' : 'Private matched conversation'}</p>
                  </div>
                </header>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-6 md:px-7">
                  {messages.length === 0 ? (
                    <div className="flex h-full items-center justify-center text-center"><p className="max-w-xs text-[12px] leading-5 text-[#8a949b]">You are connected. Send the first message when you are ready.</p></div>
                  ) : (
                        ...
                        return (
                          <div key={msg.id || `${msg.created_at}-${i}`} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[82%] md:max-w-[68%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                              ...
                              <p className="mt-1 px-1 text-[9px] tracking-wide text-[#8a949b]">{formatTimestamp(msg.created_at)}</p>
                            </div>
                          </div>
                        )
                      })}
                ...
                <div className="border-t border-[#e5e5e5] bg-white p-3 md:p-4">
                  ...
                  <div className="flex items-end gap-2">
                    <div className="flex min-w-0 flex-1 items-center rounded-2xl border border-[#e0e0e0] bg-[#fafafa] px-3 py-1.5 focus-within:border-[#555555] focus-within:bg-white">
