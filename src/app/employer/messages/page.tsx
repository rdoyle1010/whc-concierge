            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="flex h-full flex-col items-center justify-center px-8 text-center">
                  ...
                  <p className="mt-2 text-[12px] leading-5 text-[#6b6b6b]">When interest is mutual, the conversation will appear here automatically.</p>
                </div>
                ...
                return (
                  <button type="button" key={convo.partnerId} onClick={() => setActiveConvo(convo.partnerId)} className={`group w-full border-b border-[#dddddd] px-4 py-4 text-left transition ${selected ? 'bg-white' : 'hover:bg-white/70'}`}>
                    <div className="flex items-start gap-3">
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold ${selected ? 'border-[#555555] bg-[#f1f1f1] text-[#1c1c1c]' : 'border-[#dddddd] bg-white text-[#555555]'}`}>{initials}</div>
          ...
          <section className={`${activeConvo ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-[#f1f1f1]`}>
            {!activeConvo ? (
              <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                ...
                <p className="mt-2 max-w-sm text-[12px] leading-5 text-[#6b6b6b]">Matched talent and protected Residency discussions will stay organised here.</p>
              </div>
            ) : (
              <>
                ...
                <div className="border-t border-[#dddddd] bg-white p-3 md:p-4">
                  ...
                  <div className="flex items-end gap-2">
                    <div className="flex min-w-0 flex-1 items-center rounded-2xl border border-[#dddddd] bg-[#f1f1f1] px-3 py-1.5 focus-within:border-[#555555] focus-within:bg-white">
                    ...
                    <button type="button" onClick={sendMessage} aria-label="Send message" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-accent text-white transition hover:bg-[#333333] disabled:opacity-50" disabled={attachmentUploading || (!newMsg.trim() && !attachmentFile)}><Send size={17} /></button>
                  </div>
                </div>
              </>
            )}
