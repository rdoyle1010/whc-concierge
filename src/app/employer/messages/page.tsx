...
export default function EmployerMessagesPage() {
  ...
  const sendMessage = async () => {
    ...
    await supabase.from('messages').insert({ sender_id: userId, receiver_id: activeConvo, content: newMsg.trim(), read: false })
    setMessages([...messages, { sender_id: userId, receiver_id: activeConvo, content: newMsg.trim(), created_at: new Date().toISOString() }])
    setNewMsg('')
    setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: 'smooth' }), 100)
  }
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.sender_id === userId ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs px-4 py-2.5 rounded-2xl text-sm ${msg.sender_id === userId ? 'bg-gold text-white rounded-br-sm' : 'bg-gray-100 text-ink rounded-bl-sm'}`}>{msg.content}</div>
                    </div>
                  ))}
                  <div ref={bottomRef} />
                </div>
                <div className="p-4 border-t border-gray-100">
                  <div className="flex space-x-3">
                    <input type="text" value={newMsg} onChange={(e) => setNewMsg(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} className="input-field flex-1" placeholder="Type a message..." />
