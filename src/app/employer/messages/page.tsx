        ...
        <div className="flex h-full">
          <aside className={`${activeConvo ? 'hidden md:flex' : 'flex'} w-full md:w-[330px] lg:w-[360px] flex-col border-r border-[#e3e7eb] bg-[#f5f6f8]`}>
            <div className="border-b border-[#e3e7eb] px-5 py-5">
              ...
              <p className="mt-1 text-[12px] text-[#6b7580]">{conversations.length} active thread{conversations.length === 1 ? '' : 's'}</p>
            </div>
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="flex h-full flex-col items-center justify-center px-8 text-center">
                  ...
                  <p className="text-[15px] font-medium text-[#10283b]">No conversations yet</p>
                  <p className="mt-2 text-[12px] leading-5 text-[#6b7580]">When interest is mutual, the conversation will appear here automatically.</p>
                </div>
                ...
                return (
                  <button type="button" key={convo.partnerId} onClick={() => setActiveConvo(convo.partnerId)} className={`group w-full border-b border-[#e3e7eb] px-4 py-4 text-left transition ${selected ? 'bg-white' : 'hover:bg-white/70'}`}>
                    <div className="flex items-start gap-3">
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold ${selected ? 'border-[#5a6a76] bg-[#f5f6f8] text-[#10283b]' : 'border-[#e3e7eb] bg-white text-[#5a6a76]'}`}>{initials}</div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <p className="min-w-0 flex-1 truncate text-[13px] font-semibold text-[#10283b]">{convo.partnerName || 'Unknown User'}</p>
                        ...
                        {convo.residencyPrivate && <p className="mt-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#10283b]">Protected Residency</p>}
                        <p className="mt-1.5 truncate text-[11px] leading-4 text-[#6b7580]">{convo.lastMessage?.content || (convo.lastMessage?.attachment_name ? `Attachment: ${convo.lastMessage.attachment_name}` : 'Conversation ready')}</p>
                      </div>
                    </div>
                  </button>
                )
          ...
          <section className={`${activeConvo ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-[#f5f6f8]`}>
            {!activeConvo ? (
              <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                ...
                <p className="text-[20px] font-medium tracking-[-0.02em] text-[#10283b]">Select a conversation</p>
                <p className="mt-2 max-w-sm text-[12px] leading-5 text-[#6b7580]">Matched talent and protected Residency discussions will stay organised here.</p>
              </div>
            ) : (
              <>
                <header className="flex min-h-[76px] items-center gap-3 border-b border-[#e3e7eb] bg-white px-4 md:px-6">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[10px] text-[#6b7580]">{residencyPrivate ? 'Protected Residency conversation' : 'Private matched conversation'}</p>
                  </div>
                </header>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-6 md:px-7">
                  {messages.length === 0 ? (
                    <div className="flex h-full items-center justify-center text-center"><p className="max-w-xs text-[12px] leading-5 text-[#6b7580]">You are connected. Send the first message when you are ready.</p></div>
                  ) : (
                        ...
                        return (
                          <div key={msg.id || `${msg.created_at}-${i}`} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[82%] md:max-w-[68%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                              ...
                              <p className="mt-1 px-1 text-[9px] tracking-wide text-[#6b7580]">{formatTimestamp(msg.created_at)}</p>
                            </div>
                          </div>
                        )
                      })}
                ...
                <div className="border-t border-[#e3e7eb] bg-white p-3 md:p-4">
                  {!residencyPrivate && attachmentFile && (
                    <div className="mb-3 flex items-center justify-between rounded-xl border border-[#e3e7eb] bg-[#f5f6f8] px-3 py-2.5">
                      ...
                      <button type="button" onClick={removeAttachment} className="ml-3 text-[#6b7580] hover:text-[#10283b]">×</button>
                    </div>
                  )}
                  <div className="flex items-end gap-2">
                    <div className="flex min-w-0 flex-1 items-center rounded-2xl border border-[#e3e7eb] bg-[#f5f6f8] px-3 py-1.5 focus-within:border-[#5a6a76] focus-within:bg-white">
                      <input type="text" aria-label="Write a message" value={newMsg} onChange={(e) => setNewMsg(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !attachmentUploading && sendMessage()} className="min-w-0 flex-1 bg-transparent px-1 py-2 text-[13px] text-[#10283b] outline-none placeholder:text-[#6b7580]" placeholder={residencyPrivate ? 'Discuss the Residency without sharing contact details…' : 'Write a message…'} disabled={attachmentUploading} />
                      {!residencyPrivate && <><input ref={fileInputRef} type="file" aria-label="Attach a file" onChange={handleFileSelected} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" style={{ display: 'none' }} /><button type="button" onClick={handleAttachmentClick} aria-label="Attach a file" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[#6b7580] transition hover:bg-[#f5f6f8] hover:text-[#0b2f4d]" disabled={attachmentUploading} title="Attach file"><Paperclip size={17} /></button></>}
                    </div>
