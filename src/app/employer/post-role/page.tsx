...
function scopesFromSaved(scope: string): ScopeValue[] {
  if (scope === 'same_level') return ['same_level']
  if (scope === 'emerging') return ['step_up','emerging']
  if (scope === 'open_transferable') return ['step_up','emerging','open_transferable']
  return ['step_up']
}
...
export default function PostRolePage() {
  ...
  const toggleScope=(value:ScopeValue)=>{
    if(value==='same_level'){setSelectedScopes(['same_level']);return}
    setSelectedScopes(current=>{
      const withoutSame=current.filter(v=>v!=='same_level')
      if(withoutSame.includes(value)){
        const next=withoutSame.filter(v=>v!==value)
        return next.length?next:['same_level']
      }
      return [...withoutSame,value]
    })
  }
  ...
  useEffect(()=>{(async()=>{
    ...
    const savedScope=String(j.candidate_scope||'step_up')
    setSelectedScopes(scopesFromSaved(savedScope))
  ...
  const selectedScopeLabels=SCOPE_OPTIONS.filter(o=>selectedScopes.includes(o.value)).map(o=>o.title)
  return <DashboardShell role="employer" userName={profile.company_name}>
    ...
    {phase==='form'?<div className="max-w-4xl mt-8 space-y-7">
      ...
      <section className="dashboard-card"><div className="flex items-start gap-3"><Users size={20} className="text-[#6f7f88] mt-1"/><div><p className="eyebrow">Who should WHC consider?</p><h2 className="text-[25px] mt-1">Set the career-progression window.</h2><p className="text-[12px] text-secondary mt-2 max-w-2xl">Tick every group you are genuinely open to. Same level only is exclusive; the other options can be combined. WHC will use the widest selected progression window when calculating matches.</p></div></div><div className="grid md:grid-cols-2 gap-3 mt-6">{SCOPE_OPTIONS.map(o=>{const selected=selectedScopes.includes(o.value);return <button type="button" key={o.value} onClick={()=>toggleScope(o.value)} aria-pressed={selected} className={`text-left rounded-xl border p-4 transition-all ${selected?'border-[#0b2f4d] bg-[#f1f4f6]':'border-[#dfe5e8] bg-white hover:border-[#aebbc2]'}`}><div className="flex items-center justify-between gap-3"><p className="text-[14px] font-semibold text-[#10283b]">{o.title}</p><span className={`h-5 w-5 rounded border flex items-center justify-center ${selected?'bg-[#0b2f4d] border-[#0b2f4d]':'bg-white border-[#aebbc2]'}`}>{selected&&<Check size={13} className="text-white"/>}</span></div><p className="text-[11px] leading-5 text-secondary mt-2">{o.text}</p></button>})}</div>{selectedScopes.length>1&&<p className="text-[11px] text-secondary mt-4">Selected: {selectedScopeLabels.join(' + ')}</p>}</section>
