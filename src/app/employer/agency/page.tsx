...
export default function EmployerAgencyPage() {
  ...
  async function submitUrgent() {
    ...
    try {
      ...
      setUrgentSuccess(`The shift has been offered to ${j.first_name} (nearest available match). ${j.queue_size > 1 ? `If they can't take it, it moves automatically through ${j.queue_size - 1} more therapist${j.queue_size > 2 ? 's' : ''}, 30 minutes each.` : 'They are the only match right now.'} You'll be notified the moment someone accepts.`)
