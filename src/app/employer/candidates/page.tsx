        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(c => (
              <div key={c.id} className={`dashboard-card relative ${c.is_featured ? 'border-accent ring-1 ring-accent/20' : ''}`}>
                ...
                {typeof c.matchScore === 'number' ? <div className="mb-3"><div className="flex items-center gap-1.5"><span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ backgroundColor: c.matchBg, color: c.matchColour }}>{c.matchScore}% · {c.matchLabel}</span><span className="text-[10px] text-gray-400 truncate">for {c.bestJob}</span></div>{c.matchExplanation && <p className="mt-2 text-[11px] leading-5 text-gray-500">{c.matchExplanation}</p>}</div> : <p className="mb-3 text-[11px] text-gray-400">Post a live role to calculate a role-specific match.</p>}
                ...
                <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100"><button type="button" onClick={() => handlePass(c.id)} className="py-2 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-400 flex items-center justify-center gap-1 text-xs"><X size={13} />Pass</button><button type="button" onClick={() => toggleShortlist(c.id, c.bestJobId)} className={`py-2 rounded-lg flex items-center justify-center gap-1 text-xs ${shortlistedIds.has(c.id) ? 'bg-[#FDF6EC] text-accent' : 'bg-gray-50 text-gray-500'}`}><Star size={13} fill={shortlistedIds.has(c.id) ? 'currentColor' : 'none'} />Save</button><button type="button" disabled={!c.bestJobId || isInterested(c)} onClick={() => handleInterest(c)} className="py-2 rounded-lg bg-[#0b2f4d] text-white flex items-center justify-center gap-1 text-xs disabled:opacity-50"><Heart size={13} fill={isInterested(c) ? 'currentColor' : 'none'} />Interested</button></div>
              </div>
            ))}
          ...
          {hasMore && (
            <div className="flex justify-center mt-7">
              <button type="button" onClick={loadMore} disabled={loadingMore} className="btn-secondary min-w-40 disabled:opacity-50">
                {loadingMore ? 'Loading more...' : 'Load more talent'}
              </button>
            </div>
          )}
        </>
