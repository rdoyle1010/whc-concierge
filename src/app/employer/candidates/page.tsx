...
export default function EmployerCandidatesPage() {
  ...
  const [approachedIds, setApproachedIds] = useState<Set<string>>(new Set())
  const viewingDialog = useDialog(() => setViewing(null), 'candidate-profile-heading', { enabled: Boolean(viewing) })
  ...
  // Private Career Mode: the primary action for an anonymised profile is a
  // confidential introduction the professional approves before their identity
  // is revealed. The 30-day duplicate refusal is surfaced plainly.
  async function requestIntroduction(candidate: any) {
    if (busyKey) return
    setBusyKey(`approach:${candidate.id}`)
    setActionMessage('')
    try {
      const res = await fetch('/api/private-approach', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ candidateId: candidate.id }) })
      const body = await res.json().catch(() => ({}))
      if (!res.ok) {
        if (res.status === 409) setApproachedIds(prev => new Set(prev).add(candidate.id))
        setActionMessage(body.error || 'Could not request the introduction - please try again.')
        return
      }
      setApproachedIds(prev => new Set(prev).add(candidate.id))
      setActionMessage('Confidential introduction requested. The professional decides whether to reveal their full profile - you will be notified either way.')
    } catch {
      setActionMessage('Could not request the introduction - please try again.')
    } finally {
      setBusyKey('')
    }
  }
  ...
  return (
    <DashboardShell role="employer" userName={profile?.company_name || profile?.property_name}>
      ...
      <div className="dashboard-card mb-6">
        ...
        <div className="mt-4 flex items-center gap-3"><button type="button" onClick={() => setSavedOnly(v => !v)} className={savedOnly ? 'btn-primary' : 'btn-secondary'}><Star size={14} className="inline mr-1" fill={savedOnly ? 'currentColor' : 'none'}/>{savedOnly ? `Saved only (${shortlistedIds.size})` : `Saved Talent (${shortlistedIds.size})`}</button>{savedOnly && <button type="button" onClick={() => setSavedOnly(false)} className="text-xs text-secondary underline">Show everyone</button>}</div>
        ...
        {filtered.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.map(c => <div key={c.id} className={`dashboard-card relative ${c.is_featured ? 'border-accent ring-1 ring-accent/20' : ''}`}>
          ...
          {(c.whc_verified || c.has_insurance || c.right_to_work_status === 'verified') && (
            <div className="mb-3 flex flex-wrap gap-1.5">
              {c.whc_verified && <span className="inline-flex items-center gap-1 rounded-full bg-[#222321] px-2.5 py-1 text-[10px] font-semibold text-white"><BadgeCheck size={11} />Talent House Verified</span>}
              {c.has_insurance && <span className="inline-flex items-center gap-1 rounded-full border border-[#dcd4c8] px-2.5 py-1 text-[10px] font-medium text-[#222321]"><ShieldCheck size={11} />Insured</span>}
              {c.right_to_work_status === 'verified' && <span className="inline-flex items-center gap-1 rounded-full border border-[#dcd4c8] px-2.5 py-1 text-[10px] font-medium text-[#222321]"><Check size={11} />Right to work</span>}
            </div>
          )}
          ...
          {c.private_mode ? <>
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#dcd4c8]">
              <button type="button" onClick={() => handlePass(c.id)} className="py-2 bg-[#ede8df] hover:bg-gray-100 text-muted flex items-center justify-center gap-1 text-xs"><X size={13} />Pass</button>
              <button type="button" disabled={approachedIds.has(c.id) || busyKey === `approach:${c.id}`} onClick={() => requestIntroduction(c)} className="col-span-2 py-2 bg-[#222321] text-white flex items-center justify-center gap-1 text-xs disabled:opacity-50"><Lock size={13} />{approachedIds.has(c.id) ? 'Introduction requested' : 'Request confidential introduction'}</button>
            </div>
            <p className="text-[10px] text-center text-muted mt-2">{approachedIds.has(c.id) ? 'The professional decides whether to reveal their profile.' : 'This professional keeps their identity private until they approve an introduction.'}</p>
          </> : <>
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#dcd4c8]">
              <button type="button" onClick={() => handlePass(c.id)} className="py-2 rounded-lg bg-[#ede8df] hover:bg-gray-100 text-muted flex items-center justify-center gap-1 text-xs"><X size={13} />Pass</button>
              <button type="button" disabled={busyKey === `save:${c.id}`} onClick={() => toggleShortlist(c.id, c.bestJobId)} className={`py-2 rounded-lg flex items-center justify-center gap-1 text-xs ${shortlistedIds.has(c.id) ? 'bg-[#ede8df] text-accent' : 'bg-[#ede8df] text-secondary'}`}><Star size={13} fill={shortlistedIds.has(c.id) ? 'currentColor' : 'none'} />{shortlistedIds.has(c.id) ? 'Saved' : 'Save'}</button>
              <button type="button" disabled={liveRoleCount === 0 || !c.bestJobId || allRolesApproached(c) || busyKey === `interest:${c.id}`} onClick={() => handleInterest(c)} title={liveRoleCount === 0 ? 'Post a live role to express role-specific interest' : undefined} className="py-2 bg-[#222321] text-white flex items-center justify-center gap-1.5 text-xs disabled:opacity-50">{allRolesApproached(c) ? <><CheckCircle2 size={13} />Interest registered</> : isInterested(c) ? 'Register for another role' : 'Register interest'}</button>
            </div>
            {isInterested(c) && <p className="text-[10px] text-center text-emerald-700 mt-2">Emailed, and shown in their notifications.</p>}
          </>}
        </div>)}</div> : candidates.length === 0 ? <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-[#ede8df] mx-auto flex items-center justify-center mb-4"><Users size={24} className="text-muted" /></div><h2 className="text-xl font-serif text-ink">New professionals are joining the platform.</h2><p className="text-sm text-secondary mt-2 max-w-xl mx-auto">There are no discoverable professionals in your current search area yet.</p><div className="flex flex-wrap justify-center gap-3 mt-6"><a href="/employer/post-role" className="btn-primary">Post a role</a><button type="button" onClick={resetFilters} className="btn-secondary">Search all locations</button></div></div> : <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-[#ede8df] mx-auto flex items-center justify-center mb-4"><Search size={23} className="text-muted" /></div><h2 className="text-xl font-serif text-ink">No talent matches these filters.</h2><p className="text-sm text-secondary mt-2">Try widening your search area or clearing the filters.</p><button type="button" onClick={resetFilters} className="btn-secondary mt-6">Reset filters & expand search</button></div>}
      ...
      {viewing && <div className="fixed inset-0 bg-black/50 z-50 flex justify-end" onClick={() => setViewing(null)}><TrackView key={viewing.id} kind="candidate" id={String(viewing.id)} /><div {...viewingDialog.panelProps} className="bg-white w-full max-w-xl h-full overflow-y-auto"><div className="p-6 border-b border-[#dcd4c8] flex items-start justify-between sticky top-0 bg-white z-10"><div className="flex items-start gap-4 min-w-0 pr-4">{viewing.private_mode ? <div className="w-16 h-16 bg-[#ede8df] border border-[#dcd4c8] flex items-center justify-center flex-shrink-0"><span className="font-serif text-xl text-[#222321]">{viewing.full_name?.[0] || 'P'}</span></div> : <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0">{viewing.profile_image_url ? <img loading="lazy" decoding="async" src={viewing.profile_image_url} alt="" className="w-full h-full object-cover" /> : <span className="font-bold text-xl text-accent">{viewing.full_name?.[0]}</span>}</div>}<div><h3 id="candidate-profile-heading" className="text-xl font-semibold text-ink">{viewing.full_name}</h3>{viewing.private_mode && <p className="text-[10px] uppercase tracking-[1.5px] text-muted mt-0.5">Private profile</p>}{viewing.headline && <p className="text-[13px] text-secondary mt-1">{viewing.headline}</p>}</div></div><button type="button" onClick={() => setViewing(null)} className="text-secondary hover:text-ink" aria-label="Close candidate profile"><X size={20} /></button></div><div className="p-6 space-y-6 text-sm">{(() => { const value = careerValue(viewing, 0); return value.length > 0 ? <div className="border border-border bg-[#ede8df] p-4"><p className="text-[10px] uppercase tracking-wider text-accent mb-1">Career value</p><div>{value.map(entry => <div key={entry.dimension} className="flex items-baseline justify-between gap-4 border-t border-border py-2 first:border-t-0"><span className="text-[12px] text-ink">{entry.dimension}</span><span className="font-serif text-[13px] font-semibold text-accent">{entry.rating}</span></div>)}</div><p className="mt-2 text-[10px] text-secondary">Derived from the verified data on this profile, not self-assessed.</p></div> : null })()}{viewing.bestJob &&<div className="rounded-xl border border-accent/20 bg-[#ede8df] p-4"><p className="text-[10px] uppercase tracking-wider text-accent mb-1">Best live-role match</p><p className="font-semibold text-ink">{viewing.bestJob} · {viewing.matchScore}%</p>{viewing.matchExplanation && <p className="text-[12px] text-gray-600 mt-1">{viewing.matchExplanation}</p>}</div>}<div className="grid grid-cols-2 gap-4 rounded-xl bg-[#ede8df] p-4">{viewing.role_level && <div><p className="text-[10px] text-muted uppercase mb-1">Role level</p><p className="text-ink font-medium">{viewing.role_level}</p></div>}{viewing.location && <div><p className="text-[10px] text-muted uppercase mb-1">Location</p><p className="text-ink font-medium">{viewing.location}</p></div>}{viewing.experience_years && <div><p className="text-[10px] text-muted uppercase mb-1">Experience</p><p className="text-ink font-medium">{viewing.experience_years} years</p></div>}<div><p className="text-[10px] text-muted uppercase mb-1">Contact</p><p className="text-ink font-medium">Unlocks after mutual match</p></div></div>{viewing.bio && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">About</p><p className="text-ink leading-relaxed whitespace-pre-wrap text-[13px]">{viewing.bio}</p></div>}{viewing.services_offered?.length > 0 && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Treatments & services</p><div className="flex flex-wrap gap-1.5">{viewing.services_offered.map((x: string) => <span key={x} className="text-[11px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">{x}</span>)}</div></div>}{viewing.qualifications?.length > 0 && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Qualifications</p><div className="flex flex-wrap gap-1.5">{viewing.qualifications.map((x: string) => <span key={x} className="text-[11px] bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full">{x}</span>)}</div></div>}{viewing.product_houses?.length > 0 && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Product house experience</p><div className="flex flex-wrap gap-1.5">{viewing.product_houses.map((x: string) => <span key={x} className="text-[11px] bg-[#ede8df] text-[#222321] px-2 py-0.5 rounded-full">{x}</span>)}</div></div>}{viewing.systems_experience?.length > 0 && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Systems</p><div className="flex flex-wrap gap-1.5">{viewing.systems_experience.map((x: string) => <span key={x} className="text-[11px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">{x}</span>)}</div></div>}{viewing.business_skills?.length > 0 && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Business skills</p><div className="flex flex-wrap gap-1.5">{viewing.business_skills.map((x: string) => <span key={x} className="text-[11px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">{x}</span>)}</div></div>}{viewing.career_evidence && <div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">Career evidence</p><p className="text-ink leading-relaxed whitespace-pre-wrap text-[13px]">{viewing.career_evidence}</p></div>}<div><p className="text-[10px] text-muted uppercase tracking-wider mb-2">CV</p>{viewing.cv_url ? <a href={viewing.cv_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[13px] font-medium text-[#222321] underline">Open CV</a> : <p className="text-[13px] text-secondary">{viewing.private_mode ? 'Hidden while this profile is private. It is released with the rest of the profile if they accept your introduction.' : 'This professional has not uploaded a CV. Everything above comes from their structured profile, which is the part Talent House verifies.'}</p>}</div><p className="text-[12px] text-secondary">Personal phone and email stay private. A conversation opens only after both sides show interest in the same role.</p></div>{viewing.private_mode ? <div className="p-6 border-t border-[#dcd4c8] sticky bottom-0 bg-white/95 backdrop-blur"><button type="button" disabled={approachedIds.has(viewing.id) || busyKey === `approach:${viewing.id}`} onClick={() => requestIntroduction(viewing)} className="btn-primary w-full disabled:opacity-50 flex items-center justify-center gap-2"><Lock size={14} />{approachedIds.has(viewing.id) ? 'Introduction requested' : 'Request confidential introduction'}</button><p className="text-[11px] text-center text-muted mt-2">They approve the introduction before you see their full profile. You will be notified either way.</p></div> : <div className="p-6 border-t border-[#dcd4c8] sticky bottom-0 bg-white/95 backdrop-blur grid grid-cols-2 gap-2"><button type="button" onClick={() => toggleShortlist(viewing.id, viewing.bestJobId)} className="btn-secondary">{shortlistedIds.has(viewing.id) ? 'Saved ✓' : 'Save profile'}</button><button type="button" disabled={liveRoleCount === 0 || !viewing.bestJobId || allRolesApproached(viewing)} onClick={() => handleInterest(viewing)} className="btn-primary disabled:opacity-50">{liveRoleCount === 0 ? 'Post a live role first' : allRolesApproached(viewing) ? 'Interest sent ✓' : eligibleRoles(viewing).length > 1 ? 'Interested - choose role' : viewing.bestJob ? `Interested for ${viewing.bestJob}` : 'No eligible live role'}</button></div>}</div></div>}

      {roleChooser && <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setRoleChooser(null)}><div {...roleChooserDialog.panelProps} className="bg-white rounded-2xl max-w-sm w-full p-6"><h3 id="candidate-role-chooser-heading" className="text-[18px] font-semibold text-ink mb-1">Which role is this approach for?</h3><p className="text-[13px] text-secondary mb-5">{roleChooser.full_name || 'This professional'} suits more than one of your live roles. Choose the role they are being approached for.</p><div className="space-y-2">{eligibleRoles(roleChooser).map((r: any) => { const sent = interestedKeys.has(`${roleChooser.id}:${r.id}`); return <button key={r.id} type="button" disabled={sent || busyKey === `interest:${roleChooser.id}`} onClick={() => handleInterest(roleChooser, r.id)} className={`w-full text-left rounded-xl border p-4 flex items-center justify-between gap-3 ${sent ? 'border-emerald-200 bg-emerald-50/50 cursor-default' : 'border-[#dcd4c8] hover:border-accent/40 hover:bg-[#ede8df]'}`}><span className="min-w-0"><span className="block text-[13px] font-semibold text-ink truncate">{r.title}</span>{typeof r.matchScore === 'number' && <span className="block text-[11px] text-secondary mt-0.5">{r.matchScore}% match{r.matchLabel ? ` · ${r.matchLabel}` : ''}</span>}</span>{sent ? <span className="text-[11px] font-semibold text-emerald-700 whitespace-nowrap">Interest sent ✓</span> : <span className="text-[11px] font-semibold text-accent whitespace-nowrap">Register</span>}</button> })}</div><button type="button" onClick={() => setRoleChooser(null)} className="btn-secondary block w-full mt-4">Cancel</button></div></div>}
      {matchInfo && <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setMatchInfo(null)}><div {...matchInfoDialog.panelProps} className="bg-white rounded-2xl max-w-sm w-full p-8 text-center"><p className="public-eyebrow mb-3">Mutual interest</p><h3 id="candidate-match-heading" className="text-[22px] font-serif text-ink mb-2">{matchInfo.name} is interested too.</h3><p className="text-[14px] text-secondary mb-6">You have both registered interest in {matchInfo.job}, so a conversation is open. It stays on Talent House, and contact details unlock at interview stage.</p><div className="space-y-2"><a href="/employer/messages" className="btn-primary block w-full text-center">Open Talent House Messages</a><button type="button" onClick={() => setMatchInfo(null)} className="btn-secondary block w-full">Keep browsing</button></div></div></div>}
    </DashboardShell>
  )
}
