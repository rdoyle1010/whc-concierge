...
export default function EmployerCandidatesPage() {
  ...
  return (
    <DashboardShell role="employer" userName={profile?.company_name || profile?.property_name}>
      ...
      {matchInfo && <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setMatchInfo(null)}><div className="bg-white rounded-2xl max-w-sm w-full p-8 text-center" onClick={e => e.stopPropagation()}><p className="public-eyebrow mb-3">Mutual interest</p><h3 className="text-[22px] font-serif text-ink mb-2">{matchInfo.name} is interested too.</h3><p className="text-[14px] text-secondary mb-6">You have both registered interest in {matchInfo.job}, so a conversation is open. It stays on WHC, and contact details unlock at interview stage.</p><div className="space-y-2"><a href="/employer/messages" className="btn-primary block w-full text-center">Open WHC Messages</a><button type="button" onClick={() => setMatchInfo(null)} className="btn-secondary block w-full">Keep browsing</button></div></div></div>}
    </DashboardShell>
  )
}
