        ...
        {filtered.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.map(c => <div key={c.id} className={`dashboard-card relative ${c.is_featured ? 'border-accent ring-1 ring-accent/20' : ''}`}>
          ...
          {c.private_mode ? <>
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#dddddd]">
              <button type="button" onClick={() => handlePass(c.id)} className="py-2 bg-[#f1f1f1] hover:bg-gray-100 text-muted flex items-center justify-center gap-1 text-xs"><X size={13} />Pass</button>
              <button type="button" disabled={approachedIds.has(c.id) || busyKey === `approach:${c.id}`} onClick={() => requestIntroduction(c)} className="col-span-2 py-2 bg-[#1c1c1c] text-white flex items-center justify-center gap-1 text-xs disabled:opacity-50"><Lock size={13} />{approachedIds.has(c.id) ? 'Introduction requested' : 'Request confidential introduction'}</button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#dddddd]">
              <button type="button" onClick={() => handlePass(c.id)} className="py-2 rounded-lg bg-[#f1f1f1] hover:bg-gray-100 text-muted flex items-center justify-center gap-1 text-xs"><X size={13} />Pass</button>
              <button type="button" disabled={busyKey === `save:${c.id}`} onClick={() => toggleShortlist(c.id, c.bestJobId)} className={`py-2 rounded-lg flex items-center justify-center gap-1 text-xs ${shortlistedIds.has(c.id) ? 'bg-[#f1f1f1] text-accent' : 'bg-[#f1f1f1] text-secondary'}`}><Star size={13} fill={shortlistedIds.has(c.id) ? 'currentColor' : 'none'} />{shortlistedIds.has(c.id) ? 'Saved' : 'Save'}</button>
              <button type="button" disabled={liveRoleCount === 0 || !c.bestJobId || allRolesApproached(c) || busyKey === `interest:${c.id}`} onClick={() => handleInterest(c)} title={liveRoleCount === 0 ? 'Post a live role to express role-specific interest' : undefined} className="py-2 bg-[#1c1c1c] text-white flex items-center justify-center gap-1.5 text-xs disabled:opacity-50">{allRolesApproached(c) ? <><CheckCircle2 size={13} />Interest registered</> : isInterested(c) ? 'Register for another role' : 'Register interest'}</button>
            </div>
            {isInterested(c) && <p className="text-[10px] text-center text-emerald-700 mt-2">Emailed, and shown in their notifications.</p>}
          </>}
        </div>)}</div> : candidates.length === 0 ? <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-[#f1f1f1] mx-auto flex items-center justify-center mb-4"><Users size={24} className="text-muted" /></div><h2 className="text-xl font-serif text-ink">New professionals are joining the platform.</h2><p className="text-sm text-secondary mt-2 max-w-xl mx-auto">There are no discoverable professionals in your current search area yet.</p><div className="flex flex-wrap justify-center gap-3 mt-6"><a href="/employer/post-role" className="btn-primary">Post a role</a><button type="button" onClick={resetFilters} className="btn-secondary">Search all locations</button></div></div> : <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-[#f1f1f1] mx-auto flex items-center justify-center mb-4"><Search size={23} className="text-muted" /></div><h2 className="text-xl font-serif text-ink">No talent matches these filters.</h2><p className="text-sm text-secondary mt-2">Try widening your search area or clearing the filters.</p><button type="button" onClick={resetFilters} className="btn-secondary mt-6">Reset filters & expand search</button></div>}
