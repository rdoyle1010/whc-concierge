...
export default function EmployerCandidatesPage() {
  ...
  async function handleInterest(candidate: any) {
    if (!candidate.bestJobId || busyKey) return
    const key = `${candidate.id}:${candidate.bestJobId}`
    setBusyKey(`interest:${key}`)
    setActionMessage('')
    const res = await fetch('/api/swipe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ targetId: candidate.id, targetType: 'candidate', action: 'right', contextJobId: candidate.bestJobId }) }).catch(() => null)
    ...
    setActionMessage(`Interest sent for ${candidate.bestJob}. The professional is notified. If they are interested too, a conversation opens automatically.`)
    setBusyKey('')
  ...
  function isInterested(candidate: any) {
    return Boolean(candidate.bestJobId && interestedKeys.has(`${candidate.id}:${candidate.bestJobId}`))
  }
    ...
    <DashboardShell role="employer" userName={profile?.company_name || profile?.property_name}>
      ...
      {loading ? <div className="flex items-center justify-center h-64"><div className="animate-spin w-8 h-8 border-2 border-gold border-t-transparent rounded-full" /></div> : <>
        {filtered.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.map(c => <div key={c.id} className={`dashboard-card relative ${c.is_featured ? 'border-accent ring-1 ring-accent/20' : ''}`}>
          ...
          {Array.isArray(c.awards) && c.awards.length > 0 && <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-[#FDF6EC] px-2.5 py-1 text-[10px] font-semibold text-[#9a7436]"><Award size={11}/>{c.awards.length} award{c.awards.length === 1 ? '' : 's'}</div>}
          ...
          <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100">
            ...
            <button type="button" disabled={liveRoleCount === 0 || !c.bestJobId || isInterested(c) || busyKey === `interest:${c.id}:${c.bestJobId}`} onClick={() => handleInterest(c)} title={liveRoleCount === 0 ? 'Post a live role to express role-specific interest' : undefined} className="py-2 rounded-lg bg-[#0b2f4d] text-white flex items-center justify-center gap-1 text-xs disabled:opacity-50"><Heart size={13} fill={isInterested(c) ? 'currentColor' : 'none'} />{isInterested(c) ? 'Interest sent' : 'Interested'}</button>
          </div>
          {isInterested(c) && <p className="text-[10px] text-center text-emerald-700 mt-2">The professional has been notified for {c.bestJob}.</p>}
        </div>)}</div> : candidates.length === 0 ? <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-gray-50 mx-auto flex items-center justify-center mb-4"><Users size={24} className="text-gray-400" /></div><h2 className="text-xl font-serif text-ink">New professionals are joining the platform.</h2><p className="text-sm text-gray-500 mt-2 max-w-xl mx-auto">There are no discoverable professionals in your current search area yet.</p><div className="flex flex-wrap justify-center gap-3 mt-6"><a href="/employer/jobs/new" className="btn-primary">Post a role</a><button type="button" onClick={resetFilters} className="btn-secondary">Search all locations</button></div></div> : <div className="dashboard-card text-center py-14 px-6"><div className="w-14 h-14 rounded-full bg-gray-50 mx-auto flex items-center justify-center mb-4"><Search size={23} className="text-gray-400" /></div><h2 className="text-xl font-serif text-ink">No talent matches these filters.</h2><p className="text-sm text-gray-500 mt-2">Try widening your search area or clearing the filters.</p><button type="button" onClick={resetFilters} className="btn-secondary mt-6">Reset filters & expand search</button></div>}
