    ...
    <DashboardShell role="employer" userName={profile?.company_name || profile?.property_name}>
      ...
      {loading ? <div className="flex items-center justify-center h-64"><div className="animate-spin w-8 h-8 border-2 border-gold border-t-transparent rounded-full" /></div> : <>
        {filtered.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.map(c => <div key={c.id} className={`dashboard-card relative ${c.is_featured ? 'border-accent ring-1 ring-accent/20' : ''}`}>
          ...
          {Array.isArray(c.awards) && c.awards.length > 0 && <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-[#FDF6EC] px-2.5 py-1 text-[10px] font-semibold text-[#1a1a1a]"><Award size={11}/>{c.awards.length} award{c.awards.length === 1 ? '' : 's'}</div>}
          ...
          <button type="button" onClick={() => setViewing(c)} className="w-full mb-2 py-2 rounded-lg text-[12px] font-medium" style={{ background: '#FDF6EC', color: '#555555', border: '1px solid rgba(201,169,110,0.35)' }}>View Full Profile</button>
          <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100">
            ...
            <button type="button" disabled={busyKey === `save:${c.id}`} onClick={() => toggleShortlist(c.id, c.bestJobId)} className={`py-2 rounded-lg flex items-center justify-center gap-1 text-xs ${shortlistedIds.has(c.id) ? 'bg-[#FDF6EC] text-accent' : 'bg-gray-50 text-gray-500'}`}><Star size={13} fill={shortlistedIds.has(c.id) ? 'currentColor' : 'none'} />{shortlistedIds.has(c.id) ? 'Saved' : 'Save'}</button>
            <button type="button" disabled={liveRoleCount === 0 || !c.bestJobId || allRolesApproached(c) || busyKey === `interest:${c.id}`} onClick={() => handleInterest(c)} title={liveRoleCount === 0 ? 'Post a live role to express role-specific interest' : undefined} className="py-2 rounded-lg bg-[#111111] text-white flex items-center justify-center gap-1 text-xs disabled:opacity-50"><Heart size={13} fill={isInterested(c) ? 'currentColor' : 'none'} />{allRolesApproached(c) ? 'Interest sent' : isInterested(c) ? 'Another role' : 'Interested'}</button>
          </div>
