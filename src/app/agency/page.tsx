...
export default function AgencyPage() {
  ...
  function candidateCard(c:any){
    ...
    return <div className={`h-[500px] overflow-hidden rounded-3xl border bg-white shadow-xl ${c.is_featured?'border-accent ring-1 ring-accent/20':'border-border'}`}><div className="p-7"><div className="flex items-start gap-4 mb-4"><div className="w-20 h-20 rounded-full bg-[#f5f6f8] border border-[#e3e7eb] overflow-hidden flex items-center justify-center shrink-0">{c.profile_image_url?<img src={c.profile_image_url} alt="" className="w-full h-full object-cover" draggable={false}/>:<span className="text-[24px] font-semibold text-[#0b2f4d]">{c.full_name?.[0]}</span>}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="text-[24px] font-semibold text-ink truncate">{c.full_name}</h3>{c.is_featured&&<span className="text-[9px] font-semibold bg-accent text-white px-2 py-0.5 rounded-full">FEATURED</span>}<button type="button" title={favourites.has(c.id)?'Remove from favourites':'Save to favourites'} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.preventDefault();e.stopPropagation();toggleFavourite(c.id)}} disabled={favBusy===c.id} className="ml-auto shrink-0 text-accent disabled:opacity-50"><Heart size={18} className={favourites.has(c.id)?'fill-current':''}/></button></div>{c.role_level&&<p className="mt-1 text-[12px] font-medium text-secondary">{c.role_level}</p>}{c.headline&&<p className="text-[12px] text-muted line-clamp-2 mt-1">{c.headline}</p>}</div></div><div className="flex flex-wrap items-center gap-3 mb-4">{c.review_score>0?<span className="flex items-center gap-1 text-[12px]"><Star size={12} className="text-amber-400" fill="currentColor"/>{c.review_score} ({c.review_count})</span>:<span className="text-[11px] text-muted">New</span>}{(c.location||c.distance_miles!=null)&&<span className="text-[11px] text-muted flex items-center gap-1"><MapPin size={11}/>{c.location||'Location available'}{c.distance_miles!=null?` · ${c.distance_miles} mi away`:''}</span>}{c.agency_ready&&<span className="text-[9px] font-semibold uppercase tracking-[.08em] bg-[#0b2f4d] text-white px-2 py-1 flex items-center gap-1"><ShieldCheck size={10}/>Agency Ready</span>}{c.whc_verified&&<span className="text-[10px] font-semibold text-green-700 flex items-center gap-1"><ShieldCheck size={11}/>WHC Verified</span>}{c.has_insurance&&<span className="text-[10px] text-green-700 flex items-center gap-1"><Shield size={11}/>Insured</span>}{c.completed_shift_count>0&&<span className="text-[10px] text-secondary">{c.completed_shift_count} WHC shift{c.completed_shift_count===1?'':'s'} completed</span>}{c.reliability_pct!=null&&<span className={`text-[10px] font-semibold ${c.reliability_pct>=90?'text-green-700':'text-amber-600'}`}>Reliability {c.reliability_pct}%</span>}</div>{(academyMap.get(c.id)?.length||0)>0&&<p className="text-[11px] text-accent mb-3 flex items-center gap-1"><GraduationCap size={12}/>{academyMap.get(c.id)!.length} WHC Academy completion{academyMap.get(c.id)!.length===1?'':'s'}</p>}{c.services_offered?.length>0&&<div className="flex flex-wrap gap-1.5 mb-4">{c.services_offered.slice(0,5).map((s:string)=><span key={s} className="text-[10px] bg-gray-100 text-gray-700 px-2 py-1 rounded-full">{s}</span>)}</div>}{c.hourly_rate&&<div className="mb-4"><p className="text-[20px] font-semibold text-accent">£{c.hourly_rate}<span className="text-[11px] font-normal text-muted"> / hour</span></p>{therapistCost!=null&&<p className="text-[11px] text-muted">£{therapistCost} therapist cost for {selectedHours}h{fee!=null?` · £${fee} WHC fee at booking`:''}</p>}</div>}{c.availability_match!=null&&<div className="mb-5">{c.availability_match==='confirmed'?<span className="text-[12px] text-success font-semibold flex items-center gap-1"><CheckCircle2 size={13}/>Available {shiftStartTime}–{shiftEndTime}</span>:<span className="text-[11px] text-amber-600">Availability not confirmed for this shift</span>}</div>}<Link href={profileParams(c)} onPointerDown={e=>e.stopPropagation()} className="btn-primary block w-full text-center text-[12px]">View Profile & Make an Offer</Link></div></div>
  }

  return <DashboardShell role="employer">
    <section className="mb-8"><div className="max-w-[1460px] mx-auto"><p className="dashboard-eyebrow">Agency staffing</p><h1 className="dashboard-title">Find available spa professionals</h1><p className="dashboard-intro">Search verified, insured professionals who have confirmed availability for the exact date and hours you need.</p><form onSubmit={e=>{e.preventDefault();handleSearch()}} className="max-w-5xl space-y-4 mt-7 bg-white border border-border rounded-2xl p-5 md:p-6 shadow-sm"><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><label className="text-[11px] font-medium text-secondary">Shift date (optional)<input type="date" min={new Date().toLocaleDateString('en-CA')} value={shiftDate} onChange={e=>setShiftDate(e.target.value)} className="input-field mt-1 w-full"/></label><label className="text-[11px] font-medium text-secondary">Starts<input required type="time" value={shiftStartTime} onChange={e=>setShiftStartTime(e.target.value)} className="input-field mt-1 w-full"/></label><label className="text-[11px] font-medium text-secondary">Finishes<input required type="time" value={shiftEndTime} onChange={e=>setShiftEndTime(e.target.value)} className="input-field mt-1 w-full"/></label></div><div className="flex flex-col sm:flex-row gap-3"><input type="text" placeholder="Enter postcode" value={postcode} onChange={e=>{setPostcode(e.target.value);setPostcodeError('')}} className="input-field flex-1"/><select value={radius} onChange={e=>setRadius(e.target.value)} className="input-field sm:w-40"><option>UK-wide</option><option>5 miles</option><option>10 miles</option><option>25 miles</option><option>50 miles</option><option>100 miles</option></select><button type="submit" className="btn-primary flex items-center justify-center gap-2"><Search size={14}/>Find available talent</button></div><p className="text-[11px] text-muted">Leave the date blank to browse everyone on the register. With a date and hours set, availability is checked for the whole shift and overlapping bookings are excluded.</p></form>{postcodeError&&<p className="text-[12px] text-red-600 mt-2">{postcodeError}</p>}{appliedSearch&&<div className="mt-3"><span className="inline-flex items-center gap-1.5 text-[12px] font-medium bg-[#f5f6f8] text-accent border border-accent/20 px-3 py-1 rounded-full"><MapPin size={11}/>Near {appliedSearch.outward} (within ~{appliedSearch.radius})<button type="button" onClick={searchUkWide}><X size={12}/></button></span></div>}<div className="max-w-5xl mt-5 rounded-2xl border border-[#e3e7eb] bg-[#f5f6f8] p-5"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#10283b]">How WHC verifies agency professionals</p><div className="mt-3 grid gap-4 sm:grid-cols-3 text-[12px] leading-5 text-secondary"><div><p className="font-semibold text-ink mb-1">Insurance</p>Self-employed therapists must hold their own professional insurance. Documents are uploaded to WHC and shown as an Insured badge - employers can filter to insured-only.</div><div><p className="font-semibold text-ink mb-1">Qualifications</p>Certificates are uploaded and reviewed, including overseas training - profiles distinguish reviewed qualifications from self-declared ones, so HR isn't left decoding unfamiliar certificate names.</div><div><p className="font-semibold text-ink mb-1">WHC Verified</p>The WHC Verified badge covers identity and right-to-work checks, and completed WHC Academy training appears on the profile with certificate codes.</div></div></div></div></section>
    ...
    <div className="max-w-[1460px] mx-auto pb-10"><div className="flex gap-8"><aside className="hidden lg:block w-[260px] shrink-0 sticky top-[76px] self-start max-h-[calc(100vh-100px)] overflow-y-auto">{filterPanel}</aside>{filtersOpen&&<div className="fixed inset-0 z-50 lg:hidden"><div className="absolute inset-0 bg-black/50" onClick={()=>setFiltersOpen(false)}/><div className="absolute inset-y-0 left-0 w-[300px] max-w-[85vw] bg-surface overflow-y-auto p-4"><div className="flex items-center justify-between mb-3"><p className="text-[14px] font-semibold text-ink">Refine search</p><button type="button" onClick={()=>setFiltersOpen(false)}><X size={18}/></button></div>{filterPanel}<button type="button" onClick={()=>setFiltersOpen(false)} className="btn-primary w-full mt-4">Show {sorted.length} professionals</button></div></div>}<div className="flex-1 min-w-0"><div className="flex flex-col gap-3 mb-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><button type="button" onClick={()=>setFiltersOpen(true)} className="lg:hidden inline-flex items-center gap-1.5 text-[12px] font-medium border border-border bg-white rounded-lg px-3 py-1.5"><SlidersHorizontal size={13}/>Filters</button><p className="text-[13px] text-muted">{sorted.length} professional{sorted.length!==1?'s':''}</p></div><div className="flex flex-wrap items-center gap-2"><div className="inline-flex rounded-xl border border-border bg-white p-1"><button type="button" onClick={()=>setViewMode('swipe')} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-[11px] font-semibold ${viewMode==='swipe'?'bg-[#0b2f4d] text-white':'text-secondary'}`}><Layers3 size={13}/>Swipe</button><button type="button" onClick={()=>setViewMode('list')} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-[11px] font-semibold ${viewMode==='list'?'bg-[#0b2f4d] text-white':'text-secondary'}`}><Rows3 size={13}/>List</button></div><select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="input-field !w-auto !py-1.5 text-[12px]"><option value="match">Best Match</option><option value="rated">Highest Rated</option><option value="rate_low">Hourly Rate ↑</option><option value="rate_high">Hourly Rate ↓</option><option value="recent">Most Recent</option></select></div></div>
      {loading?<div className="grid grid-cols-1 xl:grid-cols-2 gap-5">{Array.from({length:4}).map((_,i)=><div key={i} className="skeleton h-72 rounded-2xl"/>)}</div>:requiresSignIn?<div className="bg-white border border-border p-10 text-center"><h2 className="text-2xl font-semibold text-ink mb-2">Agency profiles are private</h2><p className="text-[13px] text-muted max-w-lg mx-auto mb-5">Sign in with an approved hotel or spa account to search professionals.</p><Link href="/login?account=employer" className="btn-primary inline-block">Hotel & Spa Sign In</Link></div>:directoryError?<div className="bg-white border border-border p-10 text-center text-sm text-red-600">{directoryError}</div>:sorted.length===0?<div className="bg-white border border-border rounded-xl p-12 text-center"><h2 className="text-xl font-serif text-ink mb-2">No available professionals match this search.</h2><p className="text-[13px] text-muted mb-5">Try widening the radius, changing the shift hours or clearing filters.</p><div className="flex gap-3 justify-center flex-wrap"><button type="button" onClick={searchUkWide} className="btn-primary">Search UK-wide</button><button type="button" onClick={clearFilters} className="btn-secondary">Clear filters</button>{!originGeocoded&&<Link href="/employer/profile" className="btn-secondary">Add property postcode</Link>}</div></div>:viewMode==='swipe'?<div><div className="mb-5 rounded-2xl border border-[#e3e7eb] bg-[#f5f6f8] px-5 py-4"><p className="text-[12px] font-semibold text-ink">Swipe through available professionals</p><p className="mt-1 text-[11px] text-muted">Swipe right to continue with that professional and make an offer. Swipe left to pass. Nothing is booked until you review the profile and send the offer.</p></div><SwipeDeck items={sorted} renderItem={(c)=>candidateCard(c)} onLeft={async()=>{}} onRight={async(c)=>{window.location.href=profileParams(c)}}/></div>:<><div className="fade-in grid grid-cols-1 xl:grid-cols-2 gap-5">{sorted.slice(0,visible).map(c=><div key={c.id}>{candidateCard(c)}</div>)}</div>{visible<sorted.length&&<div className="text-center mt-8"><button type="button" onClick={()=>setVisible(v=>v+12)} className="btn-secondary">Load more professionals</button></div>}</>}
      </div></div></div>
