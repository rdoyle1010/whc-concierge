  ...
  return <>
    ...
    <main className="pt-[76px] bg-white">
      <section className="border-b border-[#e3e7eb] bg-white">
        <div className="max-w-6xl mx-auto px-6 py-4"><Link href="/jobs" className="text-[12px] text-[#5a6a76] hover:text-[#0b2f4d]">← All roles</Link></div>
      </section>

      <section className="bg-white">
        <div className="max-w-6xl mx-auto px-6 py-10 md:py-14 grid lg:grid-cols-[1fr_320px] gap-8 lg:gap-12 items-start">
          <div>
            <span className="inline-flex rounded-full bg-[#e8eef4] px-3 py-1 text-[10px] uppercase tracking-[.12em] font-semibold text-[#5a6a76]">{job.tier === 'Platinum' ? 'Featured role' : 'Standard role'}</span>{job.is_residency_role && <span className="ml-2 inline-flex rounded-full bg-[#e8eef4] px-3 py-1 text-[10px] uppercase tracking-[.12em] font-semibold text-[#0b2f4d]">Residency</span>}
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-2 mt-4 text-[13px] text-[#5a6a76]">
      ...
      {(propertyPhoto || employer.id) && <section className="border-y border-[#e3e7eb] bg-[#f5f6f8]">
        <div className="max-w-6xl mx-auto px-6 py-8 grid md:grid-cols-[220px_1fr_auto] gap-6 items-center">
          ...
          <div>
            ...
            {employer.tagline && <p className="text-[13px] text-[#5a6a76] mt-2">{employer.tagline}</p>}
            <div className="flex flex-wrap gap-4 mt-4 text-[12px] text-[#5a6a76]">
      ...
      <section className="max-w-6xl mx-auto px-6 py-14 md:py-16 grid lg:grid-cols-[minmax(0,1fr)_290px] gap-12">
        <div className="space-y-14">
          ...
          <JobSection eyebrow="The employer" title={`Why ${propertyName}`}>
            <div className="grid md:grid-cols-[1fr_auto] gap-6 items-start border border-[#e3e7eb] rounded-[20px] p-6">
              <div>
                <p className="text-[14px] leading-7 text-[#5a6a76]">{employer.about_text || employer.description || employer.tagline || `Explore the full WHC property profile for ${propertyName} to see its spa operation, staff reviews, property rating, brands, travel information and current opportunities.`}</p>
                <div className="flex flex-wrap gap-3 mt-4 text-[12px] text-[#5a6a76]">{employer.star_rating && <span>{isNaN(Number(employer.star_rating)) ? employer.star_rating : `${employer.star_rating}★ property`}</span>}{reviewScore > 0 && <span>{reviewScore.toFixed(1)} WHC staff rating</span>}{employer.hotel_group && <span>{employer.hotel_group}</span>}{employer.room_count && <span>{employer.room_count} rooms</span>}{employer.spa_size && <span>{employer.spa_size}</span>}{employer.num_treatment_rooms && <span>{employer.num_treatment_rooms} treatment rooms</span>}{employer.team_size && <span>{employer.team_size} spa team</span>}{employer.opening_year && <span>Opened {employer.opening_year}</span>}</div>
                {Array.isArray(employer.facilities) && employer.facilities.length > 0 && <div className="flex flex-wrap gap-1.5 mt-4">{employer.facilities.map((facility: string) => <span key={facility} className="text-[11px] bg-[#f5f6f8] text-[#5a6a76] px-2.5 py-1 rounded-full">{facility}</span>)}</div>}
                {employer.culture_statement && <div className="mt-5"><p className="text-[10px] uppercase tracking-[.14em] text-[#10283b] font-semibold mb-1.5">Working here</p><p className="text-[13px] leading-6 text-[#5a6a76] whitespace-pre-line">{employer.culture_statement}</p></div>}
                ...
                {employer.progression_notes && <div className="mt-4"><p className="text-[10px] uppercase tracking-[.14em] text-[#10283b] font-semibold mb-1.5">Progression</p><p className="text-[13px] leading-6 text-[#5a6a76] whitespace-pre-line">{employer.progression_notes}</p></div>}
              </div>
...
function Prose({ text }: { text: string }) { return <div className="text-[15px] leading-8 text-[#3d4b57] whitespace-pre-line">{text}</div> }
