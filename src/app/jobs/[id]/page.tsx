          ...
          <JobSection eyebrow="The employer" title={`Why ${propertyName}`}>
            <div className="grid md:grid-cols-[1fr_auto] gap-6 items-start border border-[#dfe5e8] rounded-[20px] p-6">
              <div>
                ...
                {employer.culture_statement && <div className="mt-5"><p className="text-[10px] uppercase tracking-[.14em] text-[#9c7a42] font-semibold mb-1.5">Working here</p><p className="text-[13px] leading-6 text-[#53636f] whitespace-pre-line">{employer.culture_statement}</p></div>}
                {Array.isArray(employer.staff_benefits) && employer.staff_benefits.length > 0 && <div className="mt-4"><p className="text-[10px] uppercase tracking-[.14em] text-[#9c7a42] font-semibold mb-1.5">Staff benefits</p><div className="flex flex-wrap gap-1.5">{employer.staff_benefits.map((benefit: string) => <span key={benefit} className="text-[11px] bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full">{benefit}</span>)}</div></div>}
                {employer.progression_notes && <div className="mt-4"><p className="text-[10px] uppercase tracking-[.14em] text-[#9c7a42] font-semibold mb-1.5">Progression</p><p className="text-[13px] leading-6 text-[#53636f] whitespace-pre-line">{employer.progression_notes}</p></div>}
              </div>
