  ...
  function swipeCard(job: any) {
    ...
    return <article className={`h-[500px] overflow-hidden rounded-3xl border bg-white shadow-xl ${rank >= 3 ? 'border-[#C9A96E]/55' : 'border-border'}`}>
      <div className="relative h-52 bg-[#e9e6df] overflow-hidden">
        {employer.property_photos?.[0] ? <img src={employer.property_photos[0]} alt={employer.company_name || 'Property'} className="h-full w-full object-cover" draggable={false}/> : <div className="h-full w-full flex items-center justify-center"><Building2 size={44} className="text-muted/50"/></div>}
      ...
      <div className="p-7">
        <div className="flex flex-wrap items-center justify-between gap-3"><div>{job.tier&&<span className={tierClass(job.tier)}>{job.tier}</span>}</div><span className="text-[11px] uppercase tracking-[.1em] text-muted">{job.job_type}</span></div>
        <h2 className="mt-4 text-[28px] font-semibold tracking-tight text-ink">{title}</h2>
        <div className="mt-2 flex flex-wrap items-center gap-3"><p className="text-[13px] font-semibold text-accent">{employer.company_name || 'Luxury wellness property'}</p>{employer.review_score ? <span className="inline-flex items-center gap-1 text-[12px] text-secondary"><Star size={13} className="fill-amber-400 text-amber-400"/>{employer.review_score}{employer.review_count?` (${employer.review_count})`:''}</span>:null}</div>
        <p className="mt-3 flex items-center gap-1 text-[12px] text-muted"><MapPin size={12}/>{job.location}</p>
        <p className="mt-3 text-[15px] font-medium text-ink">{job.salary_display_text || (job.salary_min && job.salary_max ? `£${job.salary_min.toLocaleString()} - £${job.salary_max.toLocaleString()}` : 'Competitive salary')}</p>
        {description && <p className="mt-3 line-clamp-3 text-[13px] leading-6 text-secondary">{description}</p>}
        <div className="mt-5 flex items-center justify-between border-t border-border pt-4"><Link href={`/jobs/${job.id}`} onPointerDown={e=>e.stopPropagation()} className="text-[13px] font-semibold text-[#9b7b43]">View full role →</Link>{saved.has(job.id)&&<span className="text-[11px] font-semibold text-emerald-700">Saved</span>}</div>
      </div>
    </article>
  }
