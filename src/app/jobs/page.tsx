...
export default function PublicJobsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (res.ok) { const d = await res.json(); if (active) setSaved(new Set((d.saved || []).map((s: any) => s.job_id))) }
    }
    load()
    return () => { active = false }
  }, [])
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {paginatedJobs.map((job) => (
                  <div key={job.id} className="bg-white border border-border rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all group relative">
                    <Link href={`/jobs/${job.id}`} aria-label={`View role: ${job.title}`} className="absolute inset-0 rounded-2xl z-0" />
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex flex-wrap items-center gap-3"><p className="text-[13px] font-semibold text-accent">{job.employer_profiles?.company_name || 'Luxury wellness property'}</p>{job.employer_profiles?.review_score ? <span className="inline-flex items-center gap-1 text-[12px] text-secondary"><Star size={13} className="fill-amber-400 text-amber-400" /> {job.employer_profiles.review_score} {job.employer_profiles.review_count ? `(${job.employer_profiles.review_count})` : ''}</span> : job.employer_profiles?.star_rating ? <span className="inline-flex items-center gap-1 text-[12px] text-secondary"><Star size={13} className="fill-amber-400 text-amber-400" /> {job.employer_profiles.star_rating}-star property</span> : null}</div>
                      ...
                      <p className="text-[14px] font-medium text-ink mt-3">{job.salary_min && job.salary_max ? `£${job.salary_min.toLocaleString()} - £${job.salary_max.toLocaleString()}` : 'Competitive salary'}</p>
