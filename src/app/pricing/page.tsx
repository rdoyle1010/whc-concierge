    ...
    <main className="pt-[76px]">
      ...
      <section className="py-16 px-6 bg-[#f5f6f8]">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl mb-9"><p className="text-[10px] uppercase tracking-[.18em] font-semibold text-[#10283b]">Talent memberships</p><h2 className="text-[32px] md:text-[42px] tracking-[-.04em] font-semibold text-[#10283b] mt-2">Build your career at your pace.</h2><p className="text-[13px] leading-6 text-[#5a6a76] mt-3">Start free. Upgrade when you want deeper matching, more Interview Ready support and stronger employer visibility.</p></div>
          <div className="grid md:grid-cols-3 gap-5">{talentPlans.map(plan => <article key={plan.title} className={`rounded-[22px] bg-white p-6 flex flex-col ${plan.featured?'border border-[#5a6a76] ring-1 ring-[#5a6a76]/20 shadow-lg':'border border-[#e3e7eb]'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#10283b]">{plan.eyebrow}</p><h3 className="text-[20px] font-semibold text-[#10283b] mt-2">{plan.title}</h3></div>{plan.featured&&<Sparkles size={18} className="text-[#10283b]"/>}</div><div className="mt-5"><span className="text-[34px] font-semibold text-[#10283b]">{plan.price}</span><span className="text-[12px] text-[#8a949b]">{plan.cadence}</span></div><p className="text-[12px] leading-6 text-[#5a6a76] mt-4">{plan.text}</p><div className="space-y-2 mt-5 mb-7">{plan.bullets.map(b=><div key={b} className="flex gap-2 text-[11px] text-[#5a6a76]"><Check size={13} className="text-[#10283b] shrink-0 mt-0.5"/>{b}</div>)}</div><Link href={plan.href} className={`${plan.featured?'btn-accent':'btn-secondary'} mt-auto text-center`}>{plan.cta}</Link></article>)}</div>

          <div className="mt-6 rounded-[22px] bg-[#10283b] text-white p-6 md:p-8 grid md:grid-cols-[1fr_auto] gap-6 items-center">
            <div><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#5a6a76]">Extra visibility when you need it</p><h3 className="text-[25px] font-semibold mt-2">Featured Talent</h3><p className="text-[12px] leading-6 text-white/65 mt-2">Priority visibility in employer searches, recommendation areas and selected employer communications, clearly labelled. Featuring never changes matching.</p></div>
      ...
      <section className="py-16 px-6 bg-white border-y border-[#e3e7eb]">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl mb-9"><p className="text-[10px] uppercase tracking-[.18em] font-semibold text-[#10283b]">Flexible work & specialist placements</p><h2 className="text-[32px] md:text-[42px] tracking-[-.04em] font-semibold text-[#10283b] mt-2">The professional keeps the agreed rate.</h2><p className="text-[13px] leading-6 text-[#5a6a76] mt-3">WHC adds its platform fee to the property side, so the rate a professional agrees is the rate they earn.</p></div>
          <div className="grid md:grid-cols-2 gap-5">
            <div className="rounded-[22px] bg-[#f5f6f8] border border-[#e3e7eb] p-7"><CalendarDays size={20} className="text-[#10283b]"/><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b] mt-5">Agency booking</p><h3 className="text-[26px] font-semibold text-[#10283b] mt-2">Agreed shift value + {Math.round(AGENCY_PLATFORM_FEE_PCT * 100)}%</h3><p className="text-[12px] leading-6 text-[#5a6a76] mt-3">Example: £20/hour × 10 hours = £200 to the professional. WHC fee £30. Property total £230.</p><p className="text-[11px] font-semibold text-[#10283b] mt-4">The rate you see is the rate you earn.</p></div>
            <div className="rounded-[22px] bg-[#0b2f4d] p-7 text-white"><MapPin size={20} className="text-[#5a6a76]"/><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#5a6a76] mt-5">Residency listings</p><h3 className="text-[26px] font-semibold text-white mt-2">Free to book · 10% on confirmation</h3><p className="text-[12px] leading-6 text-white/65 mt-3">Browse verified residency specialists free. Pay only when a booking is agreed and confirmed - a 10% platform fee protects both sides. Specialists list for £10/month.</p></div>
          </div>
        </div>
      </section>

      <section className="py-16 px-6 bg-[#f5f6f8] border-b border-[#e3e7eb]">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl mb-9"><p className="text-[10px] uppercase tracking-[.18em] font-semibold text-[#10283b]">Employer pricing</p><h2 className="text-[32px] md:text-[42px] tracking-[-.04em] font-semibold text-[#10283b] mt-2">Advertise once or build an ongoing hiring partnership.</h2><p className="text-[13px] leading-6 text-[#5a6a76] mt-3">Employers can start free, buy individual job listings or choose an annual membership for regular recruitment.</p></div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            <article className="rounded-[22px] bg-white border border-[#e3e7eb] p-6"><BriefcaseBusiness size={19} className="text-[#10283b]"/><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#10283b] mt-5">Job advertising</p><h3 className="text-[20px] font-semibold text-[#10283b] mt-2">Standard Job</h3><p className="text-[31px] font-semibold text-[#10283b] mt-4">{JOB_TIERS.Bronze.display}</p><p className="text-[11px] text-[#5a6a76] mt-2">30 days · matching · applications · shortlist · filled-role notifications</p></article>
            <article className="rounded-[22px] bg-white border border-[#5a6a76] ring-1 ring-[#5a6a76]/20 p-6"><Sparkles size={19} className="text-[#10283b]"/><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#10283b] mt-5">More visibility</p><h3 className="text-[20px] font-semibold text-[#10283b] mt-2">Featured Job</h3><p className="text-[31px] font-semibold text-[#10283b] mt-4">{JOB_TIERS.Platinum.display}</p><p className="text-[11px] text-[#5a6a76] mt-2">30 days · priority placement · featured badge · talent email · enhanced branding</p></article>
            <article className="rounded-[22px] bg-white border border-[#e3e7eb] p-6"><Building2 size={19} className="text-[#10283b]"/><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#10283b] mt-5">Annual membership</p><h3 className="text-[20px] font-semibold text-[#10283b] mt-2">Employer Pro</h3><p className="text-[31px] font-semibold text-[#10283b] mt-4">{pounds(EMPLOYER_MEMBERSHIPS.pro.price)}<span className="text-[12px] text-[#8a949b] font-normal">/year</span></p><p className="text-[11px] text-[#5a6a76] mt-2">Full talent search, analytics, shortlisting, Property Fact Files and {pounds(EMPLOYER_MEMBERSHIPS.pro.discountedStandardJobPrice)} Standard Jobs.</p></article>
            <article className="rounded-[22px] bg-[#10283b] text-white p-6"><Building2 size={19} className="text-[#5a6a76]"/><p className="text-[9px] uppercase tracking-[.16em] font-semibold text-[#5a6a76] mt-5">Groups & regular recruiters</p><h3 className="text-[20px] font-semibold mt-2">Employer Group</h3><p className="text-[31px] font-semibold mt-4">{pounds(EMPLOYER_MEMBERSHIPS.group.price)}<span className="text-[12px] text-white/55 font-normal">/year</span></p><p className="text-[11px] text-white/60 mt-2">Up to 20 job listings per year, multiple properties, multiple hiring managers and advanced talent access.</p></article>
          </div>
        </div>
      </section>

      <section className="py-16 px-6 bg-white border-b border-[#e3e7eb]">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-5">
          <div className="rounded-[22px] border border-[#e3e7eb] p-7"><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b]">WHC Recruitment Service</p><h3 className="text-[28px] font-semibold text-[#10283b] mt-2">12.5% of first-year salary</h3><p className="text-[12px] leading-6 text-[#5a6a76] mt-3">WHC takes the vacancy brief, searches the platform and wider network, pre-screens candidates and delivers a shortlist, with a replacement guarantee on every placement.</p></div>
          <div className="rounded-[22px] bg-[#f5f6f8] border border-[#e3e7eb] p-7"><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b]">Executive Search</p><h3 className="text-[28px] font-semibold text-[#10283b] mt-2">15–20% of first-year salary</h3><p className="text-[12px] leading-6 text-[#5a6a76] mt-3">For Director of Spa, Wellness Director, Regional leadership, Group Spa Director and other senior confidential searches.</p></div>
        </div>
      </section>

      <section className="py-16 px-6 bg-[#f5f6f8] border-b border-[#e3e7eb]">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-5">
          <div className="rounded-[22px] bg-white border border-[#e3e7eb] p-6"><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b]">Academy</p><h3 className="text-[22px] font-semibold text-[#10283b] mt-2">£29–£199+</h3><p className="text-[12px] text-[#5a6a76] mt-3">Free members pay standard price. Standard members receive 10% off. Pro members receive 20% off.</p></div>
          <div className="rounded-[22px] bg-white border border-[#e3e7eb] p-6"><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b]">Brand exposure</p><h3 className="text-[22px] font-semibold text-[#10283b] mt-2">From £295</h3><p className="text-[12px] text-[#5a6a76] mt-3">Brand Spotlight £295, Industry Feature £495 and Partner Campaigns from £995.</p></div>
          <div className="rounded-[22px] bg-white border border-[#e3e7eb] p-6"><p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#10283b]">Employer profile</p><h3 className="text-[22px] font-semibold text-[#10283b] mt-2">Free to start</h3><p className="text-[12px] text-[#5a6a76] mt-3">Create your property profile, add photographs and information, receive applications and build employer reputation.</p></div>
        </div>
      </section>

      <section className="py-16 px-6 bg-white"><div className="max-w-3xl mx-auto"><div className="text-center mb-10"><p className="text-[10px] uppercase tracking-[.18em] font-semibold text-[#10283b] mb-3">Questions</p><h2 className="text-[30px] font-semibold text-[#10283b]">Pricing without the small-print mystery.</h2></div><div className="space-y-3">{FAQS.map((faq,i)=><div key={i} className="rounded-2xl border border-[#e3e7eb] bg-white overflow-hidden"><button type="button" onClick={()=>setOpenFaq(openFaq===i?null:i)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-[#f5f6f8]"><span className="text-[14px] font-semibold text-[#10283b]">{faq.q}</span><ChevronDown size={16} className={`transition-transform ${openFaq===i?'rotate-180':''}`}/></button>{openFaq===i&&<div className="px-5 pb-4"><p className="text-[13px] leading-7 text-secondary">{faq.a}</p></div>}</div>)}</div></div></section>
      <section className="py-8 px-6 bg-[#f5f6f8] border-t border-[#e3e7eb]"><div className="max-w-5xl mx-auto flex items-center justify-center gap-3 text-center"><Shield size={16} className="text-secondary"/><p className="text-[12px] text-secondary">Secure online payments through Stripe. Paid services are subject to the platform Terms and relevant cancellation/refund rules.</p></div></section>
    </main>
    <Footer />
  </div>
}
