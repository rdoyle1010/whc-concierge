  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {step === 5 && <div className="space-y-5"><p className="eyebrow mb-4">Step 5 - Review & Submit</p><div className="bg-white border border-border rounded-xl p-6 space-y-3"><h3 className="text-[17px] font-medium text-ink">{form.title}</h3>{form.description && <p className="text-[13px] text-secondary">{form.description}</p>}{form.services_offered.length > 0 && <div className="flex flex-wrap gap-1">{form.services_offered.map(s => <span key={s} className="text-[10px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">{s}</span>)}</div>}{form.product_houses.length > 0 && <div className="flex flex-wrap gap-1">{form.product_houses.map(b => <span key={b} className="text-[10px] bg-[#f1f1f1] text-accent border border-accent/20 px-2 py-0.5 rounded-full">{b}</span>)}</div>}<div className="flex gap-3 text-[13px]">{form.day_rate && <span className="bg-surface px-2.5 py-1 rounded-lg font-medium text-ink">£{form.day_rate}/day</span>}{(form.weekly_rate || autoWeekly) && <span className="bg-surface px-2.5 py-1 rounded-lg font-medium text-ink">£{form.weekly_rate || autoWeekly}/week</span>}</div><p className="text-[12px] text-muted">Travel: {form.travel_availability} {form.availability_start ? `· From ${form.availability_start}` : ''} {form.preferred_duration ? `· ${form.preferred_duration}` : ''}</p></div><div className="bg-surface p-4 rounded-lg text-[13px] text-secondary">Your paid membership is active. Your listing will be reviewed by the Talent House Collective team before going live.</div><div className="flex gap-3"><button type="button" onClick={() => setStep(4)} className="btn-secondary flex-1">Back</button><button type="button" onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 disabled:opacity-50">{loading ? 'Submitting...' : 'Submit for Approval'}</button></div></div>}
      </div>
    </div>
  )
}
