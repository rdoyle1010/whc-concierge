    ...
    <div className="min-h-screen bg-[#F7F5F1]">
      ...
      <div className="max-w-3xl mx-auto px-6 py-14">
        <div className="bg-white border border-border rounded-[28px] p-8 md:p-10 shadow-sm">
          ...
          <div className="mt-5 flex items-start gap-2 text-xs text-muted"><ShieldCheck size={15} className="text-accent shrink-0"/><p>When a property books you, the property pays a 10% Spa Platform booking fee. Your agreed residency rate stays separate from that fee.</p></div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {step === 3 && <div className="space-y-5"><p className="eyebrow mb-4">Step 3 - Availability & Rates</p><div><label className="eyebrow block mb-1.5">Available From</label><input type="date" value={form.availability_start} onChange={e => u('availability_start', e.target.value)} className="input-field" /></div><div><label className="eyebrow block mb-1.5">Preferred Duration</label><select value={form.preferred_duration} onChange={e => u('preferred_duration', e.target.value)} className="input-field"><option value="">Select</option>{DURATION_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}</select></div><div className="grid grid-cols-3 gap-4"><div><label className="eyebrow block mb-1.5">Day Rate (£)</label><input type="number" value={form.day_rate} onChange={e => u('day_rate', e.target.value)} className="input-field" /></div><div><label className="eyebrow block mb-1.5">Weekly Rate (£)</label><input type="number" value={form.weekly_rate || autoWeekly} onChange={e => u('weekly_rate', e.target.value)} className="input-field" /></div><div><label className="eyebrow block mb-1.5">Monthly Rate (£)</label><input type="number" value={form.monthly_rate || autoMonthly} onChange={e => u('monthly_rate', e.target.value)} className="input-field" /></div></div><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={form.negotiable} onChange={e => u('negotiable', e.target.checked)} className="w-3.5 h-3.5 border-border rounded text-ink" /><span className="text-[13px] text-secondary">Open to negotiation for longer bookings</span></label><div className="flex gap-3"><button type="button" onClick={() => setStep(2)} className="btn-secondary flex-1">Back</button><button type="button" onClick={() => setStep(4)} className="btn-primary flex-1">Continue</button></div></div>}
        ...
        {step === 5 && <div className="space-y-5"><p className="eyebrow mb-4">Step 5 - Review & Submit</p><div className="bg-white border border-border rounded-xl p-6 space-y-3"><h3 className="text-[17px] font-medium text-ink">{form.title}</h3>{form.description && <p className="text-[13px] text-secondary">{form.description}</p>}{form.services_offered.length > 0 && <div className="flex flex-wrap gap-1">{form.services_offered.map(s => <span key={s} className="text-[10px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">{s}</span>)}</div>}{form.product_houses.length > 0 && <div className="flex flex-wrap gap-1">{form.product_houses.map(b => <span key={b} className="text-[10px] bg-[#FDF6EC] text-accent border border-accent/20 px-2 py-0.5 rounded-full">{b}</span>)}</div>}<div className="flex gap-3 text-[13px]">{form.day_rate && <span className="bg-surface px-2.5 py-1 rounded-lg font-medium text-ink">£{form.day_rate}/day</span>}{(form.weekly_rate || autoWeekly) && <span className="bg-surface px-2.5 py-1 rounded-lg font-medium text-ink">£{form.weekly_rate || autoWeekly}/week</span>}</div><p className="text-[12px] text-muted">Travel: {form.travel_availability} {form.availability_start ? `· From ${form.availability_start}` : ''} {form.preferred_duration ? `· ${form.preferred_duration}` : ''}</p></div><div className="bg-surface p-4 rounded-lg text-[13px] text-muted">Your paid membership is active. Your listing will be reviewed by the Spa Platform team before going live.</div><div className="flex gap-3"><button type="button" onClick={() => setStep(4)} className="btn-secondary flex-1">Back</button><button type="button" onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 disabled:opacity-50">{loading ? 'Submitting...' : 'Submit for Approval'}</button></div></div>}
      </div>
    </div>
  )
}
