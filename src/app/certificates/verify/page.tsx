...
// certificate code and sees exactly what Talent House Academy issued - or that no
  ...
  return (
    <div className="min-h-screen bg-[#ede8df]">
      ...
      <div className="mx-auto max-w-2xl px-6 pt-32 pb-24">
        <p className="text-[11px] uppercase tracking-[0.18em] text-[#222321] font-semibold mb-2">Talent House Academy</p>
        <h1 className="font-serif text-[32px] font-semibold text-[#222321] mb-2">Verify a certificate</h1>
        <p className="text-[13.5px] text-[#57544c] leading-relaxed mb-6">Every Talent House Academy certificate carries a unique verification code. Enter it below to confirm the certificate is genuine and see exactly what was awarded.</p>

        <div className="flex gap-2 mb-8">
          <input value={code} onChange={e => setCode(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') verify(code) }}
            placeholder="e.g. Talent House-XXXX-XXXX" aria-label="Certificate verification code" className="input-field flex-1 text-[14px] font-mono" />
        ...
        {state === 'done' && result && (
          <div role="status" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
            <p className="inline-flex items-center gap-2 text-[14px] font-semibold text-emerald-900 mb-3"><BadgeCheck size={18} /> Genuine Talent House Academy certificate</p>
            <div className="space-y-1.5 text-[13.5px] text-emerald-900">
              ...
              <p className="pt-1 text-[12px] text-emerald-700">Issued by Talent House Academy, Talent House Collective.</p>
            </div>
          </div>
        )}
        ...
        {state === 'notfound' && (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6">
            ...
            <p className="text-[13px] text-red-700 leading-relaxed">That code does not match any certificate issued by Talent House Academy. Check the code carefully - if it still fails, the certificate is not genuine and we would like to know: contact us through the Contact page.</p>
          </div>
        )}
      </div>
      <Footer />
...
export default function VerifyCertificatePage() {
  return <Suspense fallback={<div className="min-h-screen bg-[#ede8df]" />}><VerifyInner /></Suspense>
}
