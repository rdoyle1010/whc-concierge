      ...
      <main className="max-w-7xl mx-auto px-6 py-14 md:py-16 space-y-16">
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mt-7">
            <div className="border border-[#e5e5e5] rounded-[18px] p-5"><BedDouble size={19} className="text-[#555555]"/><p className="text-[11px] text-[#7d8990] mt-4">Treatment rooms</p><p className="text-[22px] text-[#10283b] mt-1">{property.num_treatment_rooms || 'Not supplied'}</p></div>
            <div className="border border-[#e5e5e5] rounded-[18px] p-5"><Users size={19} className="text-[#555555]"/><p className="text-[11px] text-[#7d8990] mt-4">Team size</p><p className="text-[22px] text-[#10283b] mt-1">{property.team_size || 'Not supplied'}</p></div>
            <div className="border border-[#e5e5e5] rounded-[18px] p-5"><Sparkles size={19} className="text-[#555555]"/><p className="text-[11px] text-[#7d8990] mt-4">Services listed</p><p className="text-[22px] text-[#10283b] mt-1">{services.length || '-'}</p></div>
            <div className="border border-[#e5e5e5] rounded-[18px] p-5"><Building2 size={19} className="text-[#555555]"/><p className="text-[11px] text-[#7d8990] mt-4">Property type</p><p className="text-[18px] text-[#10283b] mt-1">{property.property_type || property.company_type || 'Not supplied'}</p></div>
          </div>
        ...
        <section id="staff-reviews">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[.17em] font-semibold text-[#555555]">Verified WHC professionals</p><h2 className="text-[32px] md:text-[38px] mt-2">What is it actually like to work here?</h2></div>{whcReviewScore && <div className="text-left md:text-right"><p className="text-[34px] text-[#10283b] leading-none">{whcReviewScore.toFixed(1)} / 5</p><p className="text-[11px] text-[#7d8990] mt-2">{whcReviewCount} verified review{whcReviewCount === 1 ? '' : 's'}</p></div>}</div>
          ...
          {staffReviews.length > 0 ? <div className="grid md:grid-cols-2 gap-5 mt-7">{staffReviews.slice(0, 8).map((review: any) => <article key={review.id} className="rounded-[20px] border border-[#e5e5e5] p-6 bg-white"><div className="flex items-start justify-between gap-4"><div><div className="flex gap-1">{[1,2,3,4,5].map(n => <Star key={n} size={14} className={n <= review.rating ? 'text-[#0b2f4d]' : 'text-[#cccccc]'} fill={n <= review.rating ? 'currentColor' : 'none'}/>)}</div><p className="text-[14px] font-semibold text-[#10283b] mt-3">{review.reviewer_name}</p>{review.reviewer_role && <p className="text-[11px] text-[#7d8990]">{review.reviewer_role}</p>}</div><span className="inline-flex items-center gap-1 rounded-full bg-[#eef4f1] text-[#355d49] px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[.08em]"><BadgeCheck size={11}/>Verified</span></div>{review.comment && <p className="mt-5 text-[13px] leading-6 text-[#4d4d4d]">“{review.comment}”</p>}<div className="mt-5 pt-4 border-t border-[#e8ecee] flex items-center justify-between gap-3"><p className="text-[10px] text-[#7d8990]">{review.source}</p>{review.created_at && <p className="text-[10px] text-[#9aa3a8]">{new Date(review.created_at).toLocaleDateString('en-GB', { month: 'short', year: 'numeric' })}</p>}</div></article>)}</div> : <div className="mt-7 rounded-[20px] border border-[#e5e5e5] bg-[#f7f7f7] p-8 text-center"><MessageSquareQuote size={24} className="mx-auto text-[#555555]"/><p className="text-[16px] text-[#10283b] mt-3">No verified WHC staff reviews yet</p><p className="text-[12px] text-[#555555] mt-1">The score will build as professionals complete WHC shifts and placements here.</p></div>}
        </section>
...
function TravelCard({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return <div className="border border-[#e5e5e5] rounded-[18px] p-5"><Icon size={19} className="text-[#555555]"/><p className="text-[10px] uppercase tracking-[.12em] text-[#7d8990] mt-4">{label}</p><p className="text-[14px] leading-6 text-[#10283b] mt-1">{value}</p></div>
}
