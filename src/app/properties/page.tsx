    ...
    <section className="pt-[76px] bg-white border-b border-border overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-10 md:py-14 grid lg:grid-cols-[.9fr_1.1fr] gap-9 items-center">
        <div className="py-6 lg:py-12">
          <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[#5a6a76] mb-4">{cms.hero.eyebrow}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{properties.map((p:any)=>{
          ...
          return <Link key={p.id} href={`/properties/${p.id}`} aria-label={`Explore property: ${name}`} className="group flex flex-col overflow-hidden border border-border bg-white transition-colors hover:border-[#6b7580]">
            <div className="relative aspect-[16/10] overflow-hidden bg-[#e8eef4]">
              ...
              {p.id===featuredSlotId&&<span className="absolute left-5 top-5 bg-[#0b2f4d] px-2 py-1 text-[9px] font-semibold uppercase tracking-[.12em] text-white">Featured</span>}
            ...
            <div className="flex flex-1 flex-col px-5 py-4">
              ...
              <span className="mt-auto pt-4 inline-flex text-[12px] font-semibold text-[#0b2f4d]">Explore property →</span>
            </div>
          </Link>})}</div>
      </>}
    </div></section>
