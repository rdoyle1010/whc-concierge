...
// Posts arrive from the server, already in the HTML.
//
// This list used to start empty and fill from a browser effect, so the served
// document for /blog was a heading and nothing else: no post title, no excerpt,
// and not one crawlable link to any article. Every post was written to be found
// and none of them could be.
//
// The effect stays, because an author publishing a post should see it without
            ...
            {categories.map((cat) => (
              <button key={cat} onClick={() => { setFilter(cat); setPage(1) }}
                className={`px-4 py-2 rounded-xl text-[12px] font-semibold whitespace-nowrap transition-colors ${
                  filter === cat ? 'bg-[#222321] text-white' : 'bg-[#ede8df] text-secondary hover:text-[#222321]'
                }`}>{cat}</button>
            ))}
          </div>
        </section>
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
          {loading ? (
            ...
            <div className="dashboard-card text-center py-20">
              <h3 className="text-[22px] font-semibold text-[#222321] mb-2">Coming Soon</h3>
              ...
              {heroPost && (
                <Link href={`/blog/${heroPost.slug}`} className="dashboard-card block mb-10 group !p-0 overflow-hidden">
                  <div className="grid grid-cols-1 lg:grid-cols-2 items-stretch">
                    <div className="aspect-[4/3] lg:aspect-auto min-h-[340px] bg-[#ede8df] overflow-hidden">
                    ...
                    <div className="p-7 lg:p-10 flex flex-col justify-center">
                      ...
                      <h2 className="text-[30px] lg:text-[38px] font-semibold text-[#222321] leading-[1.1] tracking-[-0.035em] mb-4">{heroPost.title}</h2>
                      ...
                      <span className="mt-6 inline-flex items-center gap-1 text-[12px] font-semibold text-[#222321]">Read article <ArrowRight size={13} /></span>
                    </div>
                  </div>
                </Link>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 stagger-children">
                  {gridPosts.map((post) => (
                    <Link key={post.id} href={`/blog/${post.slug}`} className="dashboard-card group !p-0 overflow-hidden">
                      <div className="aspect-[16/10] bg-[#ede8df] overflow-hidden">
                      ...
                      <div className="p-5">
                        <div className="flex items-center gap-3 text-[10px] text-muted mb-2">
                          {post.category && <span className="uppercase tracking-[0.12em] text-[#222321] font-semibold">{post.category}</span>}
                          <span className="flex items-center gap-1"><Clock size={10} />{readTime(post.content)} min</span>
                        </div>
                        <h3 className="text-[18px] font-semibold text-[#222321] mb-2 leading-snug">{post.title}</h3>
