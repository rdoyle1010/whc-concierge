      ...
      <section className="pt-[68px] bg-[#0b2f4d] text-white overflow-hidden">
        <div className="mx-auto max-w-[1440px] px-6 py-18 lg:px-10 lg:py-24 grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center">
          ...
          <div className="rounded-[28px] bg-white p-7 md:p-9 text-[#10283b] shadow-xl shadow-black/15">
            <p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#6f7f88]">What you leave with</p>
            ...
            <div className="space-y-5 mt-7">
              ...
              ].map(([Icon, title, text]: any) => <div key={title} className="flex gap-4"><div className="h-10 w-10 rounded-xl bg-[#eef2f4] flex items-center justify-center shrink-0"><Icon size={18} className="text-[#6f7f88]" /></div><div><p className="text-[14px] font-semibold">{title}</p><p className="text-[12px] leading-5 text-black/55 mt-1">{text}</p></div></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <main id="courses" className="mx-auto max-w-[1440px] px-6 py-12 lg:px-10 lg:py-16 bg-white">
        ...
        <section className="mb-14 grid lg:grid-cols-[.85fr_1.15fr] gap-6 items-stretch">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            ].map(([Icon, title, text]: any) => <div key={title} className="rounded-[20px] border border-[#e2e6e8] bg-white p-6"><Icon size={20} className="text-[#6f7f88]"/><h3 className="text-[16px] font-semibold text-[#10283b] mt-5">{title}</h3><p className="text-[12px] leading-6 text-[#687681] mt-2">{text}</p></div>)}
          </div>
        </section>
        ...
        {managementCourses.length > 0 && (
          <section className="mb-16">
            <div className="mb-7 flex flex-col justify-between gap-3 md:flex-row md:items-end">
              <div>
                <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f7f88]">Flagship leadership programmes</p>
                ...
                <article key={course.slug} className="overflow-hidden rounded-[24px] border border-[#e1e5e7] bg-white shadow-sm hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-7">
                    ...
                    <div className="mb-6 grid gap-2 sm:grid-cols-3">
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#65727c]"><CheckCircle2 size={13} className="text-[#6f7f88]" /> {course.lessons.length} applied modules</span>
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#65727c]"><CheckCircle2 size={13} className="text-[#6f7f88]" /> Real spa case studies</span>
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#65727c]"><CheckCircle2 size={13} className="text-[#6f7f88]" /> Formal assessment</span>
                    </div>
        ...
        <section>
          <div className="mb-8 max-w-3xl">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f7f88]">Professional course library</p>
        ...
        <section className="mt-14 rounded-[26px] bg-white border border-[#e2e6e8] p-8 md:p-10 grid md:grid-cols-[1fr_auto] gap-8 items-center">
          <div>
            <p className="text-[10px] uppercase tracking-[.17em] text-[#6f7f88] font-semibold">Make your learning visible</p>
          ...
          <Link href="/verify" className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#aab5bb] px-5 py-3 text-[12px] font-semibold text-[#53636f] hover:bg-[#f5f7f8]">See certificate verification <ArrowRight size={13}/></Link>
        </section>

        <div className="mt-6 flex max-w-4xl items-start gap-3 rounded-2xl border border-[#e2e6e8] bg-white p-5">
          <ShieldCheck size={18} className="mt-0.5 shrink-0 text-[#6f7f88]" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setBuying(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6" onClick={e => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-lg font-semibold tracking-tight text-[#10283b]"><GraduationCap size={17} className="text-[#6f7f88]" /> {buying.title}</h2>
