                ...
                <article key={course.slug} className="overflow-hidden border border-[#dcd4c8] bg-white">
                  ...
                  <div className="p-7">
                    ...
                    <div className="mb-6 grid gap-2 sm:grid-cols-3">
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#57544c]"><CheckCircle2 size={13} className="text-[#57544c]" /> {course.lessons.length} applied modules</span>
                ...
                {standardCourses.filter(c => c.category === cat).map(course => (
                  <article key={course.slug} className="flex flex-col overflow-hidden border border-[#dcd4c8] bg-white">
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      <p className="text-[11px] text-[#6e6a60]">{course.lessons.length} module{course.lessons.length === 1 ? '' : 's'} · ~{course.minutes} min · {courseMeta(course.slug).cpdHours} CPD hour{courseMeta(course.slug).cpdHours === 1 ? '' : 's'} · £{(publicCoursePrice(course) / 100).toFixed(0)}</p>
                      {(demand[course.slug] || 0) > 0 && (
