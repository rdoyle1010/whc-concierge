    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="mx-auto max-w-[1440px] px-6 py-16 lg:px-10 lg:py-24 grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center">
          ...
          <div className="rounded-[28px] border border-[#dddddd] bg-white p-7 md:p-9 text-[#1c1c1c] shadow-[0_20px_60px_rgba(28,28,28,.06)]">
            ...
            <div className="space-y-5 mt-7">
              ...
              ].map(([Icon, title, text]: any) => <div key={title} className="flex gap-4"><div className="h-10 w-10 rounded-xl bg-[#e7e7e7] flex items-center justify-center shrink-0"><Icon size={18} className="text-[#555555]" /></div><div><p className="text-[14px] font-semibold">{title}</p><p className="text-[12px] leading-5 text-black/55 mt-1">{text}</p></div></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white border-b border-[#dddddd]">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-8 grid grid-cols-2 md:grid-cols-4 gap-5">
          ...
          ].map(([title, text], index) => <div key={title} className="flex items-start gap-3"><div className="h-8 w-8 rounded-full bg-[#1c1c1c] text-white flex items-center justify-center text-[11px] font-semibold shrink-0">{index + 1}</div><div><p className="text-[12px] font-semibold text-[#1c1c1c]">{title}</p><p className="text-[11px] text-[#6b6b6b] mt-0.5">{text}</p></div></div>)}
        </div>
      </section>
      ...
      <main id="main-content" className="mx-auto max-w-[1440px] px-6 py-12 lg:px-10 lg:py-16 bg-white">
        ...
        <section className="mb-14 grid lg:grid-cols-[.85fr_1.15fr] gap-6 items-stretch">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            ].map(([Icon, title, text]: any) => <div key={title} className="rounded-[20px] border border-[#dddddd] bg-white p-6"><Icon size={20} className="text-[#555555]"/><h3 className="text-[16px] font-semibold text-[#1c1c1c] mt-5">{title}</h3><p className="text-[12px] leading-6 text-[#555555] mt-2">{text}</p></div>)}
          </div>
        </section>
          ...
          <section className="mb-16">
            <div className="mb-7 flex flex-col justify-between gap-3 md:flex-row md:items-end">
              <div>
                ...
                <h2 className="text-[31px] font-semibold tracking-[-0.035em] text-[#1c1c1c] md:text-[42px]">Move from experienced practitioner to confident leader.</h2>
                <p className="mt-3 max-w-3xl text-[13px] leading-6 text-[#555555]">Applied programmes built around the work luxury spa managers and directors actually do: people, payroll, KPIs, profitability, forecasting, marketing and strategy.</p>
              </div>
            </div>
            <div className="grid gap-6 lg:grid-cols-2">
              {managementCourses.map((course, index) => (
                <article key={course.slug} className="overflow-hidden border border-[#dddddd] bg-white">
                  ...
                  <div className="p-7">
                    ...
                    <div className="mb-6 grid gap-2 sm:grid-cols-3">
                      ...
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#555555]"><CheckCircle2 size={13} className="text-[#555555]" /> Real spa case studies</span>
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#555555]"><CheckCircle2 size={13} className="text-[#555555]" /> Formal assessment</span>
                    </div>
                    <div className="flex items-center justify-between gap-4 border-t border-[#dddddd] pt-5">
                      <div><p className="text-[10px] uppercase tracking-[0.12em] text-[#6b6b6b]">Guest price</p><p className="text-[22px] font-semibold text-[#1c1c1c]">£{(publicCoursePrice(course) / 100).toFixed(0)}</p></div>
        ...
        <section>
          ...
          {categories.map(cat => (
            <div key={cat} className="mb-12">
              <h3 className="mb-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6b6b6b]">{cat}</h3>
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
                {standardCourses.filter(c => c.category === cat).map(course => (
                  <article key={course.slug} className="flex flex-col overflow-hidden border border-[#dddddd] bg-white">
                    <div className="relative h-44 shrink-0">
                      <img src={displayCourseImage(course)} alt="" loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover" />
                    </div>
                    <div className="flex flex-1 flex-col p-6">
                      <h3 className="mb-1 text-[19px] font-semibold leading-snug tracking-tight text-[#1c1c1c]">{course.title}</h3>
        ...
        <section className="mt-14 rounded-[26px] bg-white border border-[#dddddd] p-8 md:p-10 grid md:grid-cols-[1fr_auto] gap-8 items-center">
          <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setBuying(null)}>
          <div {...buyingDialog.panelProps} className="w-full max-w-md rounded-2xl bg-white p-6">
            <div className="mb-2 flex items-center justify-between">
              <h2 id="academy-buy-dialog-heading" className="flex items-center gap-2 text-lg font-semibold tracking-tight text-[#1c1c1c]"><GraduationCap size={17} className="text-[#555555]" /> {buying.title}</h2>
              <button type="button" onClick={() => setBuying(null)} aria-label="Close" className="p-2 -m-2 text-gray-300 hover:text-[#1c1c1c]"><X size={20} /></button>
            </div>
