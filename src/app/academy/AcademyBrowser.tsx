...
import { coursePrice, publicCoursePrice } from '@/lib/academy-pricing'
...
export default function AcademyBrowser({ initialCourses }: { initialCourses: CourseSummary[] }) {
  ...
  // The cards come from the server, already rendered into the HTML.
  //
  // This used to seed from the ACADEMY constant and then replace it with a
  // fetch of /api/academy/catalog. That meant two things: every lesson in the
  // Academy shipped in this page's JavaScript - 109KB gzipped of material
  // people pay for - and the page then downloaded 305KB more of the same to
  // draw the same cards. Neither the constant nor the fetch ever rendered a
  // lesson.
  ...
  async function submitTeamEnquiry() {
    ...
    try {
      ...
      const { error: insertError } = await (await import('@/lib/supabase/client')).createClient().from('contact_queries').insert({
  ...
  useEffect(() => {
    ...
    // Whether this visitor is already a member decides one button's wording.
    // It is not worth 57KB on first load, so the client arrives after paint.
  ...
  const displayCourseImage = (course: CourseSummary) => course.image_url || ''

  const purchaseButton = (course: CourseSummary) => isCandidate ? (
    ...
    <div className="min-h-screen bg-parchment">
      ...
      <section className="pt-[76px] bg-[#ede8df] text-ink overflow-hidden">
        <div className="mx-auto max-w-[1440px] px-6 py-16 lg:px-10 lg:py-24 grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center">
          ...
          <div className="rounded-[28px] border border-[#dcd4c8] bg-white p-7 md:p-9 text-[#222321] shadow-[0_20px_60px_rgba(28,28,28,.06)]">
            <p className="text-[10px] uppercase tracking-[.16em] font-semibold text-[#57544c]">What you leave with</p>
            ...
            <div className="space-y-5 mt-7">
              ...
              ].map(([Icon, title, text]: any) => <div key={title} className="flex gap-4"><div className="h-10 w-10 rounded-xl bg-[#e3dcd1] flex items-center justify-center shrink-0"><Icon size={18} className="text-[#57544c]" /></div><div><p className="text-[14px] font-semibold">{title}</p><p className="text-[12px] leading-5 text-black/55 mt-1">{text}</p></div></div>)}
            </div>
          </div>
        </div>
      </section>

      <section className="bg-white border-b border-[#dcd4c8]">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-8 grid grid-cols-2 md:grid-cols-4 gap-5">
          ...
          ].map(([title, text], index) => <div key={title} className="flex items-start gap-3"><div className="h-8 w-8 rounded-full bg-[#222321] text-white flex items-center justify-center text-[11px] font-semibold shrink-0">{index + 1}</div><div><p className="text-[12px] font-semibold text-[#222321]">{title}</p><p className="text-[11px] text-[#6e6a60] mt-0.5">{text}</p></div></div>)}
        </div>
      </section>
      ...
      <main id="main-content" className="mx-auto max-w-[1440px] px-6 py-12 lg:px-10 lg:py-16 bg-white">
        ...
        <section className="mb-14 grid lg:grid-cols-[.85fr_1.15fr] gap-6 items-stretch">
          <div className="rounded-[24px] bg-[#222321] text-white p-8 md:p-10">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            ].map(([Icon, title, text]: any) => <div key={title} className="rounded-[20px] border border-[#dcd4c8] bg-white p-6"><Icon size={20} className="text-[#57544c]"/><h3 className="text-[16px] font-semibold text-[#222321] mt-5">{title}</h3><p className="text-[12px] leading-6 text-[#57544c] mt-2">{text}</p></div>)}
          </div>
        </section>
        ...
        {managementCourses.length > 0 && (
          <section className="mb-16">
            <div className="mb-7 flex flex-col justify-between gap-3 md:flex-row md:items-end">
              <div>
                <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#57544c]">Flagship leadership programmes</p>
                <h2 className="text-[31px] font-semibold tracking-[-0.035em] text-[#222321] md:text-[42px]">Move from experienced practitioner to confident leader.</h2>
                <p className="mt-3 max-w-3xl text-[13px] leading-6 text-[#57544c]">Applied programmes built around the work luxury spa managers and directors actually do: people, payroll, KPIs, profitability, forecasting, marketing and strategy.</p>
              </div>
            </div>
            <div className="grid gap-6 lg:grid-cols-2">
              {managementCourses.map((course, index) => (
                <article key={course.slug} className="overflow-hidden border border-[#dcd4c8] bg-white">
                  ...
                  <div className="p-7">
                    <p className="mb-5 text-[13px] leading-6 text-[#57544c]">{course.tagline}</p>
                    {(demand[course.slug] || 0) > 0 && (
                      <p className="mb-5 border-t border-[#dcd4c8] pt-3 text-[13px] font-serif font-semibold text-[#222321]">Asked for in {demand[course.slug]} live Talent House role{demand[course.slug] === 1 ? '' : 's'} right now</p>
                    )}
                    <div className="mb-6 grid gap-2 sm:grid-cols-3">
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#57544c]"><CheckCircle2 size={13} className="text-[#57544c]" /> {course.lesson_count} applied modules</span>
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#57544c]"><CheckCircle2 size={13} className="text-[#57544c]" /> Real spa case studies</span>
                      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#57544c]"><CheckCircle2 size={13} className="text-[#57544c]" /> Formal assessment</span>
                    </div>
                    <div className="flex items-center justify-between gap-4 border-t border-[#dcd4c8] pt-5">
                      <div><p className="text-[10px] uppercase tracking-[0.12em] text-[#6e6a60]">Guest price</p><p className="text-[22px] font-semibold text-[#222321]">£{(publicCoursePrice(course) / 100).toFixed(0)}</p></div>
        ...
        <section>
          <div className="mb-8 max-w-3xl">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#57544c]">Professional course library</p>
            <h2 className="text-[30px] font-semibold tracking-[-0.035em] text-[#222321] md:text-[40px]">Build the skills luxury spas actually use.</h2>
            <p className="text-[13px] leading-6 text-[#57544c] mt-3">Choose focused learning you can complete around work, then add the achievement to your Talent House professional profile.</p>
          </div>

          {categories.map(cat => (
            <div key={cat} className="mb-12">
              <h3 className="mb-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6e6a60]">{cat}</h3>
                ...
                {standardCourses.filter(c => c.category === cat).map(course => (
                  <article key={course.slug} className="flex flex-col overflow-hidden border border-[#dcd4c8] bg-white">
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      {(demand[course.slug] || 0) > 0 && (
                        <p className="mt-2 border-t border-[#dcd4c8] pt-2 text-[12px] font-serif font-semibold text-[#222321]">Asked for in {demand[course.slug]} live Talent House role{demand[course.slug] === 1 ? '' : 's'} right now</p>
                      )}
                      <div className="mt-auto border-t border-[#dcd4c8] pt-4">
        ...
        <section className="mt-14 rounded-[26px] bg-white border border-[#dcd4c8] p-8 md:p-10 grid md:grid-cols-[1fr_auto] gap-8 items-center">
          <div>
            <p className="text-[10px] uppercase tracking-[.17em] text-[#57544c] font-semibold">Make your learning visible</p>
            <h2 className="text-[29px] md:text-[36px] font-semibold tracking-[-.035em] text-[#222321] mt-2">Don't just say you're developing. Show it.</h2>
            <p className="text-[13px] leading-6 text-[#57544c] max-w-3xl mt-3">Every Talent House certificate carries a unique verification code. For Talent members, completed Academy courses can also appear as profile badges employers can see.</p>
          </div>
          <Link href="/verify" className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#6e6a60] px-5 py-3 text-[12px] font-semibold text-[#57544c] hover:bg-[#ede8df]">See certificate verification <ArrowRight size={13}/></Link>
        </section>

        <section className="mt-14 border border-border bg-[#ede8df] p-7 md:p-9">
        ...
        <div className="mt-6 flex max-w-4xl items-start gap-3 border border-[#dcd4c8] bg-white p-5">
          <ShieldCheck size={18} className="mt-0.5 shrink-0 text-[#57544c]" />
          <p className="text-[11px] leading-5 text-[#57544c]">Talent House Academy certificates evidence course completion and assessment. They are professional-development records and are not a substitute for regulated qualifications, licences or insurance where those are required.</p>
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setBuying(null)}>
          <div {...buyingDialog.panelProps} className="w-full max-w-md rounded-2xl bg-white p-6">
            <div className="mb-2 flex items-center justify-between">
              <h2 id="academy-buy-dialog-heading" className="flex items-center gap-2 text-lg font-semibold tracking-tight text-[#222321]"><GraduationCap size={17} className="text-[#57544c]" /> {buying.title}</h2>
              <button type="button" onClick={() => setBuying(null)} aria-label="Close" className="p-2 -m-2 text-gray-300 hover:text-[#222321]"><X size={20} /></button>
            </div>
            <p className="mb-4 text-[12px] leading-5 text-[#57544c]">£{(buying.price / 100).toFixed(0)} one-off. After payment your access link arrives by email. Your certificate is issued when you complete the learning and pass the assessment.</p>
            ...
            <p className="mb-3 text-[12px] leading-5 text-[#57544c]">This address becomes your Talent House account, and it is where the course lives. Use the one you will sign in with.</p>
