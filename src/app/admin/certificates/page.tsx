    ...
    return (
      <div className="dashboard-card">
        ...
        {row.status === 'submitted' ? (
          <div>
            {!assist[row.id] && (
              <button type="button" disabled={assistBusy === row.id} onClick={() => runAssist(row.id)} className="mb-2 inline-flex items-center gap-1.5 rounded-lg border border-[#dddddd] bg-[#f1f1f1] px-3 py-1.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1]">
                <Sparkles size={13} /> {assistBusy === row.id ? 'Reviewing...' : 'AI review'}
              </button>
            )}
            {assist[row.id] && (
              <div className="mb-3 rounded-lg border border-[#dddddd] bg-[#f1f1f1] p-3">
                <p className="text-[10.5px] font-semibold uppercase tracking-[0.12em] text-[#1c1c1c] mb-1.5 inline-flex items-center gap-1"><Sparkles size={11} /> AI assessment{assist[row.id].recognition ? ` · ${String(assist[row.id].recognition).replace('_', ' ')}` : ''}</p>
                ...
                {assist[row.id].drafts && (
                  <div className="flex flex-wrap gap-1.5 pt-1 border-t border-[#dddddd]">
                    ...
                    {(['verified', 'more_info', 'rejected'] as const).map(kind => assist[row.id].drafts[kind] && (
                      <button key={kind} type="button" onClick={() => setNotes(current => ({ ...current, [row.id]: assist[row.id].drafts[kind] }))} className="rounded-full border border-[#dddddd] bg-white px-2.5 py-1 text-[11px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1]">
          ...
          <p className="text-[12px] text-secondary bg-[#f1f1f1] rounded px-3 py-2"><ShieldQuestion size={12} className="inline mr-1 -mt-0.5" /> {row.review_note}</p>
        ) : null}
      </div>
    )
  }
