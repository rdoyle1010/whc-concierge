...
export default function AdminBlogPage() {
  ...
  return <DashboardShell role="admin" userName="Admin">
    ...
    <div className="grid gap-3 sm:grid-cols-3 mb-7">
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Total</p><p className="text-[24px] font-semibold text-[#222321]">{posts.length}</p></div>
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Published</p><p className="text-[24px] font-semibold text-[#222321]">{publishedCount}</p></div>
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Drafts</p><p className="text-[24px] font-semibold text-[#222321]">{draftCount}</p></div>
    </div>

    {pageError && <div className="mb-5 rounded-xl bg-red-50 px-4 py-3 text-[13px] text-red-600">{pageError}</div>}

    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#161814]/70 p-4" onClick={() => setShowForm(false)}>
      <div {...formDialog.panelProps} className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-7 md:p-9">
        <div className="mb-6 flex items-center justify-between"><div><p className="dashboard-eyebrow">Journal editor</p><h2 id="admin-blog-editor-heading" className="text-[24px] font-semibold text-[#222321]">{editing ? 'Edit article' : 'Write new article'}</h2></div><button onClick={() => setShowForm(false)} className="p-2 text-[#57544c]"><X size={18}/></button></div>
        <div className="space-y-5">
          ...
          <div><label className="dashboard-eyebrow block mb-1.5">Featured image</label><div className="grid md:grid-cols-[240px_1fr] gap-4 rounded-2xl border border-[#dcd4c8] p-4 bg-[#ede8df]">
            <div className="aspect-[16/10] overflow-hidden rounded-xl bg-white border border-[#dcd4c8]">{form.image_url ? <img loading="lazy" decoding="async" src={form.image_url} alt="Article preview" className="w-full h-full object-cover"/> : <div className="h-full flex items-center justify-center text-[#6e6a60]"><ImageIcon size={30}/></div>}</div>
            <div className="flex flex-col justify-center gap-3"><p className="text-[12px] leading-5 text-[#57544c]">Upload the article image from your computer or phone. It will be stored in Talent House site storage and used on the Journal card and article page.</p><div className="flex flex-wrap gap-2"><label className="btn-secondary w-fit cursor-pointer inline-flex items-center gap-2"><Upload size={14}/>{uploading ? 'Uploading…' : form.image_url ? 'Replace image' : 'Upload image'}<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={uploading} onChange={e => { const file=e.target.files?.[0]; if(file) uploadImage(file); e.target.value='' }}/></label>{form.image_url ? <button type="button" onClick={() => setForm({ ...form, image_url: '' })} className="btn-secondary">Remove image</button> : null}</div><p className="text-[11px] text-[#6e6a60]">JPEG, PNG or WebP. Maximum 10 MB.</p></div>
          </div></div>
          ...
          <div className="rounded-xl bg-[#e3dcd1] p-4 flex gap-3"><Share2 size={18} className="text-[#222321] shrink-0 mt-0.5"/><div><p className="text-[12px] font-semibold text-[#222321]">Social sharing is automatic</p><p className="text-[11px] text-[#57544c] mt-1">Published articles get LinkedIn, Facebook, WhatsApp, Email and Copy Link buttons on the article page.</p></div></div>

          <div className="flex flex-wrap items-center justify-between gap-4"><label className="flex items-center gap-3"><input type="checkbox" checked={form.status === 'published'} onChange={e => setForm({ ...form, status: e.target.checked ? 'published' : 'draft' })}/><span className="text-[13px] font-semibold text-[#222321]">Publish article</span></label><div className="flex gap-2"><button onClick={() => setShowForm(false)} className="btn-secondary">Cancel</button><button onClick={handleSave} disabled={saving || uploading || !form.title || !form.content} className="btn-primary disabled:opacity-40">{saving ? 'Saving…' : editing ? 'Update article' : 'Create article'}</button></div></div>
          {formError && <div className="rounded-xl bg-red-50 px-4 py-3 text-[12px] text-red-600">{formError}</div>}
        </div>
      </div>
    </div>}

    {showDelete && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#161814]/70 p-4" onClick={() => setShowDelete(null)}><div {...deleteDialog.panelProps} className="w-full max-w-sm rounded-2xl bg-white p-6"><h3 id="admin-blog-delete-heading" className="text-[18px] font-semibold text-[#222321]">Delete this article?</h3><p className="text-[12px] text-[#57544c] mt-2">This cannot be undone.</p><div className="flex gap-2 mt-6"><button className="btn-secondary flex-1" onClick={()=>setShowDelete(null)}>Cancel</button><button className="flex-1 rounded-lg bg-red-600 px-4 py-2 text-white text-[12px] font-semibold" onClick={confirmDelete}>Delete</button></div></div></div>}

    {loading ? <div className="h-64 flex items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#57544c] border-t-transparent"/></div> : posts.length === 0 ? <div className="dashboard-card text-center py-16"><FileText size={38} className="mx-auto text-[#6e6a60]"/><p className="mt-3 text-[15px] font-semibold text-[#222321]">No articles yet</p></div> : <>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{paginated.map(post => <article key={post.id} className="overflow-hidden rounded-2xl border border-[#dcd4c8] bg-white">
        <div className="aspect-[16/9] bg-[#e3dcd1] overflow-hidden">{post.image_url ? <img decoding="async" src={post.image_url} alt={post.title} className="w-full h-full object-cover" loading="lazy"/> : <div className="h-full flex items-center justify-center text-[#6e6a60]"><ImageIcon size={28}/></div>}</div>
        <div className="p-5"><div className="flex items-center justify-between gap-2"><span className="text-[9px] uppercase tracking-[.14em] font-semibold text-[#57544c]">{post.category || 'Journal'}</span><span className={`text-[10px] rounded-full px-2 py-1 ${post.status==='published'?'bg-emerald-50 text-emerald-700':'bg-[#e3dcd1] text-[#57544c]'}`}>{post.status}</span></div><h3 className="mt-3 text-[17px] font-semibold leading-snug text-[#222321]">{post.title}</h3><p className="mt-2 line-clamp-2 text-[12px] leading-5 text-[#57544c]">{post.excerpt || post.content?.slice(0,120)}</p><div className="mt-5 flex items-center justify-between"><a href={`/blog/${post.slug}`} target="_blank" className="text-[11px] font-semibold text-[#222321]">View article →</a><div className="flex gap-1"><button title="Edit" onClick={()=>handleEdit(post)} className="p-2 text-[#57544c] hover:text-[#222321]"><Edit2 size={14}/></button><button title={post.status==='published'?'Unpublish':'Publish'} onClick={()=>togglePublish(post)} className="p-2 text-[#57544c] hover:text-[#222321]">{post.status==='published'?<EyeOff size={14}/>:<Eye size={14}/>}</button><button title="Delete" onClick={()=>setShowDelete(post.id)} className="p-2 text-[#57544c] hover:text-red-600"><Trash2 size={14}/></button></div></div></div>
      </article>)}</div>
