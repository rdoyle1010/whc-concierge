...
export default function AdminBlogPage() {
  ...
  return <DashboardShell role="admin" userName="Admin">
    ...
    <div className="grid gap-3 sm:grid-cols-3 mb-7">
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Total</p><p className="text-[24px] font-semibold text-[#1a1a1a]">{posts.length}</p></div>
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Published</p><p className="text-[24px] font-semibold text-[#1a1a1a]">{publishedCount}</p></div>
      <div className="dashboard-card !py-4"><p className="dashboard-eyebrow">Drafts</p><p className="text-[24px] font-semibold text-[#1a1a1a]">{draftCount}</p></div>
    </div>
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowForm(false)}>
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-7 md:p-9" onClick={e => e.stopPropagation()}>
        <div className="mb-6 flex items-center justify-between"><div><p className="dashboard-eyebrow">Journal editor</p><h2 className="text-[24px] font-semibold text-[#1a1a1a]">{editing ? 'Edit article' : 'Write new article'}</h2></div><button onClick={() => setShowForm(false)} className="p-2 text-[#555555]"><X size={18}/></button></div>
    ...
    {loading ? <div className="h-64 flex items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#555555] border-t-transparent"/></div> : posts.length === 0 ? <div className="dashboard-card text-center py-16"><FileText size={38} className="mx-auto text-[#8a979f]"/><p className="mt-3 text-[15px] font-semibold text-[#1a1a1a]">No articles yet</p></div> : <>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{paginated.map(post => <article key={post.id} className="overflow-hidden rounded-2xl border border-[#e5e5e5] bg-white">
        ...
        <div className="p-5"><div className="flex items-center justify-between gap-2"><span className="text-[9px] uppercase tracking-[.14em] font-semibold text-[#555555]">{post.category || 'Journal'}</span><span className={`text-[10px] rounded-full px-2 py-1 ${post.status==='published'?'bg-emerald-50 text-emerald-700':'bg-[#eef2f4] text-[#555555]'}`}>{post.status}</span></div><h3 className="mt-3 text-[17px] font-semibold leading-snug text-[#1a1a1a]">{post.title}</h3><p className="mt-2 line-clamp-2 text-[12px] leading-5 text-[#555555]">{post.excerpt || post.content?.slice(0,120)}</p><div className="mt-5 flex items-center justify-between"><a href={`/blog/${post.slug}`} target="_blank" className="text-[11px] font-semibold text-[#111111]">View article →</a><div className="flex gap-1"><button title="Edit" onClick={()=>handleEdit(post)} className="p-2 text-[#555555] hover:text-[#111111]"><Edit2 size={14}/></button><button title={post.status==='published'?'Unpublish':'Publish'} onClick={()=>togglePublish(post)} className="p-2 text-[#555555] hover:text-[#111111]">{post.status==='published'?<EyeOff size={14}/>:<Eye size={14}/>}</button><button title="Delete" onClick={()=>setShowDelete(post.id)} className="p-2 text-[#555555] hover:text-red-600"><Trash2 size={14}/></button></div></div></div>
      </article>)}</div>
