...
export default function AdminAgencyCasesPage() {
  ...
  async function resolve() {
    ...
    else { setNotice(j.awaitingPayment ? 'Resolution saved. The employer must now pay the approved additional amount before the case closes.' : 'Case resolved and payout controls updated.'); setSelected(null); await load() }
    setBusy(false)
  }
  ...
  return <DashboardShell role="admin">
    ...
    {loading ? <div className="skeleton h-72 rounded-xl"/> : !cases.length ? <div className="dashboard-card text-sm text-gray-500">No Agency cases have been raised.</div> : <div className="space-y-4">{cases.map(row => <div key={row.id} className="dashboard-card"><div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4"><div><div className="flex flex-wrap gap-2 items-center"><span className="text-[10px] uppercase tracking-wider text-accent">Case {row.id.slice(0,8).toUpperCase()}</span><span className="text-xs rounded-full bg-gray-100 px-2.5 py-1">{label(row.status)}</span></div><h2 className="font-medium text-ink mt-2">{row.candidate_name} · {row.employer_name}</h2><p className="text-xs text-gray-500 mt-1">Shift {row.booking?.shift_date ? new Date(row.booking.shift_date).toLocaleDateString('en-GB') : '—'} · {row.booking?.hours || 8}h · £{row.booking?.rate}/hr</p><p className="text-sm mt-3"><strong>{label(row.issue_type)}:</strong> {row.description}</p>{row.actual_start_time || row.actual_end_time ? <p className="text-xs mt-2">Reported actual time: {row.actual_start_time ? String(row.actual_start_time).slice(0,5) : '—'}–{row.actual_end_time ? String(row.actual_end_time).slice(0,5) : '—'}</p> : null}{row.requested_adjustment_type !== 'none' && <p className="text-sm mt-2"><strong>Requested:</strong> {label(row.requested_adjustment_type)}{row.requested_amount ? ` £${row.requested_amount}` : ''}{row.requested_reason ? ` — ${row.requested_reason}` : ''}</p>}{row.counterparty_response && <div className="bg-gray-50 p-3 mt-3 text-sm"><strong>Other party response</strong><p className="mt-1 whitespace-pre-wrap">{row.counterparty_response}</p></div>}{row.resolution && <div className="bg-green-50 p-3 mt-3 text-sm"><strong>Resolution</strong><p className="mt-1 whitespace-pre-wrap">{row.resolution}</p></div>}</div>{['awaiting_response','under_review','open'].includes(row.status) && <button onClick={() => choose(row)} className="btn-primary text-xs">Review & resolve</button>}{row.status === 'awaiting_payment' && <span className="text-xs font-semibold text-amber-700">Awaiting employer payment £{Number(row.approved_extra_amount || 0) + Math.ceil(Number(row.approved_extra_amount || 0)*.10)}</span>}</div></div>)}</div>}

    {selected && <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}><div className="bg-white max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 rounded-2xl" onClick={e => e.stopPropagation()}><h2 className="font-serif text-2xl text-ink">Resolve Agency case</h2><p className="text-xs text-gray-500 mt-1">A refund is returned through Stripe. An approved additional professional payment creates a separate employer checkout with the normal 10% WHC fee.</p><div className="space-y-4 mt-5"><label className="block text-xs font-medium">Resolution<textarea value={form.resolution} onChange={e => setForm(v => ({...v,resolution:e.target.value}))} className="input-field min-h-24 mt-1" placeholder="State the decision and why."/></label><label className="block text-xs font-medium">Internal admin notes<textarea value={form.adminNotes} onChange={e => setForm(v => ({...v,adminNotes:e.target.value}))} className="input-field min-h-20 mt-1"/></label><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><label className="text-xs font-medium">Refund to employer £<input type="number" min="0" step="0.01" value={form.refundAmount} onChange={e => setForm(v=>({...v,refundAmount:e.target.value}))} className="input-field mt-1"/></label><label className="text-xs font-medium">Extra professional pay £<input type="number" min="0" step="0.01" value={form.extraAmount} onChange={e => { const extra=e.target.value; const base=Number(selected.booking?.payout_amount || ((selected.booking?.rate||0)*(selected.booking?.hours||8))); setForm(v=>({...v,extraAmount:extra,adjustedPayoutAmount:String(base+Number(extra||0))})) }} className="input-field mt-1"/></label><label className="text-xs font-medium">Final professional payout £<input type="number" min="0" step="0.01" value={form.adjustedPayoutAmount} onChange={e => setForm(v=>({...v,adjustedPayoutAmount:e.target.value}))} className="input-field mt-1"/></label></div><div className="flex justify-end gap-2"><button onClick={()=>setSelected(null)} className="btn-secondary">Cancel</button><button disabled={busy} onClick={resolve} className="btn-primary">Save resolution</button></div></div></div></div>}
  </DashboardShell>
}
