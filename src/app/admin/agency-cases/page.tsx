...
export default function AdminAgencyCasesPage() {
  ...
  return <DashboardShell role="admin">
    ...
    {loading ? <div className="skeleton h-72 rounded-xl"/> : !cases.length ? <div className="dashboard-card text-sm text-secondary">No Agency cases have been raised.</div> : <div className="space-y-4">{cases.map(row => <div key={row.id} className="dashboard-card"><div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4"><div><div className="flex flex-wrap gap-2 items-center"><span className="text-[10px] uppercase tracking-wider text-accent">Case {row.id.slice(0,8).toUpperCase()}</span><span className={`text-xs rounded-full px-2.5 py-1 ${row.status === 'awaiting_agreement' ? 'bg-[#e9e4dd] text-[#1c1b1a] font-semibold' : 'bg-gray-100'}`}>{label(row.status)}</span></div><h2 className="font-medium text-ink mt-2">{row.candidate_name} · {row.employer_name}</h2><p className="text-xs text-secondary mt-1">Shift {row.booking?.shift_date ? new Date(row.booking.shift_date).toLocaleDateString('en-GB') : '-'} · {row.booking?.hours || 8}h · £{row.booking?.rate}/hr</p><p className="text-sm mt-3"><strong>{label(row.issue_type)}:</strong> {row.description}</p>{row.actual_start_time || row.actual_end_time ? <p className="text-xs mt-2">Reported actual time: {row.actual_start_time ? String(row.actual_start_time).slice(0,5) : '-'}–{row.actual_end_time ? String(row.actual_end_time).slice(0,5) : '-'}</p> : null}{row.requested_adjustment_type !== 'none' && <p className="text-sm mt-2"><strong>Requested:</strong> {label(row.requested_adjustment_type)}{row.requested_amount ? ` £${row.requested_amount}` : ''}{row.requested_reason ? ` - ${row.requested_reason}` : ''}</p>}{row.counterparty_response && <div className="bg-[#f3f0eb] p-3 mt-3 text-sm"><strong>Other party response</strong><p className="mt-1 whitespace-pre-wrap">{row.counterparty_response}</p></div>}{row.resolution && <div className="bg-green-50 p-3 mt-3 text-sm"><strong>Resolution</strong><p className="mt-1 whitespace-pre-wrap">{row.resolution}</p></div>}{row.status === 'awaiting_agreement' && row.proposed_resolution && <div className="bg-[#e9e4dd] p-3 mt-3 text-sm"><strong>Proposed resolution</strong><p className="mt-1 whitespace-pre-wrap">{row.proposed_resolution}</p><p className="mt-2 text-xs text-secondary">Refund £{Number(row.proposed_refund_amount || 0)} · Extra pay £{Number(row.proposed_extra_amount || 0)} · Final payout £{Number(row.proposed_payout_amount || 0)}</p></div>}
    {['awaiting_response','under_review','open','awaiting_agreement'].includes(row.status) && <div className="mt-4 border-t border-border pt-3">
      ...
      {(row.messages || []).length === 0 ? <p className="text-xs text-muted">No messages on this case yet.</p> : <div className="space-y-2 max-h-56 overflow-y-auto pr-1">{(row.messages || []).map((m: any) => <div key={m.id} className={`text-sm p-2.5 ${m.sender_role === 'admin' ? 'bg-[#e9e4dd]' : 'bg-surface'}`}><p className="text-[10px] uppercase tracking-wider text-muted">{label(m.sender_role || 'party')} · {m.created_at ? new Date(m.created_at).toLocaleString('en-GB') : ''}</p><p className="mt-1 whitespace-pre-wrap">{m.message}</p></div>)}</div>}
