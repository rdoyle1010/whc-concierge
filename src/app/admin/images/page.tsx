...
export default function AdminImagesPage() {
  ...
  const handleDelete = async (img: any) => {
    ...
    const pathMatch = img.url.match(/site-images\/(.+)$/)
  ...
  return (
    <DashboardShell role="admin" userName="Admin">
      ...
      <div className="dashboard-card mb-8">
        <h3 className="font-serif text-lg font-semibold mb-4">Upload to Site Images</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="input-field">
            ...
            <option>Hero</option><option>Property</option><option>Blog</option><option>Logo</option><option>Icon</option><option>Banner</option><option>Profile</option><option>Other</option>
          </select>
          <label className={`btn-primary cursor-pointer flex items-center justify-center space-x-2 text-center ${uploading ? 'opacity-50' : ''}`}>
            <Upload size={16} /><span>{uploading ? 'Uploading...' : 'Upload Image'}</span>
            <input type="file" accept="image/*" onChange={handleUpload} disabled={uploading} className="hidden" />
          </label>
        </div>
        <p className="text-xs text-gray-400">Uploads to Supabase Storage bucket: <code className="bg-gray-100 px-1.5 py-0.5 rounded">site-images</code></p>
      </div>
            ...
            <div key={img.id} className="dashboard-card p-3 group">
              ...
              <div className="flex items-center space-x-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => copyUrl(img.url)}
                  className="flex-1 py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-500 text-xs flex items-center justify-center space-x-1">
                  {copied === img.url ? <><Check size={12} /><span>Copied</span></> : <><Copy size={12} /><span>Copy URL</span></>}
                </button>
                <button onClick={() => handleDelete(img)} className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-500">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
