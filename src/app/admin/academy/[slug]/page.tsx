...
export default function AcademyCourseEditorPage() {
  ...
  if (loading) {
    return <DashboardShell role="admin" userName="Admin"><div className="flex h-64 items-center justify-center"><div className="h-8 w-8 animate-spin border-2 border-[#0b2f4d] border-t-transparent" /></div></DashboardShell>
  }
    ...
    <DashboardShell role="admin" userName="Admin">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="text-[26px] font-semibold leading-tight text-[#10283b]">{doc?.title || course.title}</h1>
          <p className="mt-2 max-w-2xl text-[13px] leading-6 text-[#5a6a76]">{course.slug}</p>
        </div>
      ...
      {notice && <div className="mb-4 border border-[#e3e7eb] bg-[#f5f6f8] px-4 py-3 text-[13px] text-[#10283b]">{notice}</div>}
      ...
      {doc && (
        <>
          ...
          <div className="mb-5 border border-[#e3e7eb] bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${source === 'custom' ? 'bg-[#0b2f4d] text-white' : 'bg-[#f5f6f8] text-[#5a6a76]'}`}>{source === 'custom' ? 'Your version is live' : 'Your version is saved, not live'}</span>
                {dirty && <span className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700">Unsaved changes</span>}
                {stats && <span className="text-[11px] text-[#5a6a76]">{stats.modules} module{stats.modules === 1 ? '' : 's'} · {stats.lessons} lesson{stats.lessons === 1 ? '' : 's'} · {stats.words.toLocaleString('en-GB')} words · {stats.minutes} min · {stats.questions} assessment question{stats.questions === 1 ? '' : 's'}</span>}
              </div>
              <div className="flex flex-wrap gap-2">
          ...
          <div className="mb-8 space-y-3">
            {doc.modules.map((module, index) => {
              ...
              return (
                <div key={index} className="border border-[#e3e7eb] bg-white">
                  ...
                  {open && (
                    <div className="border-t border-[#e3e7eb] bg-[#f5f6f8] p-4">
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <Field label="Why this matters" hint="Shown in the navy panel at the top of the module."><textarea rows={3} value={module.whyThisMatters} onChange={event => updateModule(index, current => ({ ...current, whyThisMatters: event.target.value }))} className="input-field mt-1 resize-y" /></Field>
                      ...
                      <div className="mt-5 grid grid-cols-1 gap-4 border-t border-[#e3e7eb] pt-4 md:grid-cols-2">
          ...
          <div className="mb-10 flex flex-wrap items-center gap-2 border-t border-[#e3e7eb] pt-5">
            <button type="button" onClick={save} disabled={busy === 'save' || Boolean(problem)} className="btn-primary inline-flex items-center gap-2 text-[13px] disabled:opacity-50"><Save size={14} /> {busy === 'save' ? 'Saving...' : source === 'custom' ? 'Save and publish' : 'Save your version'}</button>
