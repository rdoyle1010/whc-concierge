...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <label className="block text-[12px] text-[#555555]">
      <span className="font-medium text-[#1c1c1c]">{label}</span>
      {children}
      {hint ? <span className="mt-1 block text-[11px] leading-5 text-[#6b6b6b]">{hint}</span> : null}
    </label>
  )
}
...
function StringList({ label, hint, placeholder, values, onChange }: {
  ...
  return (
    <div>
      <p className="text-[12px] font-medium text-[#1c1c1c]">{label}</p>
      {hint ? <p className="mt-0.5 text-[11px] leading-5 text-[#6b6b6b]">{hint}</p> : null}
      <div className="mt-2 space-y-2">
        {values.map((value, index) => (
          <div key={index} className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => onChange(values.filter((_, i) => i !== index))} className="border border-[#dddddd] bg-white p-2 text-[#6b6b6b] hover:text-red-600" aria-label={`Remove ${label} ${index + 1}`}><Trash2 size={13} /></button>
          </div>
        ))}
      </div>
...
export default function AcademyCourseEditorPage() {
  ...
  if (loading) {
    return <DashboardShell role="admin" userName="Admin"><div className="flex h-64 items-center justify-center"><div className="h-8 w-8 animate-spin border-2 border-[#1c1c1c] border-t-transparent" /></div></DashboardShell>
  }

  if (!detail) {
    return (
      <DashboardShell role="admin" userName="Admin">
        <button type="button" onClick={leave} className="mb-4 inline-flex items-center gap-1 text-[13px] text-[#6b6b6b] hover:text-[#1c1c1c]"><ArrowLeft size={14} /> Academy</button>
    ...
    <DashboardShell role="admin" userName="Admin">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="text-[26px] font-semibold leading-tight text-[#1c1c1c]">{doc?.title || course.title}</h1>
          <p className="mt-2 max-w-2xl text-[13px] leading-6 text-[#555555]">{course.slug}</p>
        </div>
      ...
      {notice && <div className="mb-4 border border-[#dddddd] bg-[#f1f1f1] px-4 py-3 text-[13px] text-[#1c1c1c]">{notice}</div>}
        ...
        <div className="dashboard-card">
          ...
          <div className="my-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[['Modules', detail.platform_stats.modules], ['Lessons', detail.platform_stats.lessons], ['Words', detail.platform_stats.words.toLocaleString('en-GB')], ['Assessment questions', detail.platform_stats.questions]].map(([label, value]) => (
              <div key={String(label)} className="border border-[#dddddd] bg-[#f1f1f1] p-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6b6b6b]">{label}</p>
                <p className="mt-1 text-[18px] font-semibold text-[#1c1c1c]">{value}</p>
              </div>
            ))}
          </div>
      ...
      {doc && (
        <>
          ...
          <div className="mb-5 border border-[#dddddd] bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${source === 'custom' ? 'bg-[#1c1c1c] text-white' : 'bg-[#f1f1f1] text-[#555555]'}`}>{source === 'custom' ? 'Your version is live' : 'Your version is saved, not live'}</span>
                {dirty && <span className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700">Unsaved changes</span>}
                {stats && <span className="text-[11px] text-[#555555]">{stats.modules} module{stats.modules === 1 ? '' : 's'} · {stats.lessons} lesson{stats.lessons === 1 ? '' : 's'} · {stats.words.toLocaleString('en-GB')} words · {stats.minutes} min · {stats.questions} assessment question{stats.questions === 1 ? '' : 's'}</span>}
              </div>
              <div className="flex flex-wrap gap-2">
            ...
            {problem ? (
              <p className="mt-3 flex items-start gap-2 border-t border-[#dddddd] pt-3 text-[12px] text-red-700"><AlertTriangle size={14} className="mt-0.5 shrink-0" /> {problem}</p>
            ) : (
              <p className="mt-3 border-t border-[#dddddd] pt-3 text-[12px] text-[#6b6b6b]">Nothing saves on its own. Your writing is only stored when you press Save.</p>
            )}
          </div>

          {/* 1. The course itself */}
          <div className="dashboard-card mb-5">
            <h2 className="mb-1 text-[16px] font-semibold text-[#1c1c1c]">The course</h2>
            <p className="mb-4 text-[12px] text-[#6b6b6b]">What a learner reads before they begin, and what appears on every Academy card.</p>
          ...
          <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
            <div>
              <h2 className="text-[16px] font-semibold text-[#1c1c1c]">Modules and lessons</h2>
              <p className="text-[12px] text-[#6b6b6b]">A module is one step of the syllabus. Each module holds one or more lessons - the written content the learner reads. Adding, deleting or reordering modules changes what part-way learners still have to complete, so make those changes deliberately.</p>
            </div>
          ...
          <div className="mb-8 space-y-3">
            {doc.modules.map((module, index) => {
              ...
              return (
                <div key={index} className="border border-[#dddddd] bg-white">
                  <div className="flex flex-wrap items-center gap-2 p-4">
                    <button type="button" onClick={() => setOpenModule(open ? null : index)} className="min-w-0 flex-1 text-left">
                      <p className="text-[13px] font-semibold text-[#1c1c1c]">Module {index + 1}. {module.title || <span className="text-red-600">Untitled module</span>}</p>
                      <p className="mt-0.5 text-[11px] text-[#6b6b6b]">{module.lessons.length} lesson{module.lessons.length === 1 ? '' : 's'} · {module.knowledgeCheck.length} knowledge check{module.knowledgeCheck.length === 1 ? '' : 's'} · {module.keyTerms.length} key term{module.keyTerms.length === 1 ? '' : 's'}</p>
                    </button>
                    <div className="flex items-center gap-1">
                      <button type="button" disabled={index === 0} onClick={() => { update(current => ({ ...current, modules: move(current.modules, index, index - 1) })); setOpenModule(index - 1) }} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move module up"><ArrowUp size={13} /></button>
                      <button type="button" disabled={index === doc.modules.length - 1} onClick={() => { update(current => ({ ...current, modules: move(current.modules, index, index + 1) })); setOpenModule(index + 1) }} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move module down"><ArrowDown size={13} /></button>
                      <button type="button" onClick={() => { if (window.confirm(`Delete module ${index + 1}${module.title ? ` "${module.title}"` : ''} and every lesson in it? This is not saved until you press Save.`)) { update(current => ({ ...current, modules: current.modules.filter((_, i) => i !== index) })); setOpenModule(null) } }} className="border border-[#dddddd] p-2 text-[#6b6b6b] hover:text-red-600" aria-label="Delete module"><Trash2 size={13} /></button>
                      <button type="button" onClick={() => setOpenModule(open ? null : index)} className="btn-secondary text-[11px]">{open ? 'Close' : 'Open'}</button>
                    </div>
                  </div>

                  {open && (
                    <div className="border-t border-[#dddddd] bg-[#f1f1f1] p-4">
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <div className="mt-5 border-t border-[#dddddd] pt-4">
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <p className="text-[13px] font-semibold text-[#1c1c1c]">Lessons in this module</p>
                            <p className="text-[11px] leading-5 text-[#6b6b6b]">{LESSON_BODY_HELP}</p>
                          </div>
                        ...
                        <div className="space-y-3">
                          {module.lessons.map((lesson, lessonIndex) => (
                            <div key={lessonIndex} className="border border-[#dddddd] bg-white p-3">
                              <div className="mb-2 flex items-center gap-2">
                                <span className="text-[11px] font-semibold text-[#6b6b6b]">Lesson {lessonIndex + 1}</span>
                                ...
                                <button type="button" disabled={lessonIndex === 0} onClick={() => updateModule(index, current => ({ ...current, lessons: move(current.lessons, lessonIndex, lessonIndex - 1) }))} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move lesson up"><ArrowUp size={13} /></button>
                                <button type="button" disabled={lessonIndex === module.lessons.length - 1} onClick={() => updateModule(index, current => ({ ...current, lessons: move(current.lessons, lessonIndex, lessonIndex + 1) }))} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move lesson down"><ArrowDown size={13} /></button>
                                <button type="button" onClick={() => { if (window.confirm(`Delete lesson ${lessonIndex + 1}${lesson.title ? ` "${lesson.title}"` : ''}?`)) updateModule(index, current => ({ ...current, lessons: current.lessons.filter((_, i) => i !== lessonIndex) })) }} className="border border-[#dddddd] p-2 text-[#6b6b6b] hover:text-red-600" aria-label="Delete lesson"><Trash2 size={13} /></button>
                              </div>
                              ...
                              <p className="mt-1 text-[11px] text-[#6b6b6b]">{lesson.body.trim() ? `${lesson.body.trim().split(/\s+/).length.toLocaleString('en-GB')} words` : 'No content yet - this lesson cannot go live empty.'}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="mt-5 border-t border-[#dddddd] pt-4">
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                          <div><p className="text-[13px] font-semibold text-[#1c1c1c]">Key terms</p><p className="text-[11px] text-[#6b6b6b]">The glossary shown under the module content.</p></div>
                        ...
                        <div className="space-y-2">
                          {module.keyTerms.map((term, termIndex) => (
                            <div key={termIndex} className="grid grid-cols-1 gap-2 border border-[#dddddd] bg-white p-3 md:grid-cols-[220px_1fr_auto]">
                              ...
                              <button type="button" onClick={() => updateModule(index, current => ({ ...current, keyTerms: current.keyTerms.filter((_, i) => i !== termIndex) }))} className="border border-[#dddddd] p-2 text-[#6b6b6b] hover:text-red-600" aria-label="Delete key term"><Trash2 size={13} /></button>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="mt-5 border-t border-[#dddddd] pt-4">
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                          <div><p className="text-[13px] font-semibold text-[#1c1c1c]">Knowledge checks</p><p className="text-[11px] text-[#6b6b6b]">Practice questions inside the module. The learner sees the answer and your explanation after choosing, so these are not the final assessment.</p></div>
                        ...
                        <div className="space-y-3">
                          {module.knowledgeCheck.map((check, checkIndex) => (
                            <div key={checkIndex} className="border border-[#dddddd] bg-white p-3">
                              <div className="mb-2 flex items-center gap-2">
                                <span className="shrink-0 text-[11px] font-semibold text-[#6b6b6b]">Q{checkIndex + 1}</span>
                                ...
                                <button type="button" onClick={() => updateModule(index, current => ({ ...current, knowledgeCheck: current.knowledgeCheck.filter((_, i) => i !== checkIndex) }))} className="border border-[#dddddd] p-2 text-[#6b6b6b] hover:text-red-600" aria-label="Delete knowledge check"><Trash2 size={13} /></button>
                              </div>
          ...
          <div className="dashboard-card mb-8">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <div>
                <h2 className="text-[16px] font-semibold text-[#1c1c1c]">End-of-course assessment</h2>
                <p className="text-[12px] text-[#6b6b6b]">Marked on the server, so the answers below never reach a learner&apos;s browser. Passing issues the certificate.</p>
              </div>
            ...
            <div className="space-y-3">
              {doc.assessment.map((question, index) => (
                <div key={index} className="border border-[#dddddd] p-3">
                  <div className="mb-2 flex items-center gap-2">
                    <span className="shrink-0 text-[11px] font-semibold text-[#6b6b6b]">Q{index + 1}</span>
                    ...
                    <button type="button" disabled={index === 0} onClick={() => update(current => ({ ...current, assessment: move(current.assessment, index, index - 1) }))} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move question up"><ArrowUp size={13} /></button>
                    <button type="button" disabled={index === doc.assessment.length - 1} onClick={() => update(current => ({ ...current, assessment: move(current.assessment, index, index + 1) }))} className="border border-[#dddddd] p-2 text-[#555555] disabled:opacity-30" aria-label="Move question down"><ArrowDown size={13} /></button>
                    <button type="button" onClick={() => { if (window.confirm(`Delete assessment question ${index + 1}?`)) update(current => ({ ...current, assessment: current.assessment.filter((_, i) => i !== index) })) }} className="border border-[#dddddd] p-2 text-[#6b6b6b] hover:text-red-600" aria-label="Delete question"><Trash2 size={13} /></button>
                  </div>
                  <div className="mb-2 grid grid-cols-1 gap-2 md:grid-cols-2">
          ...
          <div className="mb-10 flex flex-wrap items-center gap-2 border-t border-[#dddddd] pt-5">
            <button type="button" onClick={save} disabled={busy === 'save' || Boolean(problem)} className="btn-primary inline-flex items-center gap-2 text-[13px] disabled:opacity-50"><Save size={14} /> {busy === 'save' ? 'Saving...' : source === 'custom' ? 'Save and publish' : 'Save your version'}</button>
