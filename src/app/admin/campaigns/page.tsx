...
export default function AdminCampaignsPage() {
  ...
  const previewHtml = useMemo(() => renderNewsletterHtml(form, { test: true, featuredHtml: featuredSel.length ? `<div style="margin:28px 0;padding:18px;border:1px solid #e5e5e5;border-radius:12px"><div style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#5a6a76;font-weight:600">Featured this week</div><div style="margin-top:9px;color:#4d4d4d;font-size:13px">${featuredSel.map(f => f.label).join(' · ')}</div></div>` : '' }), [form, featuredSel])
  ...
  return <DashboardShell role="admin" userName="Admin">
    ...
    {showForm && <div className="fixed inset-0 z-[70] bg-[#07243b]/70 p-3 md:p-6 overflow-y-auto"><div {...formDialog.panelProps} className="mx-auto max-w-6xl rounded-[24px] bg-white shadow-2xl overflow-hidden">
      ...
      <div className="grid lg:grid-cols-[1.05fr_.95fr]">
        ...
        <div className="bg-[#e8eef4] p-5 md:p-7 lg:sticky lg:top-0 lg:h-[calc(100vh-70px)] overflow-auto"><p className="text-[10px] uppercase tracking-[.16em] text-[#5a6a76] mb-3">Live preview</p><iframe title="Newsletter preview" srcDoc={previewHtml} className="w-full h-[760px] bg-white rounded-[16px] shadow-sm border border-[#e3e7eb]"/></div>
      </div>
    </div></div>}
