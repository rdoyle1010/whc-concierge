...
import { Plus, Edit2, Trash2, Send, Eye, Save, Upload, Mail, X, TestTube2 } from 'lucide-react'
import { renderNewsletterHtml } from '@/lib/newsletter-template'

type Pick = { type: string; id: string; label: string }
type Form = {
  name: string; description: string; type: string; status: string; start_date: string; end_date: string; target_audience: string;
...
const empty: Form = { name: '', description: '', type: 'Email', status: 'draft', start_date: '', end_date: '', target_audience: 'All', content: '', preheader: '', header_image_url: '', body_image_url: '', cta_label: '', cta_url: '', footer_text: 'Better matches. Better careers. Better teams.', layout_style: 'editorial' }
...
export default function AdminCampaignsPage() {
  ...
  const previewHtml = useMemo(() => renderNewsletterHtml(form, { test: true, featuredHtml: featuredSel.length ? `<div style="margin:28px 0;padding:18px;border:1px solid #e3e7eb;border-radius:12px"><div style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#6f7f88;font-weight:600">Featured this week</div><div style="margin-top:9px;color:#53636f;font-size:13px">${featuredSel.map(f => f.label).join(' · ')}</div></div>` : '' }), [form, featuredSel])
  ...
  const openEdit = (c: any) => {
    ...
    const picks: Pick[] = (c.featured_ids || []).map((f: any) => {
    ...
    setFeaturedSel(picks); setBanner(null); setShowForm(true)
  }
  ...
  return <DashboardShell role="admin" userName="Admin">
    ...
    {audiences && <div className="dashboard-metrics mb-8">
      <Metric label="Reachable inboxes" value={audiences.all} detail="confirmed contacts"/><Metric label="Talent" value={audiences.candidates} detail="confirmed marketing"/><Metric label="Properties" value={audiences.employers} detail="confirmed marketing"/><Metric label="Newsletter list" value={audiences.newsletter || 0} detail="double opt-in subscribers"/>
    </div>}
    ...
    <section className="dashboard-panel !p-0 overflow-hidden">
      ...
      {loading ? <div className="p-8 text-[13px] text-muted">Loading…</div> : campaigns.length===0 ? <div className="p-10 text-center"><Mail className="mx-auto text-muted mb-3"/><p className="text-[13px] text-muted">No newsletters yet.</p></div> : campaigns.map(c=><div key={c.id} className="flex flex-col gap-4 border-b border-border px-5 py-5 last:border-0 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0"><div className="flex items-center gap-2"><p className="font-semibold text-ink truncate">{c.name || 'Untitled newsletter'}</p><span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[.12em] ${c.status==='sent'?'bg-emerald-50 text-emerald-700':'bg-[#eef2f4] text-[#65717a]'}`}>{c.status || 'draft'}</span></div><p className="mt-1 text-[11px] text-muted">{c.target_audience || 'All'} · {c.layout_style || 'editorial'} layout{c.sent_at?` · sent ${new Date(c.sent_at).toLocaleDateString('en-GB')}`:''}{c.recipients_count?` · ${c.recipients_count} recipients`:''}</p></div>
        <div className="flex flex-wrap gap-2"><button onClick={()=>openEdit(c)} className="btn-secondary inline-flex items-center gap-2 !px-3 !py-2"><Edit2 size={13}/>Edit</button>{c.status!=='sent'&&<><button onClick={()=>{openEdit(c);setTimeout(()=>setShowPreview(true),0)}} className="btn-secondary inline-flex items-center gap-2 !px-3 !py-2"><Eye size={13}/>Preview</button><button disabled={busy===`send-${c.id}`} onClick={()=>sendLive(c)} className="btn-primary inline-flex items-center gap-2 !px-3 !py-2"><Send size={13}/>{busy===`send-${c.id}`?'Sending…':'Send'}</button><button onClick={()=>remove(c.id)} className="btn-secondary !p-2.5" aria-label="Delete"><Trash2 size={13}/></button></>}</div>
      </div>)}
    </section>
    ...
    {showForm && <div className="fixed inset-0 z-[70] bg-[#071d2d]/70 p-3 md:p-6 overflow-y-auto"><div className="mx-auto max-w-6xl rounded-[24px] bg-white shadow-2xl overflow-hidden">
      ...
      <div className="grid lg:grid-cols-[1.05fr_.95fr]">
        <div className="p-6 md:p-8 space-y-7 border-r border-border">
          <Section title="1. Email details"><Field label="Email subject / campaign name" value={form.name} onChange={v=>setForm({...form,name:v})}/><Field label="Preheader text" value={form.preheader} onChange={v=>setForm({...form,preheader:v})} hint="The short line many inboxes show after the subject."/><div className="grid sm:grid-cols-2 gap-4"><Select label="Audience" value={form.target_audience} onChange={v=>setForm({...form,target_audience:v})} options={['All','Candidates','Employers']}/><Select label="Layout" value={form.layout_style} onChange={v=>setForm({...form,layout_style:v})} options={['editorial','feature','simple']}/></div></Section>
        ...
        <div className="bg-[#eef2f4] p-5 md:p-7 lg:sticky lg:top-0 lg:h-[calc(100vh-70px)] overflow-auto"><p className="text-[10px] uppercase tracking-[.16em] text-[#65717a] mb-3">Live preview</p><iframe title="Newsletter preview" srcDoc={previewHtml} className="w-full h-[760px] bg-white rounded-[16px] shadow-sm border border-[#dfe5e8]"/></div>
      </div>
    </div></div>}
...
function Section({title,children}:{title:string;children:React.ReactNode}){return <section><h3 className="text-[13px] font-semibold text-[#10283b] mb-4">{title}</h3><div className="space-y-4">{children}</div></section>}
