...
export default function AdminCampaignsPage() {
  ...
  return <DashboardShell role="admin" userName="Admin">
    ...
    {showForm && <div className="fixed inset-0 z-[70] bg-[#161814]/70 p-3 md:p-6 overflow-y-auto"><div {...formDialog.panelProps} className="mx-auto max-w-6xl rounded-[24px] bg-white shadow-2xl overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-6 py-4"><div><p className="text-[10px] uppercase tracking-[.16em] text-muted">Newsletter studio</p><h2 id="admin-campaign-editor-heading" className="text-[24px] text-[#222321] mt-1">{editing?'Edit newsletter':'Create newsletter'}</h2></div><button onClick={()=>setShowForm(false)} className="p-2 text-muted"><X size={20}/></button></div>
      <div className="grid lg:grid-cols-[1.05fr_.95fr]">
        ...
        <div className="bg-[#e3dcd1] p-5 md:p-7 lg:sticky lg:top-0 lg:h-[calc(100vh-70px)] overflow-auto"><p className="text-[10px] uppercase tracking-[.16em] text-[#57544c] mb-3">Live preview</p><iframe title="Newsletter preview" srcDoc={previewHtml} className="w-full h-[760px] bg-white rounded-[16px] shadow-sm border border-[#dcd4c8]"/></div>
      </div>
    </div></div>}

    {showPreview && <div className="fixed inset-0 z-[90] bg-[#161814]/80 p-4 md:p-8 overflow-y-auto"><div {...previewDialog.panelProps} className="max-w-4xl mx-auto"><div className="flex justify-between items-center mb-3 text-white"><span className="text-[12px] uppercase tracking-[.15em]">Newsletter preview</span><button onClick={()=>setShowPreview(false)} className="p-2"><X/></button></div><iframe title="Full newsletter preview" srcDoc={previewHtml} className="w-full min-h-[850px] bg-white rounded-[20px]"/></div></div>}
  </DashboardShell>
}
...
function Section({title,children}:{title:string;children:React.ReactNode}){return <section><h3 className="text-[13px] font-semibold text-[#222321] mb-4">{title}</h3><div className="space-y-4">{children}</div></section>}
