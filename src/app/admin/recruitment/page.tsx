...
const STATUS_COLORS: Record<string, string> = {
  ...
  search_active: 'bg-[#e7e7e7] text-[#1c1c1c]', shortlist_sent: 'bg-[#1c1c1c] text-white',
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {[{ value: 'active', label: 'Active' }, ...STATUSES, { value: 'all', label: 'All' }].map(option => (
            <button key={option.value} type="button" onClick={() => setFilter(option.value)}
              className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold ${filter === option.value ? 'bg-[#1c1c1c] text-white' : 'bg-white border border-border text-secondary hover:text-ink'}`}>
              {option.label}
            </button>
          ))}
        </div>
              ...
              <div key={row.id} className="dashboard-card">
                <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
                  <div className="min-w-0">
                    <p className="text-[15px] font-semibold text-ink inline-flex items-center gap-1.5"><Briefcase size={14} className="text-[#1c1c1c]" /> {row.job_title}{row.service === 'executive' ? ' · Executive search' : ''}</p>
                ...
                <p className="text-[12.5px] text-gray-700 leading-relaxed bg-[#f1f1f1] rounded-lg px-3 py-2.5 mb-3 whitespace-pre-wrap">{row.brief}</p>
