...
export default function AdminAdvertisingPage() {
  ...
  return <DashboardShell role="admin" userName="Admin">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-6"><div><p className="dashboard-eyebrow">Content & revenue</p><div className="flex items-center gap-2 mb-1"><Megaphone size={22} className="text-accent" /><h1 className="dashboard-title">Sponsored Ads</h1></div><p className="dashboard-intro">Stripe-paid brand adverts. Nothing appears publicly until you approve it.</p></div><Link href="/advertise" target="_blank" className="btn-secondary text-[12px] inline-flex items-center gap-2"><ExternalLink size={13} /> View booking page</Link></div>
    {notice && <div className="bg-green-50 text-green-700 text-sm px-4 py-3 rounded-lg mb-4">{notice}</div>}
    {error && <div className="bg-red-50 text-red-600 text-sm px-4 py-3 rounded-lg mb-4">{error}</div>}
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8"><div className="dashboard-card !py-4"><p className="text-[11px] uppercase tracking-wide text-muted">Live adverts</p><p className="text-[22px] font-semibold text-ink">{live.length}</p></div><div className="dashboard-card !py-4"><p className="text-[11px] uppercase tracking-wide text-muted">Monthly booked value</p><p className="text-[22px] font-semibold text-ink">£{monthly.toFixed(2)}</p></div><div className="dashboard-card !py-4"><p className="text-[11px] uppercase tracking-wide text-muted">Awaiting approval</p><p className="text-[22px] font-semibold text-amber-700">{adverts.filter(advert => advert.review_status === 'pending').length}</p></div></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">{(Object.entries(AD_PLACEMENTS) as [AdPlacementKey, (typeof AD_PLACEMENTS)[AdPlacementKey]][]).map(([key, config]) => <div key={key} className="dashboard-card !py-4"><p className="text-[12px] font-medium text-ink">{config.label}</p><p className="text-[18px] font-semibold text-accent my-1">£{formatPounds(prices[key] ?? config.monthlyPence)}/month</p><p className="text-[10px] uppercase tracking-wide text-muted mb-1">Self-serve price</p><p className="text-[11px] text-secondary">{config.description}</p></div>)}</div>
    <h2 className="text-[16px] font-medium text-ink mb-3">All adverts</h2>
    {loading ? <div className="flex justify-center h-40"><div className="animate-spin w-8 h-8 border-2 border-accent border-t-transparent rounded-full" /></div> : adverts.length === 0 ? <div className="dashboard-card text-center py-12 text-muted">No advert bookings yet.</div> : <div className="space-y-3">{adverts.map(advert => {
