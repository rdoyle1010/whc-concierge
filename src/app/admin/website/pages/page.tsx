...
export default function PublicPagesEditor() {
  ...
  useEffect(() => { fetch('/api/admin/public-pages').then(r => r.json()).then(data => { if (data.draft) setContent(data.draft); if (data.published) setPublished(data.published) }).finally(() => setLoading(false)) }, [])
  ...
  const ImageEditor = ({ path, image, title }: any) => <div className="border border-border bg-white p-4">
    ...
    <div className="aspect-[16/9] bg-surface overflow-hidden mb-3">{image.url ? <img src={image.url} alt={image.alt} className="w-full h-full object-cover" style={{objectPosition:`${image.focalX}% ${image.focalY}%`}}/> : <div className="h-full flex items-center justify-center text-muted"><ImageIcon/></div>}</div>
