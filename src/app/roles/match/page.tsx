  ...
  if (profileIncomplete) return (
    <div className="min-h-screen bg-surface flex items-center justify-center px-6">
      <div className="max-w-lg dashboard-card text-center">
        <div className="w-14 h-14 rounded-2xl bg-[#f5f5f5] flex items-center justify-center mx-auto mb-5"><Sparkles size={22} className="text-[#1a1a1a]" /></div>
  ...
  if (idx >= jobs.length) return (
    <div className="min-h-screen bg-surface flex items-center justify-center px-6">
      <div className="max-w-md text-center">
        <div className="w-16 h-16 bg-white border border-border rounded-2xl flex items-center justify-center mx-auto mb-5"><Sparkles size={24} className="text-[#1a1a1a]" /></div>
  ...
  if (savedDraft) return (
    <div className="min-h-screen bg-surface flex items-center justify-center px-6">
      <div className="w-full max-w-xl rounded-3xl border border-border bg-white p-8 shadow-xl">
        <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#f5f5f5]"><FileText size={23} className="text-[#1a1a1a]" /></div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-9 md:py-12">
        <div className="mb-8 md:flex md:items-end md:justify-between">
          <div>
            ...
            <div className="mb-0 mt-3 inline-flex rounded-xl border border-[#dcd4c6] bg-white p-1 text-[12px] font-semibold"><a href="/talent/jobs" className="rounded-lg px-4 py-2 text-[#5c6a75] hover:text-[#111111]">Browse list</a><span className="rounded-lg bg-[#111111] px-4 py-2 text-white">Match deck</span></div>
          ...
          <Link href="/talent/applications" className="mt-4 md:mt-0 text-[12px] font-semibold text-[#1a1a1a] hover:underline">View My Applications →</Link>
        </div>
        ...
        <article className="overflow-hidden rounded-3xl border border-border bg-white shadow-[0_18px_55px_rgba(16,40,59,.08)] lg:grid lg:grid-cols-[42%_58%]">
          <div className="relative min-h-[300px] lg:min-h-[620px] bg-[#111111] overflow-hidden">
            {photo ? <img src={photo} alt={property} className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-[#111111] via-[#333333] to-[#1a1a1a]" />}
          ...
          <div className="p-6 md:p-8 lg:p-10 flex flex-col">
            ...
            <div className="rounded-2xl border border-[#e5e5e5] bg-[#fafafa] p-5 mb-5">
              <p className="text-[10px] uppercase tracking-[.16em] text-[#1a1a1a] font-semibold mb-2">Why this role is showing</p>
              ...
              {job.matchingSkills?.length > 0 && <div className="flex flex-wrap gap-1.5 mt-3">{job.matchingSkills.slice(0,5).map((skill:string)=><span key={skill} className="text-[10px] border border-[#e5e5e5] bg-white text-[#1a1a1a] px-2.5 py-1 rounded-full inline-flex items-center gap-1"><Check size={9}/>{skill}</span>)}</div>}
            </div>
            ...
            <button type="button" onClick={() => setExpanded(!expanded)} className="w-fit text-[12px] font-semibold text-[#111111] inline-flex items-center gap-1.5"><ChevronDown size={13} className={expanded ? 'rotate-180 transition-transform' : 'transition-transform'}/>{expanded ? 'Hide detailed match' : 'See detailed match & role'}</button>
