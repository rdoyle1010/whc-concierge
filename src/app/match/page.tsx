...
export default async function MatchExplainerPage() {
  ...
  return (
    <div className="website-theme min-h-screen bg-white text-[#1c1c1c]" style={websiteCssVariables(content)}>
      ...
      <main id="main-content" className="pt-[76px]">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-[1fr_0.9fr] lg:items-center lg:px-8 lg:py-24">
            <div className="max-w-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login?role=talent&next=%2Froles%2Fmatch" className="inline-flex items-center gap-2 bg-[#1c1c1c] px-6 py-3.5 text-[13px] font-semibold text-white">Sign in to start matching <ArrowRight size={15}/></Link>
                <Link href="/register/talent" className="inline-flex items-center gap-2 border border-[#6b6b6b] bg-white px-6 py-3.5 text-[13px] font-semibold text-[#1c1c1c]">Create free profile</Link>
              </div>
              <p className="mt-5 text-[11px] leading-5 text-[#6b6b6b]">Nothing is sent to an employer just because you swipe. You stay in control and choose when to submit an application.</p>
            </div>

            <div className="relative mx-auto w-full max-w-[440px] py-8">
              <div className="absolute left-2 top-12 h-[470px] w-[90%] -rotate-3 border border-[#dddddd] bg-[#f1f1f1]" />
              <div className="absolute right-2 top-10 h-[470px] w-[90%] rotate-3 border border-[#dddddd] bg-white" />
              <div className="relative overflow-hidden border border-[#dddddd] bg-white shadow-[0_24px_70px_rgba(28,28,28,.14)]">
                ...
                <div className="p-6">
                  <div className="mb-5 flex items-start justify-between gap-4">
                    <div><p className="text-[10px] uppercase tracking-[.14em] text-[#555555]">Your match</p><p className="mt-1 text-[13px] text-[#555555]">Strong alignment across skills, experience and location.</p></div>
                    <div className="shrink-0 border border-[#dddddd] bg-[#f1f1f1] px-4 py-2 text-center"><p className="text-[27px] font-semibold text-[#1c1c1c]">86%</p><p className="text-[9px] font-semibold uppercase tracking-[.1em] text-[#555555]">Strong</p></div>
                  </div>
                  <div className="border-t border-[#dddddd] pt-5">
                    <p className="text-[11px] font-semibold text-[#1c1c1c]">Why it fits</p>
                    <div className="mt-3 space-y-2 text-[12px] text-[#555555]">
                      <p className="flex gap-2"><Check size={14} className="mt-0.5 text-[#1c1c1c]"/> Luxury spa experience aligns</p>
                      <p className="flex gap-2"><Check size={14} className="mt-0.5 text-[#1c1c1c]"/> Required treatment skills match</p>
                      <p className="flex gap-2"><Check size={14} className="mt-0.5 text-[#1c1c1c]"/> Within your preferred travel radius</p>
                    </div>
                  </div>
                  <div className="mt-6 grid grid-cols-2 gap-3">
                    <div className="flex items-center justify-center gap-2 border border-[#dddddd] py-3 text-[12px] font-semibold text-[#555555]"><X size={17}/> Swipe left</div>
                    <div className="flex items-center justify-center gap-2 bg-[#1c1c1c] py-3 text-[12px] font-semibold text-white"><Heart size={16}/> Swipe right</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="bg-white py-16 lg:py-20">
          <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[.17em] text-[#555555]">What creates your score</p>
              <h2 className="site-heading mt-4 text-[38px] leading-[1.02] md:text-[48px]">More than a job title.</h2>
              <p className="mt-5 text-[14px] leading-7 text-[#555555]">A Spa Director and a newly qualified therapist should never get the same matching logic. Your score reflects the level and detail of the role you are actually looking at.</p>
            </div>
            <div className="grid gap-px border border-[#dddddd] bg-[#dddddd] sm:grid-cols-2">
              {signals.map(signal => <div key={signal} className="flex items-center gap-3 bg-white px-5 py-5 text-[13px] font-medium text-[#3a3a3a]"><Sparkles size={14} className="text-[#1c1c1c]"/>{signal}</div>)}
            </div>
          </div>
        </section>
