...
export default function PrivacyPreferences() {
  ...
  return <div className="space-y-6">
    {message && <div className="border border-[#dfe5e8] bg-[#f7f9fa] px-4 py-3 text-[13px] text-[#53636f]">{message}</div>}

    <section className="dashboard-card">
      <div className="flex gap-3"><ShieldCheck size={20} className="text-[#0b2f4d] mt-0.5"/><div><h2 className="text-[22px]">Privacy & contact preferences</h2><p className="text-[13px] leading-6 text-[#65717a] mt-2">Marketing is optional and separate from the service messages needed to run your account, applications, bookings, payments and security. You can change optional preferences at any time.</p></div></div>
    </section>

    <section className="dashboard-card">
      <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><Mail size={19} className="text-[#6f7f88] mt-0.5"/><div><h3 className="text-[19px]">WHC marketing emails</h3><p className="text-[12px] leading-5 text-[#65717a] mt-1 max-w-2xl">{marketingWording}</p></div></div><Status value={prefs.marketing_email_status}/></div>
      ...
      {marketingPending && <p className="text-[11px] text-[#65717a] mt-3">Marketing remains OFF until you click the confirmation link in the email.</p>}
    </section>

    <section className="dashboard-card">
      <div className="flex gap-3 mb-5"><Bell size={19} className="text-[#6f7f88]"/><div><h3 className="text-[19px]">Updates you choose</h3><p className="text-[12px] text-[#65717a] mt-1">These are optional preference-based updates, not essential account/security messages.</p></div></div>
    ...
    <section className="dashboard-card">
      <div className="flex gap-3 mb-5"><Users size={19} className="text-[#6f7f88]"/><div><h3 className="text-[19px]">Profile & data sharing</h3><p className="text-[12px] text-[#65717a] mt-1">Control optional sharing outside the steps needed to process an application, booking or contract you initiate.</p></div></div>
      <div className="divide-y divide-[#e7ecee]">
    ...
    <section className="dashboard-card bg-[#f7f9fa]">
      <div className="flex gap-3"><Database size={18} className="text-[#6f7f88] mt-0.5"/><div><h3 className="text-[17px]">Essential service communications</h3><p className="text-[12px] leading-5 text-[#65717a] mt-1">Account verification, security alerts, application/booking updates, receipts, payment information, legal notices and messages needed to deliver a service are not treated as optional marketing. WHC may still need to send those while your account or transaction is active.</p><p className="text-[11px] text-[#65717a] mt-3">Read the <Link href="/privacy" className="underline">Privacy Policy</Link> for the purposes and lawful bases used for each category.</p></div></div>
    </section>
  </div>
}

function Toggle({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: (v: boolean) => void }) {
  return <label className="flex items-start justify-between gap-5 py-4 cursor-pointer"><div><p className="text-[13px] font-semibold text-[#10283b]">{label}</p><p className="text-[11px] leading-5 text-[#65717a] mt-1 max-w-2xl">{description}</p></div><input type="checkbox" className="mt-1 h-4 w-4" checked={checked} onChange={e => onChange(e.target.checked)}/></label>
}

function Status({ value }: { value: Preferences['marketing_email_status'] }) {
  const labels: Record<string, [string, string]> = { confirmed: ['Confirmed', 'text-green-700 bg-green-50'], pending: ['Awaiting confirmation', 'text-amber-700 bg-amber-50'], unsubscribed: ['Unsubscribed', 'text-[#65717a] bg-[#f1f4f6]'], never: ['Off', 'text-[#65717a] bg-[#f1f4f6]'] }
