            ...
            {data.map((property: any) => {
              ...
              return (
                <Link key={property.id} href={`/properties/${property.id}`} className="group overflow-hidden rounded-2xl border border-[#dddddd] bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                  <div className="relative h-36 overflow-hidden bg-[#e7e7e7]">
                    ...
                    <span className="absolute left-4 top-4 inline-flex items-center gap-1 rounded-full bg-[#1c1c1c] px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-white"><Star size={10} fill="currentColor" /> Featured</span>
                  </div>
                  <div className="p-5">
