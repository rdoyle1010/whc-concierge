...
export default function PropertyAddressLookup({
  ...
  return (
    <div className="md:col-span-2 border border-border bg-[#f7f8fa] p-4">
      ...
      {suggestions.length > 0 && (
        <div className="mt-3 max-w-2xl border border-border bg-white divide-y divide-border">
          {suggestions.map(item => <button key={item.id} type="button" onClick={() => selectAddress(item.id)} className="block w-full px-4 py-3 text-left text-[12px] text-ink hover:bg-[#f7f8fa]">{item.address}</button>)}
        </div>
      )}
