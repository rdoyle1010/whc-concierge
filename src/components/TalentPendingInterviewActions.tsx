...
export default function TalentPendingInterviewActions() {
  ...
  return (
    <section className="mb-7 overflow-hidden rounded-[22px] border border-[#57534e] bg-white shadow-[0_16px_40px_rgba(28,27,26,0.07)]">
      <div className="border-b border-[#e0dad2] bg-amber-50 px-5 py-4">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#1c1b1a]">Action required</p>
      ...
      <div className="p-5">
        ...
        <div className="space-y-4">
          {pending.map(({ item, interview }) => {
            ...
            return (
              <div key={interview.id} className="rounded-2xl border border-[#e0dad2] bg-[#f3f0eb] p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#1c1b1a]">Stage {interview.round_number} of 3</p>
                    <h3 className="mt-1 text-[21px] font-semibold text-ink">{label}</h3>
                    <p className="mt-1 text-[13px] font-medium text-[#1c1b1a]">{item.job?.job_title || 'Role'}</p>
                ...
                <div className="mt-4 flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-[12px] text-secondary">
                  <CalendarDays size={14} className="text-[#1c1b1a]" />
                    ...
                    {(interview.proposed_slots || []).map(slot => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-xl border px-4 py-3 text-left transition ${isSelected ? 'border-[#1c1b1a] bg-[#e9e4dd] shadow-sm' : 'border-[#57534e] bg-white hover:border-[#1c1b1a]'}`}
                        >
                          <span className="flex items-center gap-2 text-[12px] font-semibold text-[#1c1b1a]"><Clock size={13} />{when(slot)}</span>
                ...
                <div className="mt-4">
                  <label htmlFor={`note-${interview.id}`} className="mb-2 flex items-center gap-2 text-[11px] font-semibold text-ink">
                    <MessageSquare size={13} className="text-[#1c1b1a]" />2. Add a note to the property <span className="font-normal text-muted">(optional)</span>
                  </label>
                  <textarea
                    ...
                    className="w-full rounded-xl border border-[#e0dad2] bg-white px-3 py-3 text-[12px] text-ink outline-none transition placeholder:text-muted focus:border-[#1c1b1a]"
                  />
                </div>
                ...
                <div className="mt-4 border-t border-[#e0dad2] pt-4">
                  ...
                  <button
                    ...
                    className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#1c1b1a] px-5 py-3 text-[12px] font-semibold text-white transition hover:bg-[#33302d] disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <CheckCircle size={15} />
                  ...
                  <button
                    ...
                    className="mt-2 block text-[11px] font-semibold text-[#1c1b1a] underline disabled:opacity-40 sm:ml-3 sm:mt-0 sm:inline-block"
                  >
