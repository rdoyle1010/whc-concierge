...
import { CalendarDays, CheckCircle, Clock, Briefcase, XCircle } from 'lucide-react'
      ...
      return <div key={item.id} className="rounded-2xl border border-[#e7e1d7] bg-[#fcfbf8] p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="w-full shrink-0 lg:w-[240px]">
            {role === 'employer' && item.status === 'interview' && latestInterview?.status === 'confirmed' && !item.offer && <button type="button" onClick={()=>setOfferDraft({applicationId:item.id,salary:'',period:'annual',startDate:'',note:''})} className="btn-primary w-full">Make offer</button>}
