...
export default function ApplicationPipelineHub({ role }: { role: Role }) {
  ...
  return <div className="mb-7 rounded-[22px] border border-[#ded8cc] bg-white p-5 shadow-[0_12px_35px_rgba(22,40,55,0.05)]">
    ...
    <div className="space-y-4">{items.map(item => {
      ...
      return <div key={item.id} className="rounded-2xl border border-[#e5e5e5] bg-[#fcfbf8] p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            ...
            {(item.interviews || []).length > 0 && <div className="mt-4 space-y-2">{(item.interviews || []).map(interview => <div key={interview.id} className="rounded-xl border border-[#e4ddd1] bg-white p-3">
              <div className="flex flex-wrap items-center gap-2"><CalendarDays size={14} className="text-[#1a1a1a]"/><span className="text-[12px] font-semibold text-ink">Interview {interview.round_number}</span><span className="text-[10px] uppercase tracking-wide text-muted">{methodLabel(interview.interview_method)}</span><span className={`ml-auto rounded-full px-2 py-1 text-[9px] font-semibold uppercase ${interview.status === 'confirmed' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{interview.status}</span></div>
              {interview.status === 'confirmed' && interview.selected_slot ? <p className="mt-2 flex items-center gap-1.5 text-[12px] text-secondary"><CheckCircle size={13} className="text-emerald-600"/>{when(interview.selected_slot)}</p> : role === 'talent' ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{(interview.proposed_slots || []).map((slot:string) => <button key={slot} type="button" disabled={busy===interview.id} onClick={()=>confirmInterview(interview.id,slot)} className="rounded-xl border border-[#dcd4c8] bg-[#f7f7f7] px-3 py-2 text-left text-[11px] font-medium text-[#17344d] hover:border-[#555555]"><Clock size={12} className="mr-1 inline"/>{when(slot)}</button>)}</div> : <div className="mt-2 flex flex-wrap gap-2">{(interview.proposed_slots || []).map((slot:string) => <span key={slot} className="rounded-full bg-[#f5f1e9] px-2.5 py-1 text-[10px] text-secondary">{when(slot)}</span>)}</div>}
              <div className="mt-3 grid gap-2 text-[11px] text-secondary sm:grid-cols-2">
                {interview.meeting_link && <p className="flex items-start gap-2"><LinkIcon size={13} className="mt-0.5 shrink-0 text-[#1a1a1a]"/><a className="break-all underline" href={interview.meeting_link} target="_blank" rel="noreferrer">Join interview</a></p>}
                {interview.venue_address && <p className="flex items-start gap-2"><MapPin size={13} className="mt-0.5 shrink-0 text-[#1a1a1a]"/>{interview.venue_address}</p>}
                {interview.contact_name && <p className="flex items-start gap-2"><UserRound size={13} className="mt-0.5 shrink-0 text-[#1a1a1a]"/>Ask for {interview.contact_name}</p>}
                {interview.preparation_required && <p className="flex items-start gap-2 sm:col-span-2"><ClipboardList size={13} className="mt-0.5 shrink-0 text-[#1a1a1a]"/><span><strong>Prepare:</strong> {interview.preparation_required}</span></p>}
