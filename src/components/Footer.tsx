...
export default function Footer({ siteContent }: { siteContent?: WebsiteContent }) {
  ...
  return <>
    ...
    <section className="bg-white border-t border-[#e0dad2] border-b border-[#e0dad2]" aria-label="Share and follow Wellness House Collective">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 md:py-10 grid lg:grid-cols-2 gap-8 lg:gap-12">
        <div><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#57534e]">Share this page</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#1c1b1a]">Worth sharing? Pass it on.</h2><p className="mt-2 text-[12px] leading-5 text-[#57534e] max-w-xl">Share jobs, Academy courses, Residency opportunities, articles or any WHC page directly with your network.</p><div className="flex flex-wrap gap-2 mt-5"><button type="button" onClick={() => openShare('linkedin')} className="inline-flex items-center gap-2 border border-[#e0dad2] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-[#f3f0eb] transition-colors rounded-lg"><Linkedin size={15}/>LinkedIn</button><button type="button" onClick={() => openShare('facebook')} className="inline-flex items-center gap-2 border border-[#e0dad2] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-[#f3f0eb] transition-colors rounded-lg"><Facebook size={15}/>Facebook</button><button type="button" onClick={() => openShare('whatsapp')} className="inline-flex items-center gap-2 border border-[#e0dad2] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-[#f3f0eb] transition-colors rounded-lg"><MessageCircle size={15}/>WhatsApp</button><button type="button" onClick={() => openShare('email')} className="inline-flex items-center gap-2 border border-[#e0dad2] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-[#f3f0eb] transition-colors rounded-lg"><Mail size={15}/>Email</button><button type="button" onClick={copyPage} className="inline-flex items-center gap-2 border border-[#e0dad2] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-[#f3f0eb] transition-colors rounded-lg">{copied ? <Check size={15}/> : <Link2 size={15}/>} {copied ? 'Copied' : 'Copy link'}</button><button type="button" onClick={nativeShare} className="inline-flex items-center gap-2 bg-[#1c1b1a] px-3.5 py-2.5 text-[12px] font-semibold text-white hover:bg-[#33302d] transition-colors rounded-lg"><Share2 size={15}/>More</button></div></div>

        <div className="lg:border-l lg:border-[#e0dad2] lg:pl-12"><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#57534e]">Connect with WHC</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#1c1b1a]">Follow the conversation.</h2><p className="mt-2 text-[12px] leading-5 text-[#57534e] max-w-xl">Jobs, industry insight, Academy updates, employer features and new opportunities from Wellness House Collective.</p>{socialCards.length ? <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mt-5">{socialCards.map(item => <a key={item.key} href={item.url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 border border-[#e0dad2] rounded-xl p-4 hover:border-[#6e6a66] hover:bg-[#f3f0eb] transition-all"><div className="h-9 w-9 rounded-lg bg-[#e9e4dd] flex items-center justify-center text-[#1c1b1a]">{item.icon}</div><div><p className="text-[12px] font-semibold text-[#1c1b1a]">{item.label}</p><p className="text-[10px] text-[#6e6a66] group-hover:text-[#57534e]">View public profile</p></div></a>)}</div> : <p className="mt-5 text-[12px] text-[#6e6a66]">Public social profiles will appear here when added by WHC Admin.</p>}</div>
      </div>
    </section>

    <footer className="bg-[#1c1b1a] text-white border-t border-white/10"><div className="max-w-7xl mx-auto px-6 lg:px-8 py-12"><div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr_1fr_1fr] gap-9 pb-10"><div><Wordmark dark /><p className="mt-5 max-w-sm text-[13px] leading-6 text-white/60">The professional platform for spa and wellness careers. Live roles, agency cover, residencies and the Academy.</p>
      ...
      {newsletterState === 'success' ? (
        ...
        <form onSubmit={subscribeToNewsletter} className="mt-3">
          <div className="flex gap-2">
            ...
            />
            <button type="submit" disabled={newsletterState === 'sending'} className="shrink-0 rounded-lg bg-white px-4 py-2.5 text-[12px] font-semibold text-[#1c1b1a] hover:bg-white/85 transition-colors disabled:opacity-60">
