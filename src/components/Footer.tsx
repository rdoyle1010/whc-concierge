...
export default function Footer({ siteContent }: { siteContent?: WebsiteContent }) {
  ...
  return <>
    <section className="bg-white border-t border-[#dddddd] overflow-hidden" aria-label="Talent House Collective hospitality photography">
      <div className="max-w-[1500px] mx-auto px-0 md:px-6 lg:px-8 py-0 md:py-8"><div className="grid grid-cols-2 lg:grid-cols-4 gap-px md:gap-3 bg-[#dddddd] md:bg-transparent">{editorialImages.map((image, index) => <div key={`${image.url}-${index}`} className="group relative overflow-hidden bg-[#f1f1f1] aspect-[4/5] md:aspect-[3/4]">{image.url ? <img src={image.url} alt={image.alt} loading="lazy" decoding="async" fetchPriority="low" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.025]" style={{ objectPosition: `${image.focalX}% ${image.focalY}%` }} /> : null}<div className="absolute inset-0 bg-gradient-to-t from-[#0f0f0f]/55 via-transparent to-transparent" /><p className="absolute bottom-4 left-4 right-4 text-[9px] md:text-[10px] uppercase tracking-[.16em] font-semibold text-white/90">{image.label}</p></div>)}</div></div>
    </section>
