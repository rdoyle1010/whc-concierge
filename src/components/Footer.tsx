  ...
  return <>
    ...
    <section className="bg-white border-t border-[#e3e7eb] border-b border-[#e3e7eb]" aria-label="Share and follow Wellness House Collective">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 md:py-10 grid lg:grid-cols-2 gap-8 lg:gap-12">
        <div><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#5a6a76]">Share this page</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#10283b]">Worth sharing? Pass it on.</h2><p className="mt-2 text-[12px] leading-5 text-[#5a6a76] max-w-xl">Share jobs, Academy courses, Residency opportunities, articles or any WHC page directly with your network.</p><div className="flex flex-wrap gap-2 mt-5"><button type="button" onClick={() => openShare('linkedin')} className="inline-flex items-center gap-2 border border-[#e3e7eb] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f5f6f8] transition-colors rounded-lg"><Linkedin size={15}/>LinkedIn</button><button type="button" onClick={() => openShare('facebook')} className="inline-flex items-center gap-2 border border-[#e3e7eb] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f5f6f8] transition-colors rounded-lg"><Facebook size={15}/>Facebook</button><button type="button" onClick={() => openShare('whatsapp')} className="inline-flex items-center gap-2 border border-[#e3e7eb] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f5f6f8] transition-colors rounded-lg"><MessageCircle size={15}/>WhatsApp</button><button type="button" onClick={() => openShare('email')} className="inline-flex items-center gap-2 border border-[#e3e7eb] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f5f6f8] transition-colors rounded-lg"><Mail size={15}/>Email</button><button type="button" onClick={copyPage} className="inline-flex items-center gap-2 border border-[#e3e7eb] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f5f6f8] transition-colors rounded-lg">{copied ? <Check size={15}/> : <Link2 size={15}/>} {copied ? 'Copied' : 'Copy link'}</button><button type="button" onClick={nativeShare} className="inline-flex items-center gap-2 bg-[#0b2f4d] px-3.5 py-2.5 text-[12px] font-semibold text-white hover:bg-[#123f64] transition-colors rounded-lg"><Share2 size={15}/>More</button></div></div>

        <div className="lg:border-l lg:border-[#e3e7eb] lg:pl-12"><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#5a6a76]">Connect with WHC</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#10283b]">Follow the conversation.</h2><p className="mt-2 text-[12px] leading-5 text-[#5a6a76] max-w-xl">Jobs, industry insight, Academy updates, employer features and new opportunities from Wellness House Collective.</p>{socialCards.length ? <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mt-5">{socialCards.map(item => <a key={item.key} href={item.url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 border border-[#e3e7eb] rounded-xl p-4 hover:border-[#8a949b] hover:bg-[#f5f6f8] transition-all"><div className="h-9 w-9 rounded-lg bg-[#e8eef4] flex items-center justify-center text-[#0b2f4d]">{item.icon}</div><div><p className="text-[12px] font-semibold text-[#10283b]">{item.label}</p><p className="text-[10px] text-[#8a949b] group-hover:text-[#5a6a76]">View public profile</p></div></a>)}</div> : <p className="mt-5 text-[12px] text-[#8a949b]">Public social profiles will appear here when added by WHC Admin.</p>}</div>
      </div>
    </section>
