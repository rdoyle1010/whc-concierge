...
export default function Footer({ siteContent }: { siteContent?: WebsiteContent }) {
  ...
  return <>
    <section className="bg-white border-t border-[#dddddd] overflow-hidden" aria-label="Talent House Collective hospitality photography">
      <div className="max-w-[1500px] mx-auto px-0 md:px-6 lg:px-8 py-0 md:py-8"><div className="grid grid-cols-2 lg:grid-cols-4 gap-px md:gap-3 bg-[#dddddd] md:bg-transparent">{editorialImages.map((image, index) => <div key={`${image.url}-${index}`} className="group relative overflow-hidden bg-[#f1f1f1] aspect-[4/5] md:aspect-[3/4]">{image.url ? <Image src={image.url} alt={image.alt} fill sizes="(max-width: 1024px) 50vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.025]" style={{ objectPosition: `${image.focalX}% ${image.focalY}%` }} /> : null}<div className="absolute inset-0 bg-gradient-to-t from-[#0f0f0f]/80 via-[#0f0f0f]/25 to-transparent" /><p className="absolute bottom-4 left-4 right-4 text-[9px] md:text-[10px] uppercase tracking-[.16em] font-semibold text-white [text-shadow:0_1px_3px_rgba(15,15,15,.65)]">{image.label}</p></div>)}</div></div>
    </section>

    <section className="bg-white border-t border-[#dddddd] border-b border-[#dddddd]" aria-label="Share and follow Talent House Collective">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 md:py-10 grid lg:grid-cols-2 gap-8 lg:gap-12">
        <div><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#555555]">Share this page</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#1c1c1c]">Worth sharing? Pass it on.</h2><p className="mt-2 text-[12px] leading-5 text-[#555555] max-w-xl">Share jobs, Academy courses, Residency opportunities, articles or any Talent House page directly with your network.</p><div className="flex flex-wrap gap-2 mt-5"><button type="button" onClick={() => openShare('linkedin')} className="inline-flex items-center gap-2 border border-[#dddddd] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1] transition-colors rounded-lg"><Linkedin size={15}/>LinkedIn</button><button type="button" onClick={() => openShare('facebook')} className="inline-flex items-center gap-2 border border-[#dddddd] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1] transition-colors rounded-lg"><Facebook size={15}/>Facebook</button><button type="button" onClick={() => openShare('whatsapp')} className="inline-flex items-center gap-2 border border-[#dddddd] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1] transition-colors rounded-lg"><MessageCircle size={15}/>WhatsApp</button><button type="button" onClick={() => openShare('email')} className="inline-flex items-center gap-2 border border-[#dddddd] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1] transition-colors rounded-lg"><Mail size={15}/>Email</button><button type="button" onClick={copyPage} className="inline-flex items-center gap-2 border border-[#dddddd] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#1c1c1c] hover:bg-[#f1f1f1] transition-colors rounded-lg">{copied ? <Check size={15}/> : <Link2 size={15}/>} {copied ? 'Copied' : 'Copy link'}</button><button type="button" onClick={nativeShare} className="inline-flex items-center gap-2 bg-[#1c1c1c] px-3.5 py-2.5 text-[12px] font-semibold text-white hover:bg-[#333333] transition-colors rounded-lg"><Share2 size={15}/>More</button></div></div>

        <div className="lg:border-l lg:border-[#dddddd] lg:pl-12"><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#555555]">Connect with Talent House</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#1c1c1c]">Follow the conversation.</h2><p className="mt-2 text-[12px] leading-5 text-[#555555] max-w-xl">Jobs, industry insight, Academy updates, employer features and new opportunities from Talent House Collective.</p>{socialCards.length ? <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mt-5">{socialCards.map(item => <a key={item.key} href={item.url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 border border-[#dddddd] rounded-xl p-4 hover:border-[#6b6b6b] hover:bg-[#f1f1f1] transition-all"><div className="h-9 w-9 rounded-lg bg-[#e7e7e7] flex items-center justify-center text-[#1c1c1c]">{item.icon}</div><div><p className="text-[12px] font-semibold text-[#1c1c1c]">{item.label}</p><p className="text-[10px] text-[#6b6b6b] group-hover:text-[#555555]">View public profile</p></div></a>)}</div> : <p className="mt-5 text-[12px] text-[#6b6b6b]">Public social profiles will appear here when added by Talent House Admin.</p>}</div>
      </div>
    </section>
