  ...
  return <>
    ...
    <section className="bg-white border-t border-[#e3e7eb] border-b border-[#e3e7eb]" aria-label="Share and follow Wellness House Collective">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 md:py-10 grid lg:grid-cols-2 gap-8 lg:gap-12">
        <div><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#6f7f88]">Share this page</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#10283b]">Worth sharing? Pass it on.</h2><p className="mt-2 text-[12px] leading-5 text-[#65717a] max-w-xl">Share jobs, Academy courses, Residency opportunities, articles or any WHC page directly with your network.</p><div className="flex flex-wrap gap-2 mt-5"><button type="button" onClick={() => openShare('linkedin')} className="inline-flex items-center gap-2 border border-[#dfe5e8] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f7f9fa] transition-colors rounded-lg"><Linkedin size={15}/>LinkedIn</button><button type="button" onClick={() => openShare('facebook')} className="inline-flex items-center gap-2 border border-[#dfe5e8] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f7f9fa] transition-colors rounded-lg"><Facebook size={15}/>Facebook</button><button type="button" onClick={() => openShare('whatsapp')} className="inline-flex items-center gap-2 border border-[#dfe5e8] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f7f9fa] transition-colors rounded-lg"><MessageCircle size={15}/>WhatsApp</button><button type="button" onClick={() => openShare('email')} className="inline-flex items-center gap-2 border border-[#dfe5e8] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f7f9fa] transition-colors rounded-lg"><Mail size={15}/>Email</button><button type="button" onClick={copyPage} className="inline-flex items-center gap-2 border border-[#dfe5e8] bg-white px-3.5 py-2.5 text-[12px] font-semibold text-[#10283b] hover:bg-[#f7f9fa] transition-colors rounded-lg">{copied ? <Check size={15}/> : <Link2 size={15}/>} {copied ? 'Copied' : 'Copy link'}</button><button type="button" onClick={nativeShare} className="inline-flex items-center gap-2 bg-[#0b2f4d] px-3.5 py-2.5 text-[12px] font-semibold text-white hover:bg-[#123f64] transition-colors rounded-lg"><Share2 size={15}/>More</button></div></div>

        <div className="lg:border-l lg:border-[#e3e7eb] lg:pl-12"><p className="text-[9px] uppercase tracking-[.2em] font-semibold text-[#6f7f88]">Connect with WHC</p><h2 className="mt-2 text-[24px] md:text-[28px] text-[#10283b]">Follow the conversation.</h2><p className="mt-2 text-[12px] leading-5 text-[#65717a] max-w-xl">Jobs, industry insight, Academy updates, employer features and new opportunities from Wellness House Collective.</p>{socialCards.length ? <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mt-5">{socialCards.map(item => <a key={item.key} href={item.url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 border border-[#dfe5e8] rounded-xl p-4 hover:border-[#aebbc2] hover:bg-[#f7f9fa] transition-all"><div className="h-9 w-9 rounded-lg bg-[#eef2f4] flex items-center justify-center text-[#0b2f4d]">{item.icon}</div><div><p className="text-[12px] font-semibold text-[#10283b]">{item.label}</p><p className="text-[10px] text-[#7d8990] group-hover:text-[#65717a]">View public profile</p></div></a>)}</div> : <p className="mt-5 text-[12px] text-[#7d8990]">Public social profiles will appear here when added by WHC Admin.</p>}</div>
      </div>
    </section>

    <footer className="bg-[#0b2f4d] text-white border-t border-white/10"><div className="max-w-7xl mx-auto px-6 lg:px-8 py-12"><div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr_1fr_1fr] gap-9 pb-10"><div><Wordmark dark /><p className="mt-5 max-w-sm text-[13px] leading-6 text-white/60">Luxury wellness recruitment, agency cover and career opportunities for exceptional professionals and properties.</p></div><div><p className="text-[9px] uppercase tracking-[0.2em] text-white/55 mb-4 font-semibold">Discover</p><LinkList items={primary} /></div><div><p className="text-[9px] uppercase tracking-[0.2em] text-white/55 mb-4 font-semibold">WHC</p><LinkList items={company} /></div><div><p className="text-[9px] uppercase tracking-[0.2em] text-white/55 mb-4 font-semibold">Support</p><LinkList items={support} /></div></div><div className="border-t border-white/10 pt-5 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between"><p className="text-[11px] text-white/45">{content.footer.copyright}</p><Link href="/admin-sign-in" className="text-[11px] text-white/35 hover:text-white/70 transition-colors" aria-label={`${content.footer.staffLabel} sign in`}>{content.footer.staffLabel}</Link></div></div></footer>
  </>
}
