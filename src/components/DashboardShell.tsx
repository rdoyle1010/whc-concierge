...
export default function DashboardShell({ children, role, userName }: DashboardShellProps) {
  ...
  if (isPublicAgencyRoute && agencyShell !== 'employer') {
    return <div className="min-h-screen bg-white"><Navbar /><main id="main-content" className="pt-[68px]"><div className="p-5 md:p-8 lg:p-10 xl:p-12 max-w-[1560px] mx-auto">{agencyShell === 'checking' ? <div className="flex min-h-[55vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#57534e] border-t-transparent" /></div> : children}</div></main></div>
  }

  return <div className="dashboard-shell min-h-screen">
    <header className="lg:hidden sticky top-0 z-30 bg-[#1c1b1a] text-white px-4 py-3.5 flex items-center justify-between border-b border-white/10"><button type="button" onClick={() => setSidebarOpen(true)} aria-label="Open dashboard navigation" className="p-1 -ml-1 text-white/85"><Menu size={22}/></button><div className="text-center leading-none"><Wordmark dark compact href={null}/><p className="mt-1.5 text-[8px] uppercase tracking-[0.2em] text-white/55">{workspaceLabel[role]}</p></div><div className="flex items-center gap-2">{viewerId && <NotificationBell userId={viewerId} />}<UniversalSearch variant="navbar" /></div></header>
    {sidebarOpen && <div className="fixed inset-0 bg-[#0f0e0d]/60 backdrop-blur-[1px] z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}
