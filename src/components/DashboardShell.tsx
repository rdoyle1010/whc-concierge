...
export default function DashboardShell({ children, role, userName, intro }: DashboardShellProps) {
  ...
  return <div className="dashboard-shell min-h-screen">
    ...
    <aside className={`dashboard-sidebar fixed top-0 left-0 h-full w-[264px] text-ink z-50 transform transition-transform duration-200 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="px-7 pt-7 pb-4"><div className="flex items-center justify-between"><Wordmark/><button type="button" onClick={() => setSidebarOpen(false)} className="lg:hidden text-muted hover:text-ink p-1" aria-label="Close dashboard navigation"><X size={19}/></button></div><div className="mt-7 pb-5 border-b border-[#dddddd]"><p className="text-muted text-[8px] uppercase tracking-[0.22em] font-semibold">{workspaceLabel[role]}</p>{userName && <p className="font-serif text-ink text-[22px] leading-tight mt-2 truncate">{userName}</p>}</div></div>
      ...
      <div className="absolute bottom-0 left-0 right-0 px-4 py-4 bg-[#f1f1f1] border-t border-[#dddddd]"><button type="button" onClick={handleSignOut} className="dashboard-nav-item flex items-center gap-3 px-3 py-2.5 text-[12.5px] text-secondary hover:text-ink hover:bg-[#e7e7e7] w-full transition-colors"><LogOut size={17}/><span>Sign out</span></button></div>
    </aside>
