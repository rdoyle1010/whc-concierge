...
import { TrendingUp,
  ...
  AlertTriangle, Heart, Building2, ChevronRight, BarChart3, CreditCard, GraduationCap, Sparkles, Ticket,
  Palette, Banknote, Download, MapPin, Brain, ClipboardList, ShieldCheck, Lock, Paperclip,
...
const navItems: Record<string, NavItem[]> = {
  ...
  admin: [
    ...
    { label: 'Who Is Looking', href: '/admin/visitors', icon: <Activity size={17} /> },
    ...
    { label: 'Events', href: '/admin/events', icon: <Calendar size={17} /> },
...
export default function DashboardShell({ children, role, userName, intro }: DashboardShellProps) {
  ...
  if (isPublicAgencyRoute && agencyShell !== 'employer') {
    return <div className="min-h-screen bg-white"><Navbar /><main id="main-content" className="pt-[68px]"><div className="p-5 md:p-8 lg:p-10 xl:p-12 max-w-[1560px] mx-auto">{agencyShell === 'checking' ? <div className="flex min-h-[55vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#57544c] border-t-transparent" /></div> : children}</div></main></div>
  }

  return <div className="dashboard-shell min-h-screen">
    <header className="lg:hidden sticky top-0 z-30 bg-[#ede8df] text-ink px-4 py-3.5 flex items-center justify-between border-b border-[#dcd4c8]"><button type="button" onClick={() => setSidebarOpen(true)} aria-label="Open dashboard navigation" className="p-1 -ml-1 text-ink"><Menu size={22}/></button><div className="text-center leading-none"><Wordmark compact href={null}/><p className="mt-1.5 text-[8px] uppercase tracking-[0.2em] text-muted">{workspaceLabel[role]}</p></div><div className="flex items-center gap-2">{viewerId && <NotificationBell userId={viewerId} />}<UniversalSearch variant="navbar" /></div></header>
    {sidebarOpen && <div className="fixed inset-0 bg-[#161814]/60 backdrop-blur-[1px] z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}
