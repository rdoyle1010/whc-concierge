...
export default function DashboardShell({ children, role, userName, intro }: DashboardShellProps) {
  ...
  return <div className="dashboard-shell min-h-screen">
    <header className="lg:hidden sticky top-0 z-30 bg-[#1c1c1c] text-white px-4 py-3.5 flex items-center justify-between border-b border-white/10"><button type="button" onClick={() => setSidebarOpen(true)} aria-label="Open dashboard navigation" className="p-1 -ml-1 text-white/85"><Menu size={22}/></button><div className="text-center leading-none"><Wordmark dark compact href={null}/><p className="mt-1.5 text-[8px] uppercase tracking-[0.2em] text-white/55">{workspaceLabel[role]}</p></div><div className="flex items-center gap-2">{viewerId && <NotificationBell userId={viewerId} />}<UniversalSearch variant="navbar" /></div></header>
