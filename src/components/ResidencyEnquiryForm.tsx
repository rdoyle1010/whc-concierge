  ...
  if (done) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        ...
        <p className="text-[12px] leading-5 text-emerald-800 mt-1">{specialistName} can accept, counter or decline inside Talent House Collective. If accepted, you&apos;ll confirm the booking and payment on-platform.</p>
      </div>
    )
  }
    ...
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div className="rounded-xl bg-parchment/70 border border-accent/20 p-4">
        <div className="flex items-start gap-2.5">
          ...
          <div>
            <p className="text-[12px] font-semibold text-ink">Book securely through Talent House Collective</p>
      ...
      {therapistTotal > 0 && (
        <div className="rounded-xl border border-border bg-surface/70 p-4 text-[12px]">
          ...
          <div className="flex justify-between py-1"><span className="text-muted">Talent House Collective fee (10%)</span><span className="font-medium text-ink">£{platformFee.toLocaleString('en-GB')}</span></div>
      ...
      <p className="text-[10px] leading-4 text-muted text-center">Keep the booking on Talent House Collective to retain the agreed terms, secure payment record and eligibility for a verified residency review.</p>
    </form>
  )
}
