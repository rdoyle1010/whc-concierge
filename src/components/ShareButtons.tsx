...
export default function ShareButtons({ url, title }: ShareButtonsProps) {
  ...
  return <div className="flex flex-wrap items-center gap-2">
    {links.map(link => <a key={link.name} href={link.href} target={link.name === 'Email' ? undefined : '_blank'} rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-[#dcd4c8] bg-white px-3 py-2 text-[11px] font-semibold text-[#222321] hover:bg-[#ede8df]" title={`Share on ${link.name}`}>{link.icon}{link.name}</a>)}
    <button type="button" onClick={handleCopyLink} className="inline-flex items-center gap-2 rounded-lg border border-[#dcd4c8] bg-white px-3 py-2 text-[11px] font-semibold text-[#222321] hover:bg-[#ede8df]" title="Copy article link">
