...
export default function NewsletterSignupBar() {
  ...
  return <aside className="fixed inset-x-3 bottom-3 z-[70] mx-auto max-w-5xl rounded-2xl border border-[#dcd4c8] bg-white shadow-[0_18px_55px_rgba(28,28,28,.18)] md:inset-x-6" aria-label="Newsletter signup">
    <div className="relative grid gap-5 px-5 py-5 md:grid-cols-[1fr_auto] md:items-center md:px-7">
      <button type="button" onClick={dismiss} aria-label="Close newsletter signup" className="absolute right-3 top-3 rounded-full p-2 text-[#6e6a60] hover:bg-[#ede8df] hover:text-[#222321]"><X size={16}/></button>
      <div className="min-w-0 pr-8 md:pr-0">
        <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.16em] text-[#57544c]"><Mail size={14}/>Talent House Newsletter</div>
        <h2 className="mt-2 text-[22px] leading-tight text-[#222321] md:text-[26px]">{config?.heading || 'The best of wellness, in your inbox.'}</h2>
        <p className="mt-1.5 max-w-2xl text-[12px] leading-5 text-[#57544c]">{config?.text || 'Jobs, industry insight, Academy updates and opportunities from Talent House Collective.'}</p>
        <p className="mt-2 text-[10px] leading-4 text-[#6e6a60]">Double opt-in: we email you once to confirm. You can unsubscribe at any time. See our <a href="/privacy" className="underline">Privacy Policy</a>.</p>
      </div>
      ...
      {success ? <div className="max-w-sm rounded-xl bg-[#eef4f1] px-4 py-3 text-[12px] leading-5 text-[#355d49]" aria-live="polite">{message}</div> : <form onSubmit={submit} className="flex min-w-0 flex-col gap-2 sm:flex-row md:w-[420px]">
        ...
        <input id="whc-newsletter-email" type="email" required autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Your email address" className="min-h-11 min-w-0 flex-1 rounded-lg border border-[#dcd4c8] bg-white px-3.5 text-[16px] text-[#222321] outline-none focus:border-[#222321] sm:text-[13px]" />
        ...
        <button type="submit" disabled={busy} className="min-h-11 rounded-lg bg-[#222321] px-5 text-[12px] font-semibold text-white hover:bg-[#3a4239] disabled:opacity-50">{busy ? 'Sending…' : (config?.button || 'Join the newsletter')}</button>
