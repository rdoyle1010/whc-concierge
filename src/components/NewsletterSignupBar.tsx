...
export default function NewsletterSignupBar() {
  ...
  return <aside className="fixed inset-x-3 bottom-3 z-[70] mx-auto max-w-5xl rounded-2xl border border-[#e0dad2] bg-white shadow-[0_18px_55px_rgba(28,27,26,.18)] md:inset-x-6" aria-label="Newsletter signup">
    <div className="relative grid gap-5 px-5 py-5 md:grid-cols-[1fr_auto] md:items-center md:px-7">
      <button type="button" onClick={dismiss} aria-label="Close newsletter signup" className="absolute right-3 top-3 rounded-full p-2 text-[#6e6a66] hover:bg-[#f3f0eb] hover:text-[#1c1b1a]"><X size={16}/></button>
      ...
      {success ? <div className="max-w-sm rounded-xl bg-[#eef4f1] px-4 py-3 text-[12px] leading-5 text-[#355d49]" aria-live="polite">{message}</div> : <form onSubmit={submit} className="flex min-w-0 flex-col gap-2 sm:flex-row md:w-[420px]">
        ...
        <input id="whc-newsletter-email" type="email" required autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Your email address" className="min-h-11 min-w-0 flex-1 rounded-lg border border-[#e0dad2] bg-white px-3.5 text-[16px] text-[#1c1b1a] outline-none focus:border-[#1c1b1a] sm:text-[13px]" />
        ...
        <button type="submit" disabled={busy} className="min-h-11 rounded-lg bg-[#1c1b1a] px-5 text-[12px] font-semibold text-white hover:bg-[#33302d] disabled:opacity-50">{busy ? 'Sending…' : (config?.button || 'Join the newsletter')}</button>
