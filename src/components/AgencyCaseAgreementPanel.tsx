...
export default function AgencyCaseAgreementPanel({ role }: { role: 'talent'|'employer' }) {
  ...
  return <section className="mt-8 space-y-4">
    <div>
      ...
      <p className="text-sm text-secondary mt-1 max-w-3xl">You can add further information while Talent House reviews the case. When Talent House proposes a resolution, both the professional and property must sign before any settlement is applied.</p>
    </div>
    ...
    {active.map(row => {
      ...
      return <div key={row.id} className="dashboard-card">
        ...
        {(row.messages || []).length > 0 && <div className="mt-4 space-y-2">{row.messages.map((m:any) => <div key={m.id} className="bg-[#f1f1f1] border border-[#dddddd] p-3 text-sm"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted"><MessageSquare size={11}/>{m.sender_role === 'candidate' ? 'Professional' : m.sender_role === 'employer' ? 'Property' : 'Talent House Admin'}</div><p className="mt-1 whitespace-pre-wrap text-gray-700">{m.message}</p></div>)}</div>}

        {!['awaiting_payment','resolved','rejected'].includes(row.status) && <div className="mt-4"><textarea aria-label="Message about this case" className="input-field min-h-20" value={messages[row.id] || ''} onChange={e => setMessages(v => ({ ...v, [row.id]: e.target.value }))} placeholder="Add more information or reply to the other side..."/><button disabled={busy} onClick={() => sendMessage(row.id)} className="btn-secondary mt-2 text-xs">Add to case discussion</button></div>}

        {row.status === 'awaiting_agreement' && <div className="mt-5 border border-accent/25 bg-[#f1f1f1] p-4 rounded-xl"><p className="text-[10px] uppercase tracking-wider text-accent">Talent House proposed resolution</p><p className="font-medium text-ink mt-2 whitespace-pre-wrap">{row.proposed_resolution}</p><div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 text-sm"><div><span className="text-muted">Employer refund</span><p className="font-semibold">£{Number(row.proposed_refund_amount || 0)}</p></div><div><span className="text-muted">Extra professional pay</span><p className="font-semibold">£{Number(row.proposed_extra_amount || 0)}</p></div><div><span className="text-muted">Final professional payout</span><p className="font-semibold">£{Number(row.proposed_payout_amount || 0)}</p></div></div><div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"><p className="text-xs text-gray-600">By signing, you confirm that you accept these terms as the resolution of this Agency case.</p>{mySigned ? <span className="text-xs text-green-700 inline-flex items-center gap-1"><CheckCircle2 size={14}/>You have signed. {otherSigned ? 'Both sides agreed.' : 'Waiting for the other side.'}</span> : <button disabled={busy} onClick={() => agree(row.id)} className="btn-primary text-xs">I agree & sign resolution</button>}</div></div>}
      </div>
    })}
  </section>
}
