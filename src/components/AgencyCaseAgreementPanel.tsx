  ...
  return <section className="mt-8 space-y-4">
    ...
    {active.map(row => {
      ...
      return <div key={row.id} className="dashboard-card">
        ...
        {(row.messages || []).length > 0 && <div className="mt-4 space-y-2">{row.messages.map((m:any) => <div key={m.id} className="bg-[#ede8df] border border-[#dcd4c8] p-3 text-sm"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted"><MessageSquare size={11}/>{m.sender_role === 'candidate' ? 'Professional' : m.sender_role === 'employer' ? 'Property' : 'Talent House Admin'}</div><p className="mt-1 whitespace-pre-wrap text-gray-700">{m.message}</p></div>)}</div>}
