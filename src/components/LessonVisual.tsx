...
export function LessonVisualBlock({ visual }: { visual: Visual }) {
  if (visual.kind === 'flow') {
    return (
      <div className="rounded-xl border border-[#dcd4c8] bg-white p-5 my-4">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#222321] mb-3">{visual.title}</p>
        <div className="flex flex-col items-stretch gap-1">
          {visual.steps.map((step, index) => (
            <div key={index} className="flex flex-col items-center">
              <div className="w-full rounded-lg bg-[#e3dcd1] border border-[#dcd4c8] px-4 py-2.5 text-center text-[13px] font-medium text-[#222321]">{step}</div>
              {index < visual.steps.length - 1 && <ArrowDown size={14} className="text-[#222321] my-0.5" />}
            </div>
          ))}
        </div>
      ...
      <div className="rounded-xl border border-[#dcd4c8] bg-white p-5 my-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12.5px] border-collapse min-w-[420px]">
            <thead><tr>{visual.headers.map(header => <th key={header} className="border-b-2 border-[#222321]/20 bg-[#ede8df] px-3 py-2 text-left font-semibold text-ink">{header}</th>)}</tr></thead>
            <tbody>{visual.rows.map((row, ri) => <tr key={ri}>{row.map((cell, ci) => <td key={ci} className={`border-b border-[#dcd4c8] px-3 py-2 text-gray-700 ${ci > 0 ? 'tabular-nums' : ''}`}>{cell}</td>)}</tr>)}</tbody>
          </table>
        </div>
        {visual.caption && <p className="mt-3 text-[11.5px] text-secondary leading-5">{visual.caption}</p>}
      </div>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 gap-1.5">
              {visual.quadrants.map((quadrant, index) => (
                <div key={index} className={`rounded-lg px-3 py-4 text-center text-[12px] font-medium leading-5 ${index === 0 ? 'bg-[#e7f2ec] text-[#2e5b45]' : index === 3 ? 'bg-[#ede8df] text-[#222321]' : 'bg-[#ede8df] text-gray-700'}`}>{quadrant}</div>
              ))}
            </div>
  ...
  if (visual.kind === 'image') {
    return (
      <figure className="rounded-xl border border-[#dcd4c8] bg-white p-5 my-4">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#222321] mb-3">{visual.title}</p>
        ...
        <img
          ...
          loading="lazy"
          decoding="async"
          className="w-full rounded-lg border border-[#dcd4c8] object-cover"
        />
  ...
  return (
    <div className="rounded-xl border border-dashed border-[#dcd4c8] bg-[#ede8df] p-5 my-4 text-center">
      <ImageIcon size={20} className="mx-auto text-[#6e6a60] mb-2" />
      <p className="text-[12px] font-semibold text-[#222321]">{visual.title}</p>
...
export function KnowledgeCheckBlock({ checks }: { checks: KnowledgeCheck[] }) {
  ...
  return (
    <div className="rounded-xl border border-[#dcd4c8] bg-[#ede8df] p-5 my-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#222321] mb-3">Knowledge check</p>
                  ...
                  return (
                    <button key={oi} type="button" disabled={revealed}
                      ...
                      className={`w-full rounded-lg border px-3 py-2 text-left text-[12.5px] transition-colors ${revealed
                        ...
                        : 'border-[#dcd4c8] bg-white hover:border-[#222321]/40 text-gray-700'}`}>
