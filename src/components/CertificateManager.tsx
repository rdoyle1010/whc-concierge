...
export default function CertificateManager({ userId }: { userId: string | null }) {
  ...
  return (
    <div>
      ...
      <details className="mb-3 rounded-lg border border-border bg-[#ede8df] px-3 py-2">
          ...
          {certificates.map(certificate => (
            <div key={certificate.id} className="rounded-lg border border-border px-3 py-2.5">
              ...
              {(certificate.status === 'rejected' || certificate.status === 'more_info') && certificate.review_note && (
                <p className="mt-1.5 text-[11.5px] text-secondary bg-[#ede8df] rounded px-2.5 py-1.5"><ShieldQuestion size={12} className="inline mr-1 -mt-0.5" />Talent House: {certificate.review_note} <button type="button" onClick={() => openForm(certificate.document_url, certificate.title)} className="underline font-medium">Resubmit</button></p>
              )}
            </div>
          ))}
