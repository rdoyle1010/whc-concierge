...
export default function ProfileStrength({ profile }: { profile: any }) {
  ...
  const colour = score >= 80 ? '#22C55E' : score >= 50 ? '#222321' : '#e5e5e5'
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative w-14 h-14">
          <svg className="w-14 h-14 -rotate-90" viewBox="0 0 36 36">
            <circle cx="18" cy="18" r="16" fill="none" stroke="#ede8df" strokeWidth="3" />
        ...
        <div className="flex flex-wrap gap-2 pt-3 border-t border-border">
          {Array.from(missingSteps.entries()).slice(0, 3).map(([step, _label]) => (
            <Link key={step} href={`/talent/onboarding?step=${step}`}
              className="text-[11px] font-medium text-accent bg-[#ede8df] border border-accent/20 px-2.5 py-1 rounded-full hover:bg-accent/10 transition-colors flex items-center gap-1">
