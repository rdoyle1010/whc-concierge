...
export default function ProfileStrength({ profile }: { profile: any }) {
  ...
  const colour = score >= 80 ? '#22C55E' : score >= 50 ? '#1c1c1c' : '#e5e5e5'
        ...
        <div className="flex flex-wrap gap-2 pt-3 border-t border-border">
          {Array.from(missingSteps.entries()).slice(0, 3).map(([step, _label]) => (
            <Link key={step} href={`/talent/onboarding?step=${step}`}
              className="text-[11px] font-medium text-accent bg-[#f1f1f1] border border-accent/20 px-2.5 py-1 rounded-full hover:bg-accent/10 transition-colors flex items-center gap-1">
