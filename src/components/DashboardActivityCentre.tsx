...
export default function DashboardActivityCentre({ role, consultantOnly = false }: { role: Role; consultantOnly?: boolean }) {
  ...
  return <>
    <section className="dashboard-card mb-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="dashboard-eyebrow !mb-1">Activity centre</p><h2 className="dashboard-section-title">What needs your attention</h2><p className="mt-1 text-[12px] text-muted">Important recruitment activity and conversations, all in one place.</p></div>{data.unreadNotifications > 0 && <span className="inline-flex items-center gap-2 rounded-full bg-[#ede8df] px-3 py-1.5 text-[11px] font-semibold text-[#222321]"><Bell size={13}/>{data.unreadNotifications} new update{data.unreadNotifications === 1 ? '' : 's'}</span>}</div>
      {hasAttention ? <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{data.unreadMessages > 0 && <Link href={`/${role}/messages`} className="rounded-2xl border border-[#dcd4c8] bg-[#ede8df] p-4 transition hover:border-[#57544c]"><div className="flex items-center justify-between"><span className="text-[#222321]"><MessageSquare size={17}/></span><span className="text-2xl font-serif text-[#222321]">{data.unreadMessages}</span></div><p className="mt-3 text-[12px] font-semibold text-ink">Unread messages</p><p className="mt-1 text-[11px] text-muted">Open conversations</p></Link>}{data.attention.map(item => { const tile = toneClasses(item.tone); const word = activityWord(item.tone); return <Link key={`${item.label}-${item.href}`} href={item.href} className={`rounded-2xl border border-[#dcd4c8] bg-[#ede8df] p-4 transition hover:border-[#57544c] ${tile.card}`}><div className="flex items-center justify-between"><span className={item.tone === 'action' ? 'text-red-700' : item.tone === 'done' ? 'text-emerald-700' : 'text-[#222321]'}>{item.label.toLowerCase().includes('interview') ? <CalendarDays size={17}/> : item.label.toLowerCase().includes('hire') || item.label.toLowerCase().includes('offer') ? <CheckCircle2 size={17}/> : <BriefcaseBusiness size={17}/>}</span><span className="text-2xl font-serif text-[#222321]">{item.count}</span></div><p className="mt-3 text-[12px] font-semibold text-ink">{item.label}</p><div className="mt-1 flex items-center justify-between gap-2"><p className="inline-flex items-center gap-1 text-[11px] text-[#222321]">Review now <ArrowRight size={11}/></p>{word && <span className={`px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[.08em] ${tile.pill}`}>{word}</span>}</div></Link> })}</div> : <div className="mt-5 flex items-center gap-3 rounded-2xl border border-[#dcd4c8] bg-[#ede8df] px-4 py-4"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><CheckCircle2 size={17}/></div><div><p className="text-[12px] font-semibold text-ink">You are up to date</p><p className="mt-0.5 text-[11px] text-muted">There are no urgent recruitment actions waiting right now.</p></div></div>}
      <div className="mt-6 border-t border-[#dcd4c8] pt-5"><div className="mb-3 flex items-center gap-2"><Sparkles size={14} className="text-[#222321]"/><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#222321]">Recent activity</p></div>{data.recent.length === 0 ? <p className="text-[12px] text-muted">No recent updates yet.</p> : <div>{data.recent.slice(0,5).map(item => {
      ...
      <div className="mt-6 border-t border-[#dcd4c8] pt-5"><div className="mb-1 flex items-center gap-2"><Sparkles size={14} className="text-[#222321]"/><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#222321]">Explore Talent House</p></div><p className="mb-4 text-[12px] text-muted">Quick links into the public platform without signing out of your account.</p><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{exploreLinks.map(link => <Link key={link.href} href={link.href} className="group flex items-start gap-3 rounded-xl border border-[#dcd4c8] bg-white p-3.5 transition hover:border-[#6e6a60] hover:bg-[#ede8df]"><span className="mt-0.5 text-[#222321]">{link.icon}</span><span className="min-w-0"><span className="flex items-center gap-1 text-[12px] font-semibold text-ink">{link.label}<ArrowRight size={11} className="opacity-45 transition group-hover:translate-x-0.5 group-hover:opacity-100"/></span><span className="mt-1 block text-[10.5px] leading-4 text-muted">{link.note}</span></span></Link>)}</div></div>
    </section>
  </>
}
