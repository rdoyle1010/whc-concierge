...
export default function DashboardActivityCentre({ role }: { role: Role }) {
  ...
  return <>
    <Link href="/" className="mb-5 flex flex-col gap-3 rounded-2xl bg-[#111111] px-5 py-5 text-white shadow-sm transition hover:bg-[#333333] sm:flex-row sm:items-center sm:justify-between">
    ...
    <section className="dashboard-card mb-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="dashboard-eyebrow !mb-1">Activity centre</p><h2 className="dashboard-section-title">What needs your attention</h2><p className="mt-1 text-[12px] text-muted">Important recruitment activity and conversations, all in one place.</p></div>{data.unreadNotifications > 0 && <span className="inline-flex items-center gap-2 rounded-full bg-[#f5f5f5] px-3 py-1.5 text-[11px] font-semibold text-[#1a1a1a]"><Bell size={13}/>{data.unreadNotifications} new update{data.unreadNotifications === 1 ? '' : 's'}</span>}</div>
      {hasAttention ? <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{data.unreadMessages > 0 && <Link href={`/${role}/messages`} className="rounded-2xl border border-[#ded8cc] bg-[#fafafa] p-4 transition hover:border-[#555555]"><div className="flex items-center justify-between"><span className="text-[#111111]"><MessageSquare size={17}/></span><span className="text-2xl font-serif text-[#17344d]">{data.unreadMessages}</span></div><p className="mt-3 text-[12px] font-semibold text-ink">Unread messages</p><p className="mt-1 text-[11px] text-muted">Open conversations</p></Link>}{data.attention.map(item => <Link key={`${item.label}-${item.href}`} href={item.href} className="rounded-2xl border border-[#ded8cc] bg-[#fafafa] p-4 transition hover:border-[#555555]"><div className="flex items-center justify-between"><span className="text-[#1a1a1a]">{item.label.toLowerCase().includes('interview') ? <CalendarDays size={17}/> : item.label.toLowerCase().includes('hire') || item.label.toLowerCase().includes('offer') ? <CheckCircle2 size={17}/> : <BriefcaseBusiness size={17}/>}</span><span className="text-2xl font-serif text-[#17344d]">{item.count}</span></div><p className="mt-3 text-[12px] font-semibold text-ink">{item.label}</p><p className="mt-1 inline-flex items-center gap-1 text-[11px] text-[#1a1a1a]">Review now <ArrowRight size={11}/></p></Link>)}</div> : <div className="mt-5 flex items-center gap-3 rounded-2xl border border-[#e4ddd1] bg-[#fafafa] px-4 py-4"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><CheckCircle2 size={17}/></div><div><p className="text-[12px] font-semibold text-ink">You are up to date</p><p className="mt-0.5 text-[11px] text-muted">There are no urgent recruitment actions waiting right now.</p></div></div>}
      <div className="mt-6 border-t border-[#e8e2d8] pt-5"><div className="mb-3 flex items-center gap-2"><Sparkles size={14} className="text-[#1a1a1a]"/><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#1a1a1a]">Recent activity</p></div>{data.recent.length === 0 ? <p className="text-[12px] text-muted">No recent updates yet.</p> : <div>{data.recent.slice(0,5).map(item => { const content = <div className="dashboard-list-row !py-3"><div className="flex min-w-0 items-start gap-3"><span className={`mt-0.5 ${item.is_read ? 'text-muted' : 'text-[#1a1a1a]'}`}>{iconFor(item.type)}</span><div className="min-w-0"><p className={`truncate text-[12px] ${item.is_read ? 'font-medium text-ink' : 'font-semibold text-ink'}`}>{item.title}</p><p className="mt-0.5 line-clamp-1 text-[11px] text-muted">{item.message}</p></div></div><span className="shrink-0 text-[10px] text-muted">{relativeTime(item.created_at)}</span></div>; return item.link ? <Link key={item.id} href={item.link}>{content}</Link> : <div key={item.id}>{content}</div> })}</div>}</div>
      <div className="mt-6 border-t border-[#e8e2d8] pt-5"><div className="mb-1 flex items-center gap-2"><Sparkles size={14} className="text-[#111111]"/><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#111111]">Explore Wellness House</p></div><p className="mb-4 text-[12px] text-muted">Quick links into the public platform without signing out of your account.</p><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{exploreLinks.map(link => <Link key={link.href} href={link.href} className="group flex items-start gap-3 rounded-xl border border-[#e2e6e8] bg-white p-3.5 transition hover:border-[#b9c5cb] hover:bg-[#fafbfb]"><span className="mt-0.5 text-[#111111]">{link.icon}</span><span className="min-w-0"><span className="flex items-center gap-1 text-[12px] font-semibold text-ink">{link.label}<ArrowRight size={11} className="opacity-45 transition group-hover:translate-x-0.5 group-hover:opacity-100"/></span><span className="mt-1 block text-[10.5px] leading-4 text-muted">{link.note}</span></span></Link>)}</div></div>
    </section>
  </>
}
