...
export default function PlatformExperienceReview({ applicationId }: { applicationId: string }) {
  ...
  if (reviewed) {
    return (
      <div className="rounded-2xl border border-[#dcd4c8] bg-[#ede8df] p-5">
        <p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#222321]">Talent House Collective experience</p>
        <div className="mt-3 flex items-center gap-1">
          {[1,2,3,4,5].map(i => <Star key={i} size={16} className={i <= rating ? 'fill-amber-400 text-amber-400' : 'text-gray-200'} />)}
        </div>
        <p className="mt-2 text-[13px] font-medium text-ink">Thank you for reviewing your Talent House Collective experience.</p>
        {googleReviewUrl && <a href={googleReviewUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-medium text-[#222321] hover:underline">Share your experience on Google <ExternalLink size={12}/></a>}
      </div>
    )
  }
