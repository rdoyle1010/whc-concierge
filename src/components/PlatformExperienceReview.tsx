...
export default function PlatformExperienceReview({ applicationId }: { applicationId: string }) {
  ...
  if (reviewed) {
    return (
      <div className="rounded-2xl border border-[#e4ddd1] bg-[#fafafa] p-5">
        <p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#1a1a1a]">Spa Platform experience</p>
        ...
        {googleReviewUrl && <a href={googleReviewUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-medium text-[#1a1a1a] hover:underline">Share your experience on Google <ExternalLink size={12}/></a>}
      </div>
    )
  }
