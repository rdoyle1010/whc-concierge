...
const config: Record<string,{icon:React.ReactNode;color:string;bg:string;label:string}> = {
  draft:{icon:<PencilLine size={14}/>,color:'text-[#1c1c1c]',bg:'bg-[#f1f1f1]',label:'Ready to Send'},
  ...
  interview:{icon:<MessageSquare size={14}/>,color:'text-[#1c1c1c]',bg:'bg-[#e7e7e7]',label:'Interview'},
  offered:{icon:<Briefcase size={14}/>,color:'text-accent',bg:'bg-[#f1f1f1]',label:'Offered'},
...
export default function TalentApplicationsWorkspace(){
  ...
  const JobAndMatch=({app,compact=false}:{app:any;compact?:boolean})=>{
    ...
    return <div className={`${compact?'mt-5':'mt-4'} space-y-4`}>
      ...
      <div className="rounded-xl border border-border p-4">
        <p className="text-[10px] uppercase tracking-[.14em] text-[#1c1c1c] font-semibold mb-2">Job description</p>
        ...
        <Link href={`/jobs/${job.id}`} className="mt-3 inline-flex text-[12px] font-semibold text-[#1c1c1c] hover:underline">View full role →</Link>
      </div>
  ...
  return <>
    ...
    {draft&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onClick={()=>!sending&&!keeping&&!aiLoading&&keepDraft()}><div {...draftDialog.panelProps} className="w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white p-6"><div className="flex justify-between gap-4 mb-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-[#1c1c1c] font-semibold">Review before sending</p><h2 id="talent-applications-draft-dialog-heading" className="text-[22px] font-semibold text-ink mt-1">{draft.job_listings?.job_title}</h2><p className="text-[12px] text-muted">{draft.job_listings?.employer_profiles?.property_name||draft.job_listings?.employer_profiles?.company_name}</p></div><button type="button" onClick={keepDraft} disabled={sending||keeping||!!aiLoading} aria-label="Close" className="p-2 -m-2"><X size={20}/></button></div><JobAndMatch app={draft} compact/>
      ...
      <div className="mt-6 rounded-2xl border border-[#dddddd] bg-[#f1f1f1] p-5">
        <div className="flex items-start gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#1c1c1c] text-white"><Sparkles size={16}/></div>
        ...
        {aiResult&&<div className="mt-4 border-t border-[#dddddd] pt-4 space-y-3">
          {aiResult.summary&&<div><p className="text-[10px] uppercase tracking-[.12em] font-semibold text-[#1c1c1c]">Application strength</p><p className="mt-1 text-[12px] leading-5 text-secondary">{aiResult.summary}</p></div>}
