...
// Exactly what leaves the profile when an application is sent. Kept in step
// with CANDIDATE_FIELDS in src/app/api/employer/applications/inbox/route.ts,
// which is the query the employer's inbox actually runs.
const SHARED_WITH_EMPLOYER = [
  'Your name, headline, role level, location and profile photograph',
  'Your bio, career evidence, desired roles and portfolio link',
  'Treatments and skills, qualifications, product houses, systems, business skills and commercial experience',
  'Years of experience, team size managed, revenue responsibility and availability',
  'Whether you hold your own insurance, your Talent House review score, verified certificates and Academy courses',
  'Your covering letter and the fit score Talent House calculated for this role',
]
...
const config: Record<string,{icon:React.ReactNode;color:string;bg:string;label:string}> = {
  draft:{icon:<PencilLine size={14}/>,color:'text-[#222321]',bg:'bg-[#ede8df]',label:'Ready to Send'},
  ...
  interview:{icon:<MessageSquare size={14}/>,color:'text-[#222321]',bg:'bg-[#e3dcd1]',label:'Interview'},
  offered:{icon:<Briefcase size={14}/>,color:'text-accent',bg:'bg-[#ede8df]',label:'Offered'},
...
export default function TalentApplicationsWorkspace(){
  ...
  const [submitted,setSubmitted]=useState<{jobTitle:string;property:string;jobId:string|null}|null>(null)
  ...
  const draftDialog=useDialog(()=>{if(!sending&&!aiLoading)setDraft(null)},'talent-applications-draft-dialog-heading',{enabled:Boolean(draft)})
  const submittedDialog=useDialog(()=>setSubmitted(null),'talent-applications-submitted-dialog-heading',{enabled:Boolean(submitted)})
  const reviewDialog=useDialog(()=>setReviewing(null),'talent-applications-review-dialog-heading',{enabled:Boolean(reviewing)})
  ...
  useEffect(()=>{loadApplications()},[page,perPage,filter]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // "Keep as Draft" closed the modal and did nothing else, so every word of a
  // covering letter was discarded by the button whose whole promise was to
  // keep it. Same loss on the backdrop, Escape, a refresh or a closed tab.
  const [keeping,setKeeping]=useState(false)
  async function keepDraft(){
    if(!draft||keeping){setDraft(null);return}
    const jobId=draft.job_listings?.id||draft.job_id||draft.role_id
    if(!jobId){setDraft(null);return}
    setKeeping(true);setSendError('')
    const r=await fetch('/api/applications/draft',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jobId,coverLetter:letter})}).catch(()=>null)
    setKeeping(false)
    if(!r?.ok){
      const j=r?await r.json().catch(()=>({})):{}
      setSendError(j.error||'Your letter could not be saved. Copy it somewhere safe before closing.')
      return
    }
    setDraft(null)
  ...
  async function send(){if(!draft||sending)return;setSending(true);setSendError('');const sentDraft=draft;const r=await fetch('/api/applications/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({applicationId:draft.id,coverLetter:letter})}).catch(()=>null);const j=r?await r.json().catch(()=>({})):{};setSending(false);if(!r?.ok){setSendError(j.error||'Could not send your application.');return}setDraft(null);setSubmitted({jobTitle:sentDraft.job_listings?.job_title||'your role',property:sentDraft.job_listings?.employer_profiles?.property_name||sentDraft.job_listings?.employer_profiles?.company_name||'the property',jobId:sentDraft.job_listings?.id||sentDraft.job_id||sentDraft.role_id||null});await loadApplications()}
  ...
  const JobAndMatch=({app,compact=false}:{app:any;compact?:boolean})=>{
    ...
    return <div className={`${compact?'mt-5':'mt-4'} space-y-4`}>
      ...
      <div className="rounded-xl border border-border p-4">
        <p className="text-[10px] uppercase tracking-[.14em] text-[#222321] font-semibold mb-2">Job description</p>
        ...
        <Link href={`/jobs/${job.id}`} className="mt-3 inline-flex text-[12px] font-semibold text-[#222321] hover:underline">View full role →</Link>
      </div>
      {match&&<div className="rounded-xl border border-[#dcd4c8] bg-[#ede8df] p-4">
        <div className="flex items-start justify-between gap-3 mb-3"><div><p className="text-[10px] uppercase tracking-[.14em] text-[#222321] font-semibold">Why you match</p><p className="mt-1 text-[13px] leading-5 text-secondary">{match.matchExplanation||'Your profile has been compared with the requirements of this role.'}</p></div><span className="rounded-full bg-white border border-[#dcd4c8] px-3 py-1 text-[13px] font-semibold text-ink shrink-0">{match.score}%</span></div>
        {match.matchingSkills?.length>0&&<div className="flex flex-wrap gap-1.5 mb-3">{match.matchingSkills.slice(0,6).map((s:string)=><span key={s} className="rounded-full border border-[#dcd4c8] bg-white px-2.5 py-1 text-[10px] font-medium text-[#222321]">✓ {s}</span>)}</div>}
  ...
  return <>
    <div className="flex items-center justify-between mb-6"><div><h1 className="text-2xl font-semibold tracking-tight text-ink">My Applications</h1><p className="mt-1 text-[13px] text-secondary">Matched roles stay private until you review the role, your match and your covering letter, then press Send Application.</p></div><Link href="/roles/match" className="btn-secondary text-[12px] flex items-center gap-1">Find Matches <ArrowRight size={12}/></Link></div>
    ...
    {loading?<div className="h-64 flex items-center justify-center"><div className="animate-spin w-8 h-8 border-2 border-ink border-t-transparent rounded-full"/></div>:error?<div className="dashboard-card text-center py-12 text-[13px] text-red-600">{error}</div>:counts.all===0&&!(counts.draft||0)?<div className="dashboard-card text-center py-16"><FileText size={40} className="mx-auto mb-3 text-muted/40"/><p className="text-[15px] font-medium text-ink mb-1">Nothing saved yet</p><p className="text-[13px] text-muted mb-6">Use Match to find roles that fit your experience.</p><Link href="/roles/match" className="btn-primary inline-flex items-center gap-1.5">Find Matches <ArrowRight size={13}/></Link></div>:!apps.length?<div className="dashboard-card text-center py-12 text-[13px] text-muted">No applications with this status.</div>:<div className="space-y-3">{apps.map(app=>{const c=app.offer_declined?{icon:config.rejected.icon,color:'text-[#57544c]',bg:'bg-[#ede8df]',label:'Offer declined by you'}:config[app.status]||config.pending;const property=app.job_listings?.employer_profiles?.property_name||app.job_listings?.employer_profiles?.company_name||'';const terminal=['accepted','rejected'].includes(app.status);const isDraft=app.status==='draft';const closed=app.job_listings&&(!app.job_listings.is_live||(app.job_listings.expires_at&&new Date(app.job_listings.expires_at).getTime()<=Date.now()));const current=ACTIVE_FLOW.indexOf(app.status);const open=!!expanded[app.id];return <div key={app.id} className={`bg-white border rounded-xl p-5 ${isDraft?'border-[#57544c]':'border-border'}`}><div className="flex items-start justify-between gap-4 mb-3"><div><h3 className="text-[16px] font-medium text-ink">{app.job_listings?.job_title||'Role no longer available'}</h3><p className="text-[13px] text-muted">{property}{app.job_listings?.location?` · ${app.job_listings.location}`:''}</p></div><span className={`flex items-center gap-1.5 text-[12px] font-medium px-2.5 py-1 rounded-full ${c.bg} ${c.color}`}>{c.icon}{c.label}</span></div>{isDraft?<div className="rounded-xl bg-[#ede8df] border border-[#dcd4c8] p-4 mb-4"><p className="text-[12px] font-semibold text-ink mb-1">Not sent to the property yet</p><p className="text-[12px] leading-5 text-muted">Check the full role and why Talent House matched you before deciding whether to send it.</p></div>:!terminal?<div className="mb-4"><div className="flex items-center gap-1">{ACTIVE_FLOW.map((s,i)=><div key={s} className="flex items-center flex-1 last:flex-none"><div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-semibold ${i<=current?'bg-ink text-white':'bg-surface text-muted'}`}>{i+1}</div>{i<ACTIVE_FLOW.length-1&&<div className={`flex-1 h-[2px] mx-1 ${i<current?'bg-ink':'bg-border'}`}/>}</div>)}</div><div className="mt-1 grid grid-cols-5 gap-1 text-center text-[9px] leading-3 text-muted">{ACTIVE_FLOW.map(s=><span key={s}>{config[s].label}</span>)}</div></div>:null}{['shortlisted','interview'].includes(app.status)&&(app.job_listings?.id||app.job_id||app.role_id)&&<div className="mb-4 border-l-2 border-[#222321] bg-[#ede8df] p-4"><p className="text-[12px] font-semibold text-ink mb-1">{app.status==='interview'?'You have an interview - be the best-prepared person they meet.':'You are shortlisted - the interview is the next step.'}</p><div className="mt-1 flex flex-wrap items-center gap-2"><Link href={`/talent/interview-ready?job=${app.job_listings?.id||app.job_id||app.role_id}`} className="btn-primary inline-block !py-2 !px-3 text-[12px]">Practise the questions you are most likely to be asked</Link>{/* Something always changes before an interview. The message link lived
                only on the recruitment-progress card higher up the page, which
                is not where somebody reading their own application looks. */}
                {app.job_listings?.employer_profiles?.user_id&&<Link href={`/talent/messages?to=${app.job_listings.employer_profiles.user_id}`} className="btn-secondary inline-flex items-center gap-1.5 !py-2 !px-3 text-[12px]"><MessageSquare size={12}/>Message {property||'the property'}</Link>}</div></div>}<button type="button" onClick={()=>toggleDetails(app)} className="mb-4 inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#222321] hover:underline"><ChevronDown size={13} className={open?'rotate-180 transition-transform':'transition-transform'}/>{open?'Hide role & match':'View role & match'}</button>{open&&<JobAndMatch app={apps.find(x=>x.id===app.id)||app}/>}<div className="flex flex-wrap items-center gap-3 text-[11px] text-muted mt-4"><span>{isDraft?'Saved':'Applied'} {new Date(app.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</span>{app.match_score&&<span>· {app.match_score}% match</span>}{isDraft&&!closed&&<button type="button" onClick={()=>edit(app)} className="ml-auto btn-primary !py-2 !px-3 inline-flex items-center gap-1.5"><PencilLine size={12}/>Review & Send</button>}{isDraft&&closed&&<span className="ml-auto text-red-500 font-medium">Role closed</span>}{app.status==='accepted'&&app.job_listings?.employer_profiles?.user_id&&<button type="button" onClick={()=>setReviewing({userId:app.job_listings.employer_profiles.user_id,name:property||'this employer'})} className="inline-flex items-center gap-1 font-medium text-amber-500"><Star size={11}/>Review employer</button>}{!terminal&&<button type="button" onClick={()=>remove(app)} className={`${isDraft?'':'ml-auto'} inline-flex items-center gap-1 font-semibold text-red-500`}><X size={12}/>{isDraft?'Remove':'Withdraw application'}</button>}</div></div>})}<Pagination page={page} perPage={perPage} total={total} onPageChange={setPage} onPerPageChange={(next)=>{setPerPage(next);setPage(1)}}/></div>}
    {draft&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onClick={()=>!sending&&!keeping&&!aiLoading&&keepDraft()}><div {...draftDialog.panelProps} className="w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white p-6"><div className="flex justify-between gap-4 mb-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-[#222321] font-semibold">Review before sending</p><h2 id="talent-applications-draft-dialog-heading" className="text-[22px] font-semibold text-ink mt-1">{draft.job_listings?.job_title}</h2><p className="text-[12px] text-muted">{draft.job_listings?.employer_profiles?.property_name||draft.job_listings?.employer_profiles?.company_name}</p></div><button type="button" onClick={keepDraft} disabled={sending||keeping||!!aiLoading} aria-label="Close" className="p-2 -m-2"><X size={20}/></button></div><JobAndMatch app={draft} compact/>

      <div className="mt-6 border-l-2 border-[#222321] bg-[#ede8df] px-5 py-4">
        <p className="text-[12px] font-semibold text-ink">What {draft.job_listings?.employer_profiles?.property_name||draft.job_listings?.employer_profiles?.company_name||'the property'} will see</p>
        <ul className="mt-2 space-y-1 text-[11.5px] leading-5 text-secondary">
          {SHARED_WITH_EMPLOYER.map(item=><li key={item}>- {item}</li>)}
        </ul>
        <p className="mt-3 text-[11px] leading-4 text-muted">Your salary expectation stays hidden unless you have chosen to show it. Your CV and documents open to a property only where you started the relationship - this application, a mutual match, or an Agency booking. Being shortlisted alone does not open them.</p>
      </div>

      <div className="mt-6 rounded-2xl border border-[#dcd4c8] bg-[#ede8df] p-5">
        <div className="flex items-start gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#222321] text-white"><Sparkles size={16}/></div>
          <div className="flex-1"><p className="text-[13px] font-semibold text-ink">Talent House AI Application Assistant</p><p className="mt-1 text-[12px] leading-5 text-muted">Uses your Talent House profile and this role to help you present your real experience clearly. It will not invent qualifications or send anything for you.</p></div>
        </div>
        ...
        {aiError&&<div role="alert" className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-[11px] text-red-600">{aiError}</div>}
        {aiResult&&<div className="mt-4 border-t border-[#dcd4c8] pt-4 space-y-3">
          {aiResult.summary&&<div><p className="text-[10px] uppercase tracking-[.12em] font-semibold text-[#222321]">Application strength</p><p className="mt-1 text-[12px] leading-5 text-secondary">{aiResult.summary}</p></div>}
      ...
      <div className="mt-6 border-t border-border pt-5"><label className="block text-[12px] font-semibold text-ink mb-2">Covering letter <span className="font-normal text-muted">(optional)</span></label><textarea aria-label="Cover letter" value={letter} onChange={e=>setLetter(e.target.value)} rows={9} maxLength={5000} className="input-field resize-y text-[13px] leading-6" placeholder="Tell the property why this role interests you and what you would bring to the team."/><div className="mt-2 flex justify-between text-[11px] text-muted"><span>You can edit every word before sending. AI suggestions are never submitted automatically.</span><span>{letter.length}/5000</span></div></div>{sendError&&<div className="mt-4 rounded-lg bg-red-50 px-3 py-2 text-[12px] text-red-600">{sendError}</div>}<div className="mt-6 flex justify-end gap-2"><button type="button" onClick={keepDraft} disabled={sending||keeping||!!aiLoading} className="btn-secondary">{keeping?'Saving…':'Keep as Draft'}</button><button type="button" onClick={send} disabled={sending||!!aiLoading} className="btn-primary inline-flex items-center gap-2"><Send size={14}/>{sending?'Sending…':'Send Application'}</button></div></div></div>}
    {submitted&&<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4"><div {...submittedDialog.panelProps} className="w-full max-w-xl bg-white p-7"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600"><CheckCircle size={26}/></div><p className="mt-5 text-[10px] uppercase tracking-[.16em] text-[#222321] font-semibold">Application submitted</p><h2 id="talent-applications-submitted-dialog-heading" className="mt-1 text-[27px] font-semibold tracking-[-.03em] text-ink">Thank you - your application has been submitted.</h2><p className="mt-3 text-[13px] leading-6 text-muted">Your application for <span className="font-semibold text-ink">{submitted.jobTitle}</span> has been sent to {submitted.property}. You can follow its progress here as the property reviews it.</p><div className="mt-7 rounded-2xl border border-border bg-[#ede8df] p-5"><div className="flex items-center gap-1">{ACTIVE_FLOW.map((s,i)=><div key={s} className="flex items-center flex-1 last:flex-none"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold ${i===0?'bg-ink text-white':'bg-white border border-border text-muted'}`}>{i+1}</div>{i<ACTIVE_FLOW.length-1&&<div className="mx-1 h-[2px] flex-1 bg-border"/>}</div>)}</div><div className="mt-2 grid grid-cols-5 gap-1 text-center text-[9px] leading-3 text-muted">{ACTIVE_FLOW.map(s=><span key={s}>{config[s].label}</span>)}</div></div><div className="mt-6 flex flex-col gap-2 sm:flex-row">{submitted.jobId&&<Link href={`/talent/interview-ready?job=${submitted.jobId}`} onClick={()=>setSubmitted(null)} className="btn-primary flex-1 text-center">Build your interview plan</Link>}<button type="button" onClick={()=>setSubmitted(null)} className={`${submitted.jobId?'btn-secondary':'btn-primary'} flex-1`}>View my applications</button><Link href="/roles/match" onClick={()=>setSubmitted(null)} className="btn-secondary flex-1 text-center">Find another role</Link></div></div></div>}
    {reviewing&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={()=>setReviewing(null)}><div {...reviewDialog.panelProps} className="bg-white max-w-md w-full max-h-[90vh] overflow-y-auto p-6"><div className="flex items-center justify-between mb-4"><h2 id="talent-applications-review-dialog-heading" className="text-lg font-bold text-ink">Review {reviewing.name}</h2><button type="button" onClick={()=>setReviewing(null)} aria-label="Close" className="p-2 -m-2"><X size={20}/></button></div><ReviewForm reviewedId={reviewing.userId} reviewedName={reviewing.name} type="employer"/></div></div>}
  </>
}
