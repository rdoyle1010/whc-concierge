...
export default function TalentApplicationsWorkspace(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const {data:profile}=await supabase.from('candidate_profiles').select('*').eq('user_id',user.id).single(); if(!profile){setLoading(false);return}
    const {data,error}=await supabase.from('applications').select('*, job_listings(*)').eq('candidate_id',profile.id).order('created_at',{ascending:false})
    ...
    let rows=(data||[]).map((a:any)=>{
      if(!a.job_listings)return a
      const job={...a.job_listings,title:a.job_listings.job_title||a.job_listings.title,description:a.job_listings.job_description||a.job_listings.description,required_product_houses:a.job_listings.required_brands||a.job_listings.required_product_houses}
      const match=calculateMatchScore(profile,job)
      return {...a,job_listings:job,match_detail:match}
    })
