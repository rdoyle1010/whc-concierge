...
import { FileText, Clock, CheckCircle, XCircle, Star, Eye, MessageSquare, Briefcase, ArrowRight, X, Send, PencilLine, ChevronDown, MapPin, Banknote, BriefcaseBusiness } from 'lucide-react'
...
const APPLICATION_SELECT = `
  id,status,match_score,cover_note,cover_letter,created_at,updated_at,submitted_at,role_id,job_id,
  job_listings(
    id,job_title,job_description,location,salary_min,salary_max,employer_id,is_live,expires_at,
    contract_type,job_type,required_role_level,required_skills,required_brands,required_qualifications,
    min_years_experience,preferred_business_skills,required_systems,latitude,longitude,radius_miles,
    shift_pattern,offers_accommodation,insurance_required,is_agency_role
  )
`
...
export default function TalentApplicationsWorkspace(){
  ...
  useEffect(()=>{let active=true;(async()=>{
    const {data:sessionData}=await supabase.auth.getSession(); const user=sessionData.session?.user; if(!user){if(active)setLoading(false);return}
    const {data:candidate}=await supabase.from('candidate_profiles').select('*').eq('user_id',user.id).single(); if(!candidate){if(active)setLoading(false);return}
    const {data,error}=await supabase.from('applications').select(APPLICATION_SELECT).eq('candidate_id',candidate.id).order('created_at',{ascending:false}).limit(APPLICATION_LIMIT)
    if(!active)return
    if(error){setError('We could not load your applications just now. Please refresh the page.');setLoading(false);return}
    let rows=(data||[]).map((a:any)=>a.job_listings?{...a,job_listings:{...a.job_listings,title:a.job_listings.job_title,description:a.job_listings.job_description,required_product_houses:a.job_listings.required_brands}}:a)
    ...
    if(!active)return
    setProfile(candidate);setApps(rows);setLoading(false)
  })();return()=>{active=false}},[])
  ...
  function edit(app:any){const ready=withMatch(app);setDraft(ready);setLetter(app.cover_letter||app.cover_note||'');setSendError('')}
