...
const config: Record<string,{icon:React.ReactNode;color:string;bg:string;label:string}> = {
  draft:{icon:<PencilLine size={14}/>,color:'text-[#10283b]',bg:'bg-[#f5f6f8]',label:'Ready to Send'},
  ...
  interview:{icon:<MessageSquare size={14}/>,color:'text-[#0b2f4d]',bg:'bg-[#e8eef4]',label:'Interview'},
  offered:{icon:<Briefcase size={14}/>,color:'text-accent',bg:'bg-[#f5f6f8]',label:'Offered'},
...
export default function TalentApplicationsWorkspace(){
  ...
  const JobAndMatch=({app,compact=false}:{app:any;compact?:boolean})=>{
    ...
    return <div className={`${compact?'mt-5':'mt-4'} space-y-4`}>
      ...
      <div className="rounded-xl border border-border p-4">
        <p className="text-[10px] uppercase tracking-[.14em] text-[#10283b] font-semibold mb-2">Job description</p>
      ...
      {match&&<div className="rounded-xl border border-[#e3e7eb] bg-[#f5f6f8] p-4">
        <div className="flex items-start justify-between gap-3 mb-3"><div><p className="text-[10px] uppercase tracking-[.14em] text-[#10283b] font-semibold">Why you match</p><p className="mt-1 text-[13px] leading-5 text-secondary">{match.matchExplanation||'Your profile has been compared with the requirements of this role.'}</p></div><span className="rounded-full bg-white border border-[#e3e7eb] px-3 py-1 text-[13px] font-semibold text-ink shrink-0">{match.score}%</span></div>
        {match.matchingSkills?.length>0&&<div className="flex flex-wrap gap-1.5 mb-3">{match.matchingSkills.slice(0,6).map((s:string)=><span key={s} className="rounded-full border border-[#e3e7eb] bg-white px-2.5 py-1 text-[10px] font-medium text-[#10283b]">✓ {s}</span>)}</div>}
  ...
  return <>
    ...
    {draft&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onClick={()=>!sending&&!aiLoading&&setDraft(null)}><div {...draftDialog.panelProps} className="w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white p-6"><div className="flex justify-between gap-4 mb-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-[#10283b] font-semibold">Review before sending</p><h2 id="talent-applications-draft-dialog-heading" className="text-[22px] font-semibold text-ink mt-1">{draft.job_listings?.job_title}</h2><p className="text-[12px] text-muted">{draft.job_listings?.employer_profiles?.property_name||draft.job_listings?.employer_profiles?.company_name}</p></div><button type="button" onClick={()=>setDraft(null)} disabled={sending||!!aiLoading} aria-label="Close" className="p-2 -m-2"><X size={20}/></button></div><JobAndMatch app={draft} compact/>
      ...
      <div className="mt-6 rounded-2xl border border-[#e3e7eb] bg-[#f5f6f8] p-5">
        ...
        {aiResult&&<div className="mt-4 border-t border-[#e3e7eb] pt-4 space-y-3">
          {aiResult.summary&&<div><p className="text-[10px] uppercase tracking-[.12em] font-semibold text-[#10283b]">Application strength</p><p className="mt-1 text-[12px] leading-5 text-secondary">{aiResult.summary}</p></div>}
