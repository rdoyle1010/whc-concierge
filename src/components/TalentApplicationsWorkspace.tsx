...
export default function TalentApplicationsWorkspace(){
  ...
  return <>
    ...
    {draft&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onClick={()=>!sending&&!aiLoading&&setDraft(null)}><div {...draftDialog.panelProps} className="w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white p-6"><div className="flex justify-between gap-4 mb-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-[#1c1c1c] font-semibold">Review before sending</p><h2 id="talent-applications-draft-dialog-heading" className="text-[22px] font-semibold text-ink mt-1">{draft.job_listings?.job_title}</h2><p className="text-[12px] text-muted">{draft.job_listings?.employer_profiles?.property_name||draft.job_listings?.employer_profiles?.company_name}</p></div><button type="button" onClick={()=>setDraft(null)} disabled={sending||!!aiLoading} aria-label="Close" className="p-2 -m-2"><X size={20}/></button></div><JobAndMatch app={draft} compact/>
