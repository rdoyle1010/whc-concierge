...
export default function AgencyResolutionCentre({ role }: { role: 'talent'|'employer' }) {
  ...
  const openingDialog = useDialog(() => setOpening(null), 'agency-resolution-case-heading', { enabled: Boolean(opening) })
  ...
  async function openCase() {
    ...
    if (!res.ok) setError(j.error || 'Could not open this case.')
      ...
      setNotice(role === 'talent' ? 'Your issue has been sent to Talent House. Any affected payout will be protected while the case is reviewed.' : 'Case opened. The payout is on hold while the professional responds and Talent House reviews the issue.')
      setOpening(null)
  ...
  async function respond(caseId: string) {
    ...
    else { setNotice('Response sent to Talent House for review.'); await load() }
    setBusy(false)
  }
  ...
  return <div className="space-y-7">
    ...
    {cases.length > 0 && <section><h2 className="text-lg font-medium text-ink mb-3">Cases</h2><div className="space-y-4">{cases.map(row => <div key={row.id} className="dashboard-card"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-accent">Case {row.id.slice(0,8).toUpperCase()}</p><h3 className="font-medium text-ink mt-1">{ALL_ISSUE_OPTIONS.find(x => x[0] === row.issue_type)?.[1] || row.issue_type}</h3><p className="text-sm text-gray-600 mt-2 whitespace-pre-wrap">{row.description}</p></div><span className="text-xs bg-gray-100 px-2.5 py-1 rounded-full capitalize">{String(row.status).replace(/_/g,' ')}</span></div>
      ...
      {row.counterparty_response && <div className="mt-4 bg-[#f1f1f1] p-3 text-sm"><strong>Other party response:</strong><p className="mt-1 whitespace-pre-wrap">{row.counterparty_response}</p></div>}
      {row.resolution && <div className="mt-4 bg-green-50 p-3 text-sm"><strong>Talent House resolution:</strong><p className="mt-1 whitespace-pre-wrap">{row.resolution}</p>{row.adjusted_payout_amount != null && <p className="mt-2">Professional payout: £{row.adjusted_payout_amount}</p>}{row.approved_refund_amount > 0 && <p>Refund: £{row.approved_refund_amount}</p>}{row.approved_extra_amount > 0 && <p>Additional professional payment: £{row.approved_extra_amount}</p>}</div>}
      {row.status === 'awaiting_response' && row.opened_by_user_id !== userId && <div className="mt-4"><textarea aria-label="Your response" value={responses[row.id] || ''} onChange={e => setResponses(v => ({ ...v, [row.id]: e.target.value }))} className="input-field min-h-24" placeholder="Give your version of what happened, including any hours, sales, expenses or other facts Talent House should consider."/><button disabled={busy} onClick={() => respond(row.id)} className="btn-primary mt-2 text-xs">Send response</button></div>}
      ...
      {row.status === 'under_review' && <p className="text-xs text-amber-700 mt-4">Both sides have been recorded. Talent House Admin is reviewing the case.</p>}
      {row.status === 'awaiting_agreement' && <p className="text-xs text-amber-700 mt-4">Talent House has proposed a resolution. Review it and sign in the case discussion and agreement section below.</p>}
      {role === 'employer' && row.status === 'awaiting_payment' && row.extra_payment_status === 'pending' && <div className="mt-4 flex items-center justify-between gap-4 bg-[#f1f1f1] p-4"><div><p className="font-medium text-ink">Additional payment approved</p><p className="text-xs text-gray-600 mt-1">Professional: £{row.approved_extra_amount} + {agencyFeePct}% Talent House fee £{Math.ceil(Number(row.approved_extra_amount || 0) * AGENCY_PLATFORM_FEE_PCT)}.</p></div><button disabled={busy} onClick={() => payAdjustment(row)} className="btn-primary text-xs">Pay £{Number(row.approved_extra_amount || 0) + Math.ceil(Number(row.approved_extra_amount || 0) * AGENCY_PLATFORM_FEE_PCT)}</button></div>}
