...
export default function ResidencySuggestions({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="mt-2">
      <button type="button" onClick={toggle} className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold text-[#222321] hover:underline">
            ...
            : (
              <div className="flex flex-wrap gap-2">
                {suggestions.map(specialist => (
                  <Link key={specialist.id} href={`/residency/${specialist.id}`} className="inline-flex items-center gap-2 rounded-full border border-[#dcd4c8] bg-[#ede8df] px-3 py-1.5 text-[11.5px] hover:bg-[#ede8df]">
                    ...
                    <span className="rounded-full bg-white px-1.5 py-0.5 text-[10px] font-bold text-[#222321]">{specialist.match_score}%</span>
                  </Link>
                ))}
              </div>
            )}
