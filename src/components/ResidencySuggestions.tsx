...
export default function ResidencySuggestions({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="mt-2">
      <button type="button" onClick={toggle} className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold text-[#1a1a1a] hover:underline">
            ...
            : (
              <div className="flex flex-wrap gap-2">
                {suggestions.map(specialist => (
                  <Link key={specialist.id} href={`/residency/${specialist.id}`} className="inline-flex items-center gap-2 rounded-full border border-[#e5e5e5] bg-[#faf6ec] px-3 py-1.5 text-[11.5px] hover:bg-[#f5eedd]">
                    ...
                    <span className="rounded-full bg-white px-1.5 py-0.5 text-[10px] font-bold text-[#111111]">{specialist.match_score}%</span>
                  </Link>
                ))}
              </div>
            )}
