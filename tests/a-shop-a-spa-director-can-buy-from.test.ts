...
test('a pack knows its own department', () => {
  ...
  // Seven suites priced on their own rather than as departments or as stages:
  ...
  // pack and the job descriptions. Each is priced against what it replaces
  ...
  assert.equal(tierPacks().length, 7)
  assert.ok(tierPacks().some(pack => pack.slug === 'job-descriptions'))
