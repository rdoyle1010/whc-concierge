...
test('the summary is the sentence a buyer repeats to whoever holds the budget', () => {
  assert.equal(formatSummary({ PDF: 36, Excel: 1 }), '36 PDFs · 1 Excel workbook')
  assert.equal(formatSummary({ PDF: 1 }), '1 PDF')
...
test('the shop says what is in a pack before the buy button, not after the payment', () => {
  ...
  // Titles, from the same catalogue the packs are priced from, so the list
  // and the count cannot disagree.
  assert.ok(panel.includes('sellableCatalogue()'))
  assert.ok(panel.includes('kindOf'), 'grouped by kind rather than a wall of titles')
