...
export default function JobsScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
    ...
    <Text style={styles.intro}>{role === 'employer' ? 'The same roles currently managed from your web dashboard.' : 'Roles are ranked using the same WHC matching logic as the website, including progression, skills, location and experience.'}</Text>
    {loading ? <ActivityIndicator color="#092b45" style={{ marginTop: 30 }} /> : null}
    {error ? <Text style={styles.error}>{error}</Text> : null}
    {!loading && !error && rankedJobs.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>Nothing to show yet.</Text><Text style={styles.emptyCopy}>{role === 'employer' ? 'Your posted roles will appear here.' : 'New live opportunities will appear here as employers publish them.'}</Text></View> : null}
    <View style={styles.list}>{rankedJobs.map(({ job, match }) => {
      ...
      return <View key={job.id} style={styles.card}>
        ...
        {role === 'talent' ? <Pressable onPress={() => toggleSave(job.id)} style={styles.save}><Text style={styles.saveText}>{saved.has(job.id) ? 'Saved ✓' : 'Save role'}</Text></Pressable> : null}
      </View>
    })}</View>
  </ScrollView>
}
