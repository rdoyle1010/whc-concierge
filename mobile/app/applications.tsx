...
export default function ApplicationsScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
    ...
    <Text style={styles.intro}>{role === 'employer' ? 'See who has applied and where each candidate sits in the process.' : 'Keep every application, interview and outcome together.'}</Text>
    ...
    <View style={styles.list}>{live.map(item => {
      ...
      return <View key={item.id} style={styles.card}>
        ...
        {role === 'talent' && !['accepted','rejected'].includes(item.status) ? <Pressable onPress={() => withdraw(item)}><Text style={styles.withdraw}>Withdraw interest</Text></Pressable> : null}
      </View>
    })}</View>
  </ScrollView>
}
