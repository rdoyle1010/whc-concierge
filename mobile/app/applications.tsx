...
export default function ApplicationsScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
    ...
    <Text style={styles.eyebrow}>{role === 'employer' ? 'ACTIVE RECRUITMENT' : 'ACTIVE RECRUITMENT'}</Text>
    <Text style={styles.title}>Applications</Text>
    <Text style={styles.intro}>{role === 'employer' ? 'Only active candidates appear here. Completed hires move to Hired & Placement History.' : 'Only roles still moving through recruitment appear here. Completed placements move to Hired & Placement History.'}</Text>
    {loading ? <ActivityIndicator color="#092b45" style={{ marginTop: 30 }} /> : null}
    {error ? <Text style={styles.error}>{error}</Text> : null}
    {!loading && live.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>No active applications.</Text><Text style={styles.emptyCopy}>{role === 'talent' ? 'Find a live role in Jobs and choose Apply with AI to start a new application.' : 'New applications will appear here when Talent apply to your live roles.'}</Text></View> : null}
    <View style={styles.list}>{live.map(item => {
      ...
      return <View key={item.id} style={styles.card}>
        ...
        {role === 'talent' ? <Pressable onPress={() => router.push({ pathname: '/talent-application/[id]', params: { id: item.id } })} style={styles.manage}><Text style={styles.manageText}>{['interview','offered','accepted'].includes(item.status) ? 'View & respond →' : 'View application progress →'}</Text></Pressable> : null}
