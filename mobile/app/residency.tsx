...
export default function ResidencyScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
    ...
    {role === 'talent' ? <>
      ...
      {bookings.length === 0 ? <Text style={styles.empty}>No Residency offers sent yet.</Text> : bookings.map(booking => <View key={booking.id} style={styles.card}>
        <View style={styles.cardTop}><Text style={styles.status}>{booking.status}</Text><Text style={styles.rate}>£{booking.proposed_day_rate}/day</Text></View>
        ...
        {booking.status === 'accepted' ? <Text style={styles.confirmHint}>Accepted · secure Residency payment is the next step.</Text> : null}
