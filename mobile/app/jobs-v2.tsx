...
const BUILD_MARKER='JOB FLOW V2'
...
export default function JobsV2(){
  ...
  async function applyCurrent(){
    ...
    setBusy(true);setError('')
    try{
      await mobileApi('/api/mobile/job-swipes',{method:'POST',body:JSON.stringify({targetId:id,action:'right'})})
  ...
  function animateAction(direction:'left'|'right'){
    if(!current||busy)return
    Animated.timing(position,{toValue:{x:direction==='right'?SCREEN_WIDTH:-SCREEN_WIDTH,y:0},duration:170,useNativeDriver:false}).start(()=>{
      if(direction==='right')void applyCurrent(); else void passCurrent()
    })
  }
  ...
  async function resetDeck(){
    ...
    try{
      ...
      setSaved(new Set());setPassed(new Set());position.setValue({x:0,y:0})
      await load()
  ...
  const panResponder=useMemo(()=>PanResponder.create({
    ...
    onPanResponderRelease:(_,g)=>{
      if(g.dx>SWIPE_THRESHOLD)animateAction('right')
      else if(g.dx< -SWIPE_THRESHOLD)animateAction('left')
      else resetPosition()
    },
  }),[current?.job.id,busy])
  ...
  return <View style={styles.screen}>
    <ScrollView contentContainerStyle={styles.page} scrollEnabled={!current}>
      <Text style={styles.marker}>{BUILD_MARKER}</Text>
      <Text style={styles.eyebrow}>OPPORTUNITIES</Text>
      <Text style={styles.title}>Your matched roles</Text>
      <Text style={styles.intro}>Swipe left to pass. Swipe right to apply with AI. You can also use the buttons below — no gesture is required.</Text>
      {loading?<ActivityIndicator color="#092b45" style={{marginTop:30}}/>:null}
      {error?<Text style={styles.error}>{error}</Text>:null}
      {!loading&&!error&&!current?<View style={styles.empty}>
        <Text style={styles.emptyTitle}>{jobs.length?'You’ve reviewed every live role.':'No live roles right now.'}</Text>
        <Text style={styles.emptyCopy}>{jobs.length?'Reset the deck to bring all current roles back for review.':'New matching roles will appear here automatically.'}</Text>
        {jobs.length?<Pressable onPress={resetDeck} style={styles.reset}><Text style={styles.resetText}>{busy?'Resetting…':'Reset swipe deck'}</Text></Pressable>:null}
      </View>:null}
