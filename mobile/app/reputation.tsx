...
type Review = { id:string; rating:number; text?:string|null; property_name?:string|null; created_at?:string|null }
...
export default function ReputationScreen(){
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
    ...
    <Text style={styles.intro}>{role==='talent'?'Verified reviews and employer references build trust from work actually completed through Wellness House.':'See what Talent says about working with your property and respond to verified reference requests.'}</Text>
    {loading?<ActivityIndicator color="#092b45" style={{marginTop:20}}/>:null}{error?<Text style={styles.error}>{error}</Text>:null}

    {!loading&&profile?<View style={styles.scoreCard}><Text style={styles.score}>{Number(profile.reviewScore||0).toFixed(1)}</Text><View style={{flex:1}}><Text style={styles.scoreTitle}>out of 5</Text><Text style={styles.scoreCopy}>{profile.reviewCount||0} verified review{Number(profile.reviewCount||0)===1?'':'s'} · {profile.name}</Text></View></View>:null}
