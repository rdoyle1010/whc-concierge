...
export default function EmployerMatchScreen(){
  ...
  async function api(options?:RequestInit){
    ...
    const response=await fetch(`${WEB_URL}/api/mobile/employer-directory`,{
      ...options,
  ...
  async function load(){
    ...
    try{
      ...
      setLiveRoleCount(Number(data.live_role_count||0))
    }catch(e:any){setError(e?.message||'Could not load Talent.')}
    finally{setLoading(false)}
  }

  async function decide(candidate:Candidate,action:'left'|'right'|'save'|'unsave'){
    if(busy)return
    if(action==='right'&&!candidate.bestJobId){
      Alert.alert('Post a suitable live role first','Interested is role-specific, just like the website. This professional does not currently have an eligible live role match.')
      return
    }
    ...
    try{
      ...
      if(action==='save'||action==='unsave'){
        setCandidates(current=>current.map(item=>item.id===candidate.id?{...item,shortlisted:action==='save'}:item))
        return
      }
      setCandidates(current=>current.map(item=>item.id===candidate.id?{...item,interested:true,mutual:Boolean(result.matched)}:item))
      if(result.matched){
        Alert.alert("It's a match!",`You and ${result.candidateName||candidate.full_name||'this professional'} both said yes to ${result.jobTitle||candidate.bestJob}. Your WHC conversation is now open.`,[
          {text:'Start conversation',onPress:()=>router.push('/messages')},
          {text:'Keep browsing',style:'cancel'},
        ])
      }else{
        Alert.alert('Interest saved',`Your interest in ${candidate.full_name||'this professional'} for ${candidate.bestJob||'this role'} has been recorded. You can keep browsing.`)
      }
  ...
  function candidateCard(candidate:Candidate){
    ...
    return <View key={candidate.id} style={[styles.card,candidate.is_featured&&styles.featuredCard]}>
      ...
      {candidate.mutual?<Pressable onPress={()=>router.push('/messages')} style={styles.matchRow}><Text style={styles.matchText}>It’s a match · Start conversation</Text><Text style={styles.matchArrow}>→</Text></Pressable>:null}
    </View>
  }
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.intro}>The same matching journey as the website: Pass, privately Save, or show role-specific Interest. You stay in the directory and keep browsing.</Text>
