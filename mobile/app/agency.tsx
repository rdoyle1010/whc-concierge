...
export default function AgencyScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.intro}>{role === 'talent' ? 'Set different hours for each day, then see the exact travel information before accepting work.' : 'Find available spa professionals for exact shift times, then manage every offer and booking here.'}</Text>
    ...
    {role === 'employer' && !loading ? <Pressable onPress={() => router.push('/agency-search')} style={styles.findTalentCard}>
      ...
      <Text style={styles.findTalentTitle}>Find Agency Talent</Text>
      <Text style={styles.findTalentCopy}>Search by exact date and hours. We only show professionals who are approved, within the travel rules and available for the whole shift.</Text>
      <Text style={styles.findTalentLink}>Search available professionals →</Text>
    </Pressable> : null}
    ...
    {!loading ? <View style={styles.section}>
      ...
      {upcoming.length === 0 ? <Text style={styles.emptyCopy}>No agency bookings to show yet.</Text> : upcoming.map(booking => {
        ...
        const actionable = ['pending', 'countered'].includes(booking.status)
        return <View key={booking.id} style={styles.card}>
          ...
          {actionable ? <Pressable onPress={() => router.push({ pathname: '/agency-booking/[id]', params: { id: booking.id } })} style={styles.actionButton}><Text style={styles.actionButtonText}>{role === 'talent' ? 'Respond to shift offer →' : booking.status === 'countered' ? 'Review counter offer →' : 'View live offer →'}</Text></Pressable> : null}
...
const styles = StyleSheet.create({
  scroll:{flex:1,backgroundColor:'#fff'}, page:{paddingHorizontal:22,paddingTop:18,paddingBottom:34}, back:{color:'#66747c',fontSize:14,marginBottom:26}, eyebrow:{color:'#71808a',fontSize:8,letterSpacing:2,marginBottom:8}, title:{color:'#092b45',fontSize:29,lineHeight:34,fontWeight:'500'}, intro:{color:'#66747c',fontSize:13,lineHeight:20,marginTop:8,marginBottom:22},
  findTalentCard:{backgroundColor:'#092b45',padding:18,marginBottom:24}, findTalentEyebrow:{color:'#dce7ec',fontSize:8,letterSpacing:1.4,fontWeight:'700'}, findTalentTitle:{color:'#fff',fontSize:18,fontWeight:'600',marginTop:6}, findTalentCopy:{color:'#dce7ec',fontSize:11,lineHeight:17,marginTop:6}, findTalentLink:{color:'#fff',fontSize:11,fontWeight:'700',marginTop:12},
  section:{marginBottom:24}, sectionTitle:{color:'#173246',fontSize:16,fontWeight:'600',marginBottom:7}, help:{color:'#71808a',fontSize:10,lineHeight:16,marginBottom:12}, availabilityCard:{borderWidth:1,borderColor:'#dce3e7',padding:14,marginBottom:9}, dayTop:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}, rowMain:{color:'#173246',fontSize:13,fontWeight:'600'}, savedState:{color:'#71808a',fontSize:8,marginTop:3,textTransform:'uppercase',letterSpacing:.7},
  timeRow:{flexDirection:'row',gap:9,marginTop:11}, label:{color:'#173246',fontSize:9,fontWeight:'600',marginBottom:5}, input:{borderWidth:1,borderColor:'#d7e0e4',paddingHorizontal:9,paddingVertical:9,color:'#173246',fontSize:11}, buttonRow:{flexDirection:'row',alignItems:'center',gap:8,marginTop:11,flexWrap:'wrap'}, saveButton:{backgroundColor:'#092b45',paddingHorizontal:12,paddingVertical:10}, saveText:{color:'#fff',fontSize:9,fontWeight:'700'}, outlineButton:{borderWidth:1,borderColor:'#cfd9de',paddingHorizontal:11,paddingVertical:9}, outlineText:{color:'#66747c',fontSize:9,fontWeight:'600'}, clearText:{color:'#7a4a4a',fontSize:9,padding:7}, disabled:{opacity:.45},
  card:{borderWidth:1,borderColor:'#dce3e7',padding:16,marginBottom:9}, topRow:{flexDirection:'row',justifyContent:'space-between'}, status:{color:'#71808a',fontSize:8,textTransform:'uppercase',letterSpacing:1.1}, urgent:{color:'#8a3434',fontSize:8,letterSpacing:1.1,fontWeight:'700'}, cardTitle:{color:'#173246',fontSize:16,fontWeight:'600',marginTop:7}, cardCopy:{color:'#66747c',fontSize:11,lineHeight:17,marginTop:5}, travelBox:{backgroundColor:'#f5f8f9',padding:12,marginTop:12}, travelTitle:{color:'#173246',fontSize:10,fontWeight:'700',marginBottom:4}, travelLine:{color:'#66747c',fontSize:10,lineHeight:15,marginTop:2}, warning:{color:'#8a3434',fontSize:10,lineHeight:15,marginTop:4,fontWeight:'600'}, actionButton:{borderTopWidth:1,borderTopColor:'#edf1f3',paddingTop:12,marginTop:12}, actionButtonText:{color:'#092b45',fontSize:10,fontWeight:'700'}, standing:{color:'#71808a',fontSize:8,marginTop:6,textTransform:'uppercase',letterSpacing:.7}, reviewButton:{borderTopWidth:1,borderTopColor:'#edf1f3',paddingTop:11,marginTop:12}, reviewButtonText:{color:'#092b45',fontSize:10,fontWeight:'700'}, reviewed:{color:'#456655',fontSize:9,fontWeight:'600',marginTop:9}, emptyCopy:{color:'#71808a',fontSize:11,lineHeight:17,backgroundColor:'#f4f7f8',padding:15}, error:{color:'#9b2c2c',fontSize:11,marginBottom:16}
})
