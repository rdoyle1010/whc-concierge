...
export default function NotificationsScreen(){
 ...
 async function load(){
  ...
  const {data}=await supabase.from('notifications').select('id,type,title,message,link,is_read,opened_at,done_at,created_at').eq('user_id',user.id).order('created_at',{ascending:false}).limit(50)
 ...
 async function markOpened(item:any){
  ...
  const now=new Date().toISOString()
  const {error}=await supabase.from('notifications').update({opened_at:now}).eq('id',item.id)
  if(!error)setItems(current=>current.map(row=>row.id===item.id?{...row,opened_at:now}:row))
 }
 ...
 const outstanding=items.filter(item=>!item.done_at).length
 return <ScrollView style={styles.scroll} contentContainerStyle={styles.page}>
  <Pressable onPress={()=>router.back()}><Text style={styles.back}>‹ Back</Text></Pressable><Text style={styles.eyebrow}>UPDATES</Text><Text style={styles.title}>Notifications</Text><Text style={styles.intro}>Updates now have a real state. Opening something changes it from New to Open. It only stops counting when you mark it Done. Agency and message actions stay separate until the actual action is completed.</Text>
